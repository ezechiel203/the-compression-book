...
In H.264, every frame is divided into fixed 16×16 pixel *macroblocks*. A macroblock can be further subdivided (down to 4×4 blocks for certain prediction modes), but the top-level grid is always 16×16. This was fine in 2003, when most video was 720×576 or 1280×720. But at 3840×2160 (4K UHD), a 16×16 macroblock covers a tiny sliver of the scene. Large flat areas like a blue sky, a concrete wall, or a field of grass could be represented far more efficiently with a single large block than with thousands of 16×16 tiles each carrying their own overhead.
...
#fig(
  ...
  cetz.canvas({
    ...
    // labels
    content((1,3), box(width: 1.6cm, align(center, text(size:7pt)[32×32])))
    content((3,3), box(width: 1.6cm, align(center, text(size:7pt)[32×32])))
    content((0.5,1.5), box(width: 0.8cm, align(center, text(size:6pt)[16])))
    content((1.5,1.5), box(width: 0.8cm, align(center, text(size:6pt)[16])))
    content((0.5,0.5), box(width: 0.8cm, align(center, text(size:6pt)[16])))
    content((1.5,0.5), box(width: 0.8cm, align(center, text(size:6pt)[16])))
    content((2.5,1.5), box(width: 0.8cm, align(center, text(size:6pt)[16])))
    content((3.5,1.5), box(width: 0.8cm, align(center, text(size:6pt)[16])))
    content((2.5,0.5), box(width: 0.8cm, align(center, text(size:6pt)[16])))
    content((3.5,0.5), box(width: 0.8cm, align(center, text(size:6pt)[16])))
    content((2,4.4), box(width: 3.2cm, align(center, text(size:7.5pt, fill: rgb("#0b5394"), weight: "bold")[64×64 CTU])))
  })
)
...
HEVC made a clean design decision: *only CABAC*. The H.264 CABAC context model was reworked from scratch, with fewer contexts overall but better tuning for the new syntax elements introduced by the quadtree structure. The result: HEVC's CABAC achieves roughly 10–15% better compression than H.264's CABAC on the entropy-coding step alone. Simplifying to one mode also simplified decoder chips, which now only needed to implement one entropy path.
...
At 4K resolution on a 16-core decoder chip, these structures allow near-linear speedup with core count, which is essential for real-time 4K playback on streaming devices.
...
#fig(
  ...
  cetz.canvas({
    ...
    content((bw/2, bh/2), box(width: 0.8cm, align(center, text(size:5pt)[None])))
    // QT
    let ox = bw + gap
    ...
    content((ox+bw/2, -0.18), box(width: 0.8cm, align(center, text(size:5.5pt)[QT])))
    // HBT
    let ox2 = ox + bw + gap
    rect((ox2,0),(ox2+bw,bh), stroke: 0.5pt)
    line((ox2,bh/2),(ox2+bw,bh/2), stroke: 0.8pt + rgb("#783f04"))
    content((ox2+bw/2, -0.18), box(width: 0.8cm, align(center, text(size:5.5pt)[HBT])))
    // VBT
    let ox3 = ox2 + bw + gap
    rect((ox3,0),(ox3+bw,bh), stroke: 0.5pt)
    line((ox3+bw/2,0),(ox3+bw/2,bh), stroke: 0.8pt + rgb("#783f04"))
    content((ox3+bw/2, -0.18), box(width: 0.8cm, align(center, text(size:5.5pt)[VBT])))
    // HTT
    let ox4 = ox3 + bw + gap
    ...
    content((ox4+bw/2, -0.18), box(width: 0.8cm, align(center, text(size:5.5pt)[HTT])))
    // VTT
    let ox5 = ox4 + bw + gap
    ...
    content((ox5+bw/2, -0.18), box(width: 0.8cm, align(center, text(size:5.5pt)[VTT])))
  })
)

Why do non-square splits help so much? Consider a vertical edge in the frame, say the border between a bright window and a dark wall. In HEVC, the quadtree can only split the block containing this edge into four square pieces. The edge might still cut diagonally through each piece, leaving costly residuals everywhere. With a vertical binary split, the encoder can place the block boundary exactly along the edge, giving one block that is pure bright and one that is pure dark. Both can be predicted trivially, leaving near-zero residuals.
...
This covers translation, rotation, scaling, and shear, all of which are common in real video. The encoder signals the affine parameters rather than a single motion vector. The decoder applies the affine warp to the reference frame to generate the prediction.
...
#gomaths("Affine Transformations")[
  ...
  The $2 times 2$ matrix handles rotation, scaling, and shear; the vector $(c_0, f_0)$ handles translation. If $a=e=1$ and $b=d=0$, you get pure translation, identical to an H.264/HEVC motion vector. The extra four parameters ($a, b, d, e$) cost a few more bits to signal but eliminate large residuals for rotating or zooming objects. This is a classic rate-distortion trade-off: spend a few more bits on the model to save many more bits on the residual.
]
...
#fig(
  ...
  cetz.canvas({
    ...
    content((3.5,5.85), box(width: 2.6cm, inset: 1pt, align(center, text(size:6.5pt, weight:"bold")[Target hardware has VVC decode?])))
    // Yes branch
    line((3.5,5.5),(1.5,4.5), stroke: 0.7pt)
    content((2.1,5.05), box(width: 0.8cm, align(center, text(size:5.5pt, fill: rgb("#0b6e4f"))[Yes])))
    rect((0.5,3.8),(2.5,4.5), fill: rgb("#e8faf0"), stroke: 0.8pt + rgb("#0b6e4f"), radius: 2pt)
    content((1.5,4.15), box(width: 1.6cm, inset: 1pt, align(center, text(size:6.5pt)[Use VVC (H.266)])))
    // No branch - AV1
    line((3.5,5.5),(5.5,4.5), stroke: 0.7pt)
    content((4.9,5.05), box(width: 0.8cm, align(center, text(size:5.5pt, fill: rgb("#783f04"))[No])))
    rect((4.5,3.4),(6.5,4.5), fill: rgb("#faf7ee"), stroke: 0.8pt + rgb("#783f04"), radius: 2pt)
    content((5.5,3.95), box(width: 1.6cm, inset: 1pt, align(center, text(size:6.5pt)[Web/mobile? Use AV1 (royalty-free)])))
    // Legacy
    line((3.5,5.5),(3.5,4.5), stroke: 0.7pt)
    content((3.7,5.0), box(width: 1.0cm, align(center, text(size:5.5pt)[Legacy?])))
    rect((2.7,3.4),(4.3,4.5), fill: rgb("#faf0f0"), stroke: 0.8pt + rgb("#9a2617"), radius: 2pt)
    content((3.5,3.95), box(width: 1.3cm, inset: 1pt, align(center, text(size:6.5pt)[Use HEVC (H.265)])))
  })
)
...
#fig(
  ...
  cetz.canvas({
    ...
    content((1.0, 1.1), box(width: 1.0cm, inset: 1pt, align(center, text(size:6.5pt, weight:"bold")[H.264/ AVC])))
    content((1.0, 0.65), box(width: 1.0cm, align(center, text(size:6pt, fill: rgb("#783f04"))[2003])))
    content((1.0, 0.25), box(width: 1.0cm, align(center, text(size:6pt, fill: rgb("#0b6e4f"))[1.0x rate])))
    // Arrow 1
    line((1.6, 0.75),(2.4, 0.75), mark: (end: ">"), stroke: 0.7pt + rgb("#0b5394"))
    content((2.0, 1.0), box(width: 0.6cm, align(center, text(size:5.5pt, fill:rgb("#783f04"))[~50% less])))
    // H.265 box
    rect((2.4, 0),(3.6, 1.5), fill: rgb("#e8f0fa"), stroke: 0.8pt + rgb("#0b5394"), radius: 3pt)
    content((3.0, 1.1), box(width: 1.0cm, inset: 1pt, align(center, text(size:6.5pt, weight:"bold")[H.265/ HEVC])))
    content((3.0, 0.65), box(width: 1.0cm, align(center, text(size:6pt, fill: rgb("#783f04"))[2013])))
    content((3.0, 0.25), box(width: 1.0cm, align(center, text(size:6pt, fill: rgb("#0b6e4f"))[~0.5x rate])))
    // Arrow 2
    line((3.6, 0.75),(4.4, 0.75), mark: (end: ">"), stroke: 0.7pt + rgb("#0b5394"))
    content((4.0, 1.0), box(width: 0.6cm, align(center, text(size:5.5pt, fill:rgb("#783f04"))[~50% less])))
    // H.266 box
    rect((4.4, 0),(5.6, 1.5), fill: rgb("#e8f0fa"), stroke: 0.8pt + rgb("#0b5394"), radius: 3pt)
    content((5.0, 1.1), box(width: 1.0cm, inset: 1pt, align(center, text(size:6.5pt, weight:"bold")[H.266/ VVC])))
    content((5.0, 0.65), box(width: 1.0cm, align(center, text(size:6pt, fill: rgb("#783f04"))[2020])))
    content((5.0, 0.25), box(width: 1.0cm, align(center, text(size:6pt, fill: rgb("#0b6e4f"))[~0.25x rate])))
    content((3.0, -0.25), box(width: 5.0cm, align(center, text(size:6.5pt)[Bitrate relative to H.264 at equal quality])))
  })
)
...
#takeaways((
  ...
  "VVC (H.266, finalized 6 July 2020) replaced HEVC's pure quadtree with a Multi-Type Tree (MTT) supporting binary and ternary splits, expanded CTUs to 128x128, added affine motion compensation, palette mode, and an adaptive loop filter. The result is roughly 50% better compression than HEVC.",
  ...
  "In December 2025, Access Advance acquired Via LA's HEVC and VVC patent pool programs, consolidating administration under the VCL Advance brand. Essential patent holders outside the pool remain, and the situation is not fully resolved.",
