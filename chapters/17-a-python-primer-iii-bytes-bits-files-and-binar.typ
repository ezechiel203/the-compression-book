...
The two shift operators move all the bits of a number sideways by a given number of positions. *Left shift* `x << k` slides every bit up by $k$ places, filling the vacated low positions with zeros. Since each position up doubles the place value, this is exactly multiplication by $2^k$. *Right shift* `x >> k` slides every bit down by $k$ places, dropping the bits that fall off the bottom: integer division by $2^k$.
...
$ x >> k = floor(x / 2^k). $
So `48 >> 4` is `3` exactly, but `50 >> 4` is also `3` (the leftover 2 falls off the bottom). Right-shifting is the fast way to ask "how many whole groups of $2^k$?" (the same question floor division `//` answers, but a CPU does a shift in a single step). Compressors lean on shifts because they are both meaningful (place value) and blisteringly fast.
]
...
This surprises everyone the first time. The practical fix, whenever you want NOT to stay inside a byte, is to *mask back down* to 8 bits afterward with `& 0xFF`. AND against `0b11111111` keeps only the low 8 bits and discards the infinite sign extension:
...
A compressor that cannot read a file and write a file is a toy. Python's gateway to the filesystem is one built-in function, `open`, which hands you a *file object* you can read from or write to. The key choice, for us, is the *mode*: whether you open the file in *text* mode or *binary* mode. We will almost always want binary.

#gopython("`open`, modes, and the `with` block")[
`open(path, mode)` returns a file object. The mode is a short string: `"r"` to read, `"w"` to write (creating or truncating), `"a"` to append. A `"b"` appended to any of these selects *binary* mode. So `"rb"` means "read binary" and `"wb"` means "write binary." Binary mode reads and writes `bytes`; text mode reads and writes `str` and silently applies an encoding. For compression you *always* want binary, so you control every byte.
...
#fig([The `BitWriter` accumulates bits in an integer, most-significant-first. As each eighth bit arrives, a finished byte is shifted out to the buffer. Leftover bits are zero-padded on `flush`.], cetz.canvas({
  ...
  // incoming bit
  content((4.0, 0.5), box(width: 5cm, align(center, text(size: 8pt)[new bit enters here; `acc <<= 1` makes room])))
...
#aside[
Python 3.14, released 7 October 2025, reorganised these modules under a new `compression` package (so `compression.gzip`, `compression.zstd`, and friends now sit together, with the old top-level names kept as aliases for compatibility). The headline addition was `compression.zstd`, bringing first-class Zstandard support to the standard library at last. We will keep using the familiar short names in this book, but if you see `from compression import zstd` in modern code, that is the 3.14 home for it.
]
...
#exercise("17.8", 3)[
Design and implement a tiny self-describing format `pack_run(data: bytes) -> bytes` and its inverse `unpack_run(blob: bytes) -> bytes` that performs *run-length encoding* on bytes: replace each maximal run of an identical byte with a (count, value) pair, where count is a single byte 1–255. Include a 4-byte big-endian header giving the original length, and round-trip it. (This is a real, if simple, compressor.)
]
#solution("17.8")[
```python
import struct
...
#exercise("17.10", 3)[
A `BitReader` reading a stream that ends mid-byte will, after the last real bit, return *padding* bits as if they were data. Modify the `tinyzip` container so the reader stops at exactly the right bit. Propose two designs: (a) store an explicit "number of valid bits in the final byte" in the header, and (b) store the total symbol count. Argue which is more robust for a variable-length code like Huffman, where you do not know the bit count until after encoding.
]
#solution("17.10")[
