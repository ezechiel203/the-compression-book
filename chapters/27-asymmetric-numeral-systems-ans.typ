...
Here is the question that opens ANS up. In base 10, every digit gets an *equal*
...
*small* one. As Shannon taught us in Chapter 18, a common symbol carries
...
ANS is precisely this `push`/`pop` pair, with `* 10` replaced by a step that
...
everything straight off the ownership table above, with no closed-form magic, just the
...
- *Decode* from state $x'$: read who owns slot $x'$ (which recovers $s$), then
...
table(columns: (auto, auto, 1fr, auto), inset: 6pt, align: (center, center, left, center),
...
cetz.canvas({
  ...
  content((2, 1.45))[#box(width: 3.6cm, align(center, text(size: 8pt)[$F_a=4, B_a=0$]))]
  content((5.5, 1.45))[#box(width: 2.6cm, align(center, text(size: 8pt)[$F_b=3, B_b=4$]))]
  content((7.5, 1.85))[#box(width: 1.0cm, align(center, text(size: 8pt)[$F_c=1$]))]
  content((7.5, 1.45))[#box(width: 1.0cm, align(center, text(size: 8pt)[$B_c=7$]))]
}))
...
These two are exact inverses (you will prove it in the exercises). Let us read
...
$x=8$.],
table(columns: (auto, auto, 1fr, auto), inset: 6pt, align: (center, center, left, center),
  table.header([*step*], [*sym*], [*encode step: $x' = floor(x \/ F_s) dot M + B_s + (x mod F_s)$*], [*new $x$*]),
  [start], [-], [-], [8],
...
`a` in reverse order, as promised.
...
end up sprinkled rather than clumped, giving the even spreading that keeps the
...
#project("Step 11 · rANS encode/decode in `ans.py`")[
  ...
  (`huffman.encode`/`decode` in Chapter 24 and `arithmetic.encode`/`decode`
  in Chapter 26), `ans.py` also exposes the same public `encode(data: bytes) -> bytes`
  ...
  $D(x)$ formulas from the worked example. Note that `<< M_BITS` is the multiply-by-$M$
...
#gopython("`reversed()` and why we encode backwards")[
  ...
  ```
  ANS decodes LIFO: it recovers the *last-encoded* symbol *first*. To make the
...
ANS has a human story too, one that reads like a parable about who owns ideas.
...
compressors on earth - deployed in the Linux kernel, databases, and package managers - using
...
- Steiner, De Vita & Bezati (2025), _Optimal Tables for Asymmetric Numeral
  Systems_: #link("https://arxiv.org/abs/2504.18541")[arXiv:2504.18541], with provably
