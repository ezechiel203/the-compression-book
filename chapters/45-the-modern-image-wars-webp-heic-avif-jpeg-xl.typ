...
and JPEG itself still reigning as the universal floor. How did we end up with
...
liability is uncertain — a nightmare for a browser vendor shipping code to
billions of users at no charge.
...
- *Film grain synthesis* — rather than compressing grain (expensive), AV1 can discard it and instruct the decoder to re-add statistically matching noise. For grainy photographs or cinematic content this saves bits with minimal visible quality change.
...
- And critically: it is *royalty-free by design*. The Alliance's license grants a perpetual, worldwide, royalty-free, no-charge right to make, use, sell, and distribute any implementation.
...
itself is royalty-free; only the codec inside determines the license). A single
...
- *Resolution limits.* AV1 was designed for video resolutions; very large
  ...
  metadata.
- *Slow encoding.* AV1 encoding is computationally expensive — real-time
...
browser coverage — essentially universal. The AVIF v1.2.0 spec (November 2025)
...
#algo(
  ...
  re-addition in February 2026 — has become the defining case study for how
...
#scoreboard(
  caption: "Modern image formats vs JPEG (approximate, photographic content, equivalent SSIMULACRA2 quality)",
  ...
  [JPEG XL (2022)], [~60%], [Yes], [Yes], [~16% → 85–90%†], [Royalty-free],
)
...
*Learned / neural image codecs* — formats like BPG (not well-known), VVC/H.266
...
#solution("45.2")[
  ...
  sizes, and better entropy coding than JPEG's fixed 8×8 DCT pipeline —
  ...
  from shipping HEIC support, making it inaccessible on the open web.
]
...
#solution("45.6")[
  ...
  the entire 64×64 area — much more visible than ringing confined to one 8×8
