...
Chapter 19 left a small debt unpaid. When you compress data with a code built for the _wrong_ probabilities — believing symbol $x$ has probability $q(x)$ when its true probability is $p(x)$ — you do not merely "do a little worse." You overpay by a precise, computable number of bits, and that number has a name: the _Kullback--Leibler divergence_, or _relative entropy_. We pay that debt now, because this one quantity is the seed from which mutual information, channel capacity, and half of modern machine learning all grow.
...
There is a second, even more illuminating way to write mutual information that connects it straight back to the previous section. Two variables are independent exactly when their joint distribution factorizes, $p(x, y) = p(x) p(y)$. So a natural way to measure how _far_ they are from independent is to ask how badly the true joint $p(x, y)$ differs from the pretend-they're-independent product $p(x) p(y)$ — and we now have the perfect tool for the gap between two distributions, the KL divergence:
...
#fig([The two-circle (Venn) picture of entropy. The whole shape is $H(X,Y)$; the overlap is $I(X;Y)$; the crescents are the conditional entropies $H(X|Y)$ and $H(Y|X)$.], cetz.canvas({
  ...
  content((0.9, -2.15))[$H(X,Y) = $ everything inside either circle]
}))
...
So although you send one full bit, the receiver learns only about $0.531$ bits about your intended bit — the noise ate the other $0.469$. That number, $0.531$ bits per use, becomes the _capacity_ of this channel in the next section: the hard ceiling on how fast you can communicate reliably across it.
...
the very same $0.531$ — and notice this is _identical_ arithmetic to the lazy-engineer KL example, because the joint distribution here happens to coincide with that $p$-versus-$q$ comparison. The two routes to mutual information, "drop in conditional entropy" and "divergence from independence," agree to the last decimal, exactly as the theory promised. The symmetry $I(X;Y) = I(Y;X)$ is visible too: the formula is unchanged when you swap $x$ and $y$.

#keyidea[
Mutual information $I(X; Y)$ is _one number with three readings_. (1) _Compression:_ the redundancy _between_ two variables — bits you can save on $Y$ by exploiting your knowledge of $X$. (2) _Channels:_ the bits about the input that survive to the output — the throughput. (3) _Statistics:_ how far the pair is from independent, $D_"KL"(p(x,y) || p(x)p(y))$. The rest of this chapter is the discovery that these three readings are not analogies — they are the _same theorem_ from three sides.
]
...
One consequence of mutual information is so useful, and so often forgotten, that it deserves its own statement. Suppose information flows in a chain: $X$ influences $Y$, and $Y$ alone influences $Z$, with $Z$ depending on $X$ only _through_ $Y$. Mathematicians say $X -> Y -> Z$ form a _Markov chain_: once you know $Y$, the value of $Z$ carries no further trace of $X$. Then post-processing $Y$ can only lose information about $X$.
...
#fig([Sphere-packing. Each codeword (dot) blossoms into a cloud of likely received sequences of size $approx 2^(n H(Y|X))$. The whole output space holds $approx 2^(n H(Y))$ typical sequences. The number of non-overlapping clouds — distinguishable messages — is the ratio $approx 2^(n I(X;Y))$.], cetz.canvas({
  ...
  content((3.5, 4.35))[output space $approx 2^(n H(Y))$ sequences]
  ...
  content((6.0, 0.45))[cloud $approx 2^(n H(Y|X))$]
}))
...
These are mirror images. Compression seeks the _most informative_ representation — strip every predictable bit, because a predictable bit is a wasted bit. Channel coding seeks the _most robust_ representation — add back predictable, structured bits, because in a noisy world that very predictability lets the receiver catch and repair mistakes. The compressor's output, ideally, looks like random noise (maximum information density, no exploitable pattern); the channel coder's output, ideally, has _exactly the right amount_ of exploitable pattern to survive the noise. One drives toward randomness; the other away from it. And the unit of account on both sides is the bit, calibrated by mutual information.
...
#scoreboard(caption: "unchanged bytes, newly explained — the gap above the floor is exactly KL divergence",
  [Entropy floor $H$ (Ch. 19)], [—], [—], [the unbeatable target, $H(p)$ bits/symbol],
  [Any real coder], [—], [$> H$], [overspend $= D_"KL"(p||q)$, the model's error in bits],
  [Channel overhead], [—], [$times 1\/C$], [error-correction multiplies size by $1\/C$ on a noisy link],
)
