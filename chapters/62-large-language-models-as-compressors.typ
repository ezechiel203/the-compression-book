...
#recap[
  ...
  - In *Chapter 18* we defined the *surprisal* of a symbol with probability $p$ as $-log_2 p$ bits, and the *entropy* $H(X) = -sum_i p_i log_2 p_i$ as the average surprisal, which is the theoretical floor on bits per symbol.
...
#gomaths("Nats vs bits: same idea, different ruler")[
  ...
  They differ by a fixed scale factor, because $log_2 p = (ln p)/(ln 2)$. Since $ln 2 approx 0.6931$, one nat $= 1/0.6931 approx 1.4427$ bits. A loss of $2.0$ nats/symbol is $2.886$ bits/symbol. It is centimetres vs inches. But you *must* convert, or your file-size predictions will be off by 44%.
]
...
#table(columns: (1.6fr, 1fr, 1fr, 1fr), inset: 6pt, align: (left, right, right, left),
  ...
  [ImageNet patches], [58.5%], [n/a], [*43.4%*],
  [LibriSpeech audio], [n/a], [n/a], [*16.4%*],
)
...
But the rows that made jaws drop are the other two. *ImageNet patches* are little squares of photographs. *LibriSpeech* is recorded human speech. Chinchilla was never trained on either; it was fed the raw bytes of images and audio as if they were a foreign language. It compressed the images to *43.4%* of their size, beating *PNG* (Chapter 44) at 58.5%, a codec built by humans specifically for images. It compressed the audio to *16.4%*, beating *FLAC* (Chapter 50) at 30.3%, a codec built specifically for lossless audio.
...
#fig([The universal-predictor pipeline: the same arithmetic coder is fed bytes from text, image, or audio; only the model-supplied probabilities differ. A general enough predictor beats the specialised codec for each modality.],
  cetz.canvas({
    ...
    content((1.1, 2.4), box(width: 1.8cm, inset: 1pt, align(center, text(size: 8pt)[text bytes])))
    rect((0, 1.0), (2.2, 1.8), fill: rgb("#eef4fb"), stroke: 0.6pt)
    content((1.1, 1.4), box(width: 1.8cm, inset: 1pt, align(center, text(size: 8pt)[image bytes])))
    rect((0, 0.0), (2.2, 0.8), fill: rgb("#eef4fb"), stroke: 0.6pt)
    content((1.1, 0.4), box(width: 1.8cm, inset: 1pt, align(center, text(size: 8pt)[audio bytes])))
    // model
    rect((3.6, 0.7), (6.1, 2.1), fill: rgb("#e8f5ee"), stroke: 0.7pt)
    content((4.85, 1.65), box(width: 2.1cm, inset: 1pt, align(center, text(size: 8pt)[language])))
    content((4.85, 1.25), box(width: 2.1cm, inset: 1pt, align(center, text(size: 8pt)[model $p_theta$])))
    content((4.85, 0.9), box(width: 2.1cm, inset: 1pt, align(center, text(size: 7pt)[(frozen)])))
    // coder
    rect((7.5, 0.7), (9.6, 2.1), fill: rgb("#fbf3ec"), stroke: 0.7pt)
    content((8.55, 1.55), box(width: 1.7cm, inset: 1pt, align(center, text(size: 8pt)[arithmetic])))
    content((8.55, 1.15), box(width: 1.7cm, inset: 1pt, align(center, text(size: 8pt)[coder])))
    // output
    rect((10.9, 1.0), (12.4, 1.8), fill: rgb("#f4f6f8"), stroke: 0.6pt)
    content((11.65, 1.4), box(width: 1.1cm, inset: 1pt, align(center, text(size: 8pt)[bits])))
    // arrows
...
Now recall that neural networks are floating-point machines (Chapter 13). Floating-point arithmetic is *not* associative: $(a + b) + c$ can differ from $a + (b + c)$ in the last bit. Different GPUs, different CPUs, different math libraries, even different thread counts can sum the same numbers in different orders and get answers that differ in the last bit. For ordinary AI that is invisible. For arithmetic coding it is *fatal*.
...
#project("Step 22 · `llmzip.py` - model P(next) + arithmetic coder")[
  ...
  The trick to reuse is to notice what Chapter 26's coder actually asks of its model. There, the `FrequencyModel` exposed exactly three things the coder calls: `cumulative(sym) -> (cum_low, cum_high, total)`, `find(scaled) -> sym`, and `update(sym)`. The coder did *not care* where those numbers came from. That is the model-plus-coder split of Chapter 1 made literal: *anything* that answers those three questions can drive the coder unchanged. So our LLM-style model just has to answer the same three questions. We are not inventing a new coder interface; we are *implementing the one Chapter 26 already defined.*
...
Let us put the LLM line on the same scoreboard we have grown all book, on the kind of text sample `tinyzip` has been chewing through. The point is not that our toy order-2 model rivals Chinchilla; it does not. We use it to slot the *technique* into the lineage and show where the real frontier sits.
...
#scoreboard(caption: "cumulative, on representative English text (≈ bits/byte; lower is better)",
  ...
  [`tinyzip` `llmzip` (toy)], [model-dep.], [n/a], [order-2 demo of the LLM mechanism],
...
Chapter 61 already warned us that a correlation in one direction is not an identity in both. The honest 2025–2026 consensus is careful. The evidence supports *intelligence $arrow.r$ good compression* (a smart model predicts text well). It does *not* establish the converse, *all intelligence $=$ compression*:
...
#misconception[that DeepMind's and Huang et al.'s results prove "intelligence is just compression."][
  They prove something more careful and, honestly, more interesting: *compression is a reliable measurable signature of capability.* A model that compresses text well is, with near-certainty, a capable model, as the $r approx -0.95$ result confirms, and it is real. But "X is a strong signature of Y" is not "X is identical to Y." Compression appears to be a *necessary* hallmark of capable language models, not a *complete definition* of intelligence. The identity *log-loss = code length* is exact and beautiful; the leap from there to *intelligence = compression* is a philosophical bet, not a theorem.]
