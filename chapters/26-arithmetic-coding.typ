...
Let us make it concrete with a three-symbol alphabet (the letters `A`, `B`, `C`) with probabilities $p(A) = 0.5$, $p(B) = 0.3$, $p(C) = 0.2$. (They sum to 1, as probabilities must.) We carve the interval $[0, 1)$ into three slices, one per symbol, each as wide as its probability and stacked in a fixed agreed order:
...
#fig([The unit interval split by symbol probability. `A` owns $[0, 0.5)$, `B` owns $[0.5, 0.8)$, `C` owns $[0.8, 1.0)$. The width of each slice _is_ its probability.], cetz.canvas({
  ...
  content((w*0.25,0.4), box(width: 4.6cm, inset: 2pt, align(center, text(size: 8pt)[*A* (0.5)])))
  content((w*0.65,0.4), box(width: 2.6cm, inset: 2pt, align(center, text(size: 8pt)[*B* (0.3)])))
  content((w*0.9,0.4), box(width: 1.6cm, inset: 2pt, align(center, text(size: 8pt)[*C* (0.2)])))
...
#fig([Three successive zooms encoding `BAC`. Each step picks the incoming symbol's slice of the current interval; the interval shrinks from width 1 to 0.3 to 0.15 to 0.03. Any number in the final $[0.62, 0.65)$ encodes `BAC`.], cetz.canvas({
  ...
  let bar(y, lo, hi, lab) = {
    ...
    content((w+1.8, y+0.3), box(width: 3.2cm, inset: 2pt, align(left, text(size: 8pt)[#lab])))
  }
  bar(3, 0.5, 0.8, [after `B`: $[0.5,0.8)$])
  bar(1.5, 0.5, 0.65, [after `A`: $[0.5,0.65)$])
  bar(0, 0.62, 0.65, [after `C`: $[0.62,0.65)$])
  content((-1.2,3.3), box(width: 1.0cm, inset: 1pt, align(center, text(size: 8pt)[Step 1])))
  content((-1.2,1.8), box(width: 1.0cm, inset: 1pt, align(center, text(size: 8pt)[Step 2])))
  content((-1.2,0.3), box(width: 1.0cm, inset: 1pt, align(center, text(size: 8pt)[Step 3])))
}))
...
*Step 2.* Is $0.41666$ in `A` $[0,0.5)$, `B` $[0.5,0.8)$, or `C` $[0.8,1.0)$? In $[0, 0.5)$, so the second symbol is *`A`*. Remove `A`:
$ "new value" = (0.41666 - C(A)) / p(A) = (0.41666 - 0.0) / 0.5 = 0.83333dots $

*Step 3.* Is $0.83333$ in `A`, `B`, or `C`? In $[0.8, 1.0)$, so the third symbol is *`C`*. We have recovered *`B`, `A`, `C`* (exactly the message).
...
#fig([Renormalization. When the interval sits wholly in the lower half, the top bit is a settled `0`; when wholly in the upper half, a settled `1`. Emit that bit, double the interval (shift left), and continue. This keeps the interval "stretched" across the register so the integers never collapse.], cetz.canvas({
  import cetz.draw: *
  let draw-box(x, lab, col) = {
    rect((x,0),(x+3,2.4), stroke: gray)
    line((x,1.2),(x+3,1.2), stroke: (dash:"dashed", paint: gray))
    content((x+1.5, 2.7), box(width: 2.6cm, inset: 2pt, align(center, text(size: 8pt)[#lab])))
  }
  draw-box(0, [lower half], none)
  rect((0,0),(3,1.2), fill: rgb("#cfe2f3"))
  content((1.5,0.6), box(width: 2.6cm, inset: 2pt, align(center, text(size: 8pt)[interval])))
  content((1.5,1.8), box(width: 2.6cm, inset: 2pt, align(center, text(size: 8pt)[(top bit 0)])))
  draw-box(4.5, [upper half], none)
  rect((4.5,1.2),(7.5,2.4), fill: rgb("#d9ead3"))
  content((6.0,1.8), box(width: 2.6cm, inset: 2pt, align(center, text(size: 8pt)[interval])))
  content((6.0,0.6), box(width: 2.6cm, inset: 2pt, align(center, text(size: 8pt)[(top bit 1)])))
  content((9.75,1.2), box(width: 3.2cm, inset: 2pt, align(center, text(size: 8pt)[emit bit, then double ->])))
}))
...
#algo(
  ...
  aim: "Code a whole message as one interval of [0,1), spending exactly -log2 p bits per symbol with no integer-bit rounding.",
...
#algo(
  ...
  strengths: "approx 9-14% bitrate saving over the simpler CAVLC at the same quality; superb adaptation; multiply-free, hardware-friendly core.",
...
#solution("26.3")[
$-log_2 0.99 approx 0.0145$ bits is the ideal cost. A prefix code assigns each symbol a fixed codeword of a _whole number_ of bits, and the shortest possible codeword is one bit, so a $0.99$-probable symbol must pay at least $1$ bit (a $69times$ overpayment). Arithmetic coding never assigns a per-symbol codeword; it multiplies the running interval width by $0.99$, shrinking it by only $-log_2 0.99 approx 0.0145$ bits' worth. Because the cost is the _total_ $-log_2("width")$ paid once at the end, fractional per-symbol contributions accumulate and are amortised across the whole message, so a near-certain symbol genuinely contributes a tiny fraction of a bit.
]
