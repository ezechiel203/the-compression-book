...
This chapter is about quantization: the art and science of replacing the high-precision floating-point numbers that describe a neural network's weights with far fewer bits, without wrecking what the network knows.
...
#gomaths("Floating-Point Numbers")[
  ...
  - `bfloat16` ("brain float 16"): 1 + 8 + 7 = 16 bits. Same exponent range as float32, but less precise mantissa. This makes it easier for hardware to handle without overflow.
...
Before looking at specific algorithms, let us survey the bit-width options. The community has standardized on a shorthand: W$k$A$m$ means "weights in $k$ bits, activations in $m$ bits."
...
#gomaths("The Hessian: Second-Order Information")[
  In calculus, the *gradient* of a loss function tells you the slope: which direction to step to decrease the loss. The *Hessian* tells you the *curvature*, or how steeply curved the loss is around your current position.
...
#algo(
  ...
  GPTQ made headlines in late 2022 when it showed that GPT-sized models could be quantized to 4 bits in a single GPU-hour, fitting in memory previously thought impossible for consumer hardware. It directly enabled the `llama.cpp` ecosystem that brought local AI to millions of laptops.
]
...
#algo(
  ...
  strengths: "Typically outperforms GPTQ on accuracy at W4; hardware-friendly (no runtime overhead, since scales are absorbed into the weights); fast calibration; widely adopted (AutoAWQ library, supported by vLLM, Hugging Face).",
...
#fig([GPTQ vs. AWQ: perplexity on WikiText-2 for a 7B model at different bit widths. Lower is better. (Schematic; representative of published results.)],
  cetz.canvas({
    ...
    content((3.25,-0.5), text(size: 9pt)[Bit width (weights)])
    content((-0.9,2.25), angle: 90deg, text(size: 9pt)[Perplexity (lower = better)])
    // Ticks X
...
At 4-bit, both methods are excellent: perplexity stays within ~5% of the float16 baseline. At 3-bit, AWQ tends to pull ahead. At 2-bit, both degrade significantly, though newer vector-quantization methods (QuIP\#, QTIP, VPTQ from 2024–2025) extend the usable range.
...
#fig([QAT vs. PTQ trade-off. QAT (blue) always achieves lower perplexity at the same bit width, at the cost of requiring a fine-tuning pass.],
  cetz.canvas({
    ...
    content((2.75,-0.6), text(size: 9pt)[Bit width])
    content((-1,2), angle: 90deg, text(size: 9pt)[Perplexity])
    // Ticks
...
At inference time, the ternary weights are stored as 2-bit integers (since you need to distinguish $-1, 0, +1$). The model does not use floating-point multiplications at all, only additions and subtractions. This is transformative for CPU inference, where multiplication is expensive and addition is cheap.
...
#algo(
  ...
  aim: "Natively train LLMs with ternary weights (−1, 0, +1), 1.58 bits per weight, achieving full-precision accuracy at ≥3B scale while enabling CPU inference and substantial energy savings.",
  ...
  The April 2025 release of BitNet b1.58 2B4T was a watershed: the first open-weight model natively trained at 1.58 bits to demonstrate competitive quality with its float16 peers. Running at 5–7 tokens per second on a laptop CPU, it showed that the ternary frontier is no longer theoretical.
]

=== Why Ternary Weights Eliminate the Outlier Problem

Recall that the outlier problem arises when *activation* values are much larger than average, amplifying weight quantization error. BitNet b1.58 takes a different path: because the weights are always exactly $\{-1, 0, +1\}$, there is no "weight quantization error" to amplify. The weight is already perfect, an exact integer. The only source of error is the activation quantization (INT8), which is much better behaved.

#misconception[
  "BitNet b1.58 is just INT2 quantization, so it should be terrible at 2 bits."
][
...
As of mid-2026, sub-2-bit quantization (W1, W1.5) remains an open problem. The D2Quant line of work (2025–2026) pushes toward 2 bits with vector quantization and hardware co-design. The honest assessment is that below 2 bits per weight, quality degrades significantly for most tasks; the information bottleneck is real. BitNet b1.58 at 1.58 bits works because it is *trained* with that precision, not because 1.58 bits is sufficient to hold a post-trained model.
...
#takeaways((
  ...
  "GPTQ (2022) uses Hessian-based second-order compensation: it rounds one weight, adjusts the remaining weights in the row to compensate, and repeats column by column, achieving accurate W4 PTQ without retraining.",
  ...
  "BitNet b1.58 (2024) trains models natively with ternary weights (−1, 0, +1), achieving 1.58 bits per weight and full-precision accuracy at ≥3B scale, which enables CPU inference at human reading speed.",
...
#solution("63.4")[
  Suppose weight $A = 10.0$ (large weight) multiplied by activation $x_A = 0.001$ (tiny activation). The contribution to output is $10.0 times 0.001 = 0.010$. If we quantize $A$ to $9.0$ (rounding error = 1.0), the output error is $1.0 times 0.001 = 0.001$, which is tiny. Now weight $B = 0.01$ (small weight) multiplied by activation $x_B = 200$ (outlier activation). Contribution = $0.01 times 200 = 2.0$. If we quantize $B$ to $0$ (rounding error = 0.01), the output error is $0.01 times 200 = 2.0$, an enormous error. AWQ's insight is to look at *output error = weight\_error × activation*, not just weight magnitude or activation magnitude alone.
]
...
#bridge[
  We now know how to compress the *weights* of a neural network (the static parameters that define what the model knows). But quantization is only one lever. Chapter 64 covers the remaining ways to shrink a static network: pruning (deleting entire weights or neurons), knowledge distillation (training a small model to mimic a large one), and low-rank factorization (replacing big matrices with thin ones). Then Chapter 65 turns to a different problem entirely. A neural network is not just its weights: when you run it on a long conversation, it also builds up a large collection of temporary stored computations called the *KV-cache*, which grows linearly with how much you have said. Chapter 65 returns to that runtime problem, showing how to compress the KV-cache itself, the memory of the conversation in progress, where new algorithmic ideas like DeepSeek's Multi-head Latent Attention are reshaping what it costs to hold a long thought.
]
