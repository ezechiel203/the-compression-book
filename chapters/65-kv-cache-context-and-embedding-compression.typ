...
A naive policy --- drop the oldest tokens --- sounds reasonable, but it fails catastrophically. Older tokens often carry *structural* information: the opening sentence, the system prompt, the question being answered. Dropping them first is like throwing away the question before you write the answer.

=== The Heavy-Hitter Oracle (H2O)

The key observation behind H2O (Zhang et al., NeurIPS 2023) is empirical but robust: *across all heads and all layers, a small fraction of token positions consistently attract most of the total attention score*. These tokens are called *Heavy Hitters* (H2), and they are the tokens that are actually load-bearing for the model's reasoning.
...
#algo(
  ...
  MLA also introduced a subtle positional-encoding trick: a small separate "decoupled" key vector carries position information while the main latent stays position-free. This allows the expansion $bold(W)_K bold(c)$ to be algebraically "absorbed" into the query projection at inference time, eliminating one of the two matrix multiplications from the hot path --- a compiler-style optimization baked into the mathematical structure of the mechanism.
]
...
#fig(
  ...
  cetz.canvas({
    ...
    content((4.0, 5.45), text(size: 8pt)[BFloat16 MHA baseline --- 100%])

    // Eviction
    rect((0, 4.0), (4.0, 4.5), fill: rgb("#0b5394").lighten(60%), stroke: none)
    content((6.5, 4.25), text(size: 7.5pt)[Eviction (H2O): ~50%])

    // 4-bit quant
    rect((0, 2.8), (2.0, 3.3), fill: rgb("#0f766e").lighten(50%), stroke: none)
    content((5.0, 3.05), text(size: 7.5pt)[4-bit quantization: ~25%])

    // MLA
    rect((0, 1.6), (0.56, 2.1), fill: rgb("#783f04").lighten(50%), stroke: none)
    content((4.0, 1.85), text(size: 7.5pt)[MLA (DeepSeek-V3): ~7%])
...
#fig(
  ...
  cetz.canvas({
    ...
    content((3.75, 3.95), text(size: 8pt)[Original 768-D float32 vector (3,072 bytes)])

    // Arrow
    line((3.75, 3.6), (3.75, 3.1), mark: (end: ">"), stroke: 0.7pt)
    content((5.5, 3.35), text(size: 7.5pt)[Split into M sub-vectors])
    ...
    for i in range(4) {
      ...
      content((x0 + 0.8, 2.43), text(size: 7.5pt)[Sub-#str(i+1) \ (8-D)])
    }
    content((7.6, 2.43), text(size: 8.5pt)[...])

    // Arrow
    line((3.75, 2.0), (3.75, 1.5), mark: (end: ">"), stroke: 0.7pt)
    content((6.0, 1.75), text(size: 7.5pt)[k-means: nearest centroid])

    // Code byte boxes
    for i in range(4) {
      let x0 = i * 1.85
      rect((x0, 0.4), (x0 + 1.6, 1.2), fill: rgb("#fff3e0"), stroke: 0.5pt + rgb("#ffb74d"))
      content((x0 + 0.8, 0.8), text(size: 7.5pt)[byte #str(i+1)])
    }
    content((7.6, 0.8), text(size: 8.5pt)[...])
    content((3.75, 0.1), text(size: 8pt, style: "italic")[96 bytes total (from 3,072): 32x compression])
  })
)
...
- #link("https://arxiv.org/pdf/2407.18003")[Liang et al. (2024). *A Survey on Efficient Inference for Large Language Models.* arXiv:2407.18003.] --- in `papers/liang-2024-kvcache-survey.pdf`. Comprehensive survey covering eviction, quantization, speculative decoding, and architectural approaches.
