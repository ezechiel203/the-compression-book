...
Let us meet the three operations that everything else is built from. They are named after three English words you already use every day: *not*, *and*, *or*. Each one means almost exactly what the English word means.
...
How do we know what $F$ does? We build its truth table by trying _every_ combination of inputs. Three variables means $2 times 2 times 2 = 8$ rows. For each row we compute the inner pieces first (just like doing the brackets first in ordinary arithmetic), then combine them.

#align(center, table(columns: (auto, auto, auto, auto, auto, 1fr), inset: 4pt, align: center,
  fill: (_, row) => if row == 0 { c-accent.lighten(85%) },
...
#gomaths("Counting the rows: why 2 to the n")[
  ...
  rows. This is the same "powers of two" pattern from Chapter 4, and it is the first whiff of an idea that will haunt the rest of this book: things built from independent binary choices grow _exponentially_. Ten yes/no questions already make $2^10 = 1024$ possible answers; twenty make over a million. We count this way again when we count messages, codes, and search-space sizes. (The general rules of counting come in Chapter 8.)
]
...
Each gate has a standard drawn symbol, and an engineer reads a circuit diagram much as a musician reads a score. Here are the three fundamental gates that match our three operations.
...
Check any row and you will find the formula agrees with the table. At $A B C = 110$, for instance, only the middle term is true, so $F = 1$, exactly as required. (We could now _simplify_ this with the laws of the next section: the last two terms share $A and B$ and the $overline(C) or C$ collapses to $1$, giving $F = (overline(A) and overline(B) and C) or (A and B)$. Same function, fewer gates, which is the whole game of the next section in miniature.)

#keyidea[
  *Every Boolean function can be written with only NOT, AND, OR.* The sum-of-products recipe proves it constructively: read off the $1$-rows, AND-build a term for each, OR them together. Keep this fact in mind: when we prove later in this chapter that a single gate type (NAND) can build _everything_, the only thing we will need is that NAND can imitate NOT, AND, and OR, because sum-of-products has already guaranteed those three suffice for any function at all. It is also the reason a designer never has to be clever to get _correct_ hardware. Correctness is mechanical; cleverness is only for making it _small_.
]
...
Look at this table for a moment and a beautiful pattern jumps out: every law comes in a _pair_, and you can turn one into the other by swapping AND with OR _and_ swapping $0$ with $1$ throughout. This is the *principle of duality*, and it means you only ever have to remember (and prove) _half_ of Boolean algebra; the other half comes for free by swapping. It is one of the more elegant facts in mathematics, and it falls straight out of the symmetry we noticed earlier.
...
Let us prove the absorption law $A or (A and B) = A$. The result is genuinely surprising, because $B$ simply _vanishes_. I also want to show two proof styles you will reuse often: the brute-force truth table, and the slicker algebraic derivation.
...
This pairing of styles is worth keeping in mind. When you just need to be _sure_, a truth table is the safest tool available: it cannot lie, because it checks everything. When you need to _simplify_ a big expression down to something a circuit can implement cheaply, you reach for the algebraic laws. Real chip-design and circuit-verification software does both, using the algebra to simplify and the exhaustive check (in clever, compressed forms) to verify.

== De Morgan's laws: the most useful identity you'll ever learn

Of all the laws in the table, the pair named after Augustus De Morgan earns its own section. A working engineer reaches for it more than any other. In words:
...
#keyidea[
  *De Morgan's laws.* "NOT (A AND B)" equals "(NOT A) OR (NOT B)", and "NOT (A OR B)" equals "(NOT A) AND (NOT B)". In symbols:
  ...
  In plain English: to push a NOT through a bracket, flip every AND to OR (and every OR to AND), and negate each piece. The bar "breaks", and AND/OR swap underneath it.
]
...
#proof[
  ...
  #align(center, table(columns: (auto, auto, auto, auto, auto, auto, auto), inset: 4pt, align: center,
    fill: (_, row) => if row == 0 { c-accent.lighten(85%) },
  ...
  The fourth column ($overline(A and B)$) and the seventh column ($overline(A) or overline(B)$) are identical: $1, 1, 1, 0$. The two expressions agree on every input, so they are equal. The second law follows by duality (or by an identical four-row table). #h(1fr)
]

Why does this matter so much in practice? Two reasons recur throughout compression and computing.
...
Second, *De Morgan is how you negate a complicated condition correctly*, and programmers get this wrong constantly. The opposite of "the file is open AND not empty" is _not_ "the file is closed AND empty"; it is "the file is closed OR empty". Negating a compound condition flips the connective. Get this wrong in a decompressor's bounds-check and you have a security bug; get it right and you sleep at night.
...
#gopython("Booleans, and/or/not, and the operators & | ^ ~")[
  ...
  Do not confuse the word-operators (`and`, `or`, `not`, which work on whole True/False values) with the symbol-operators (`&`, `|`, `^`, `~`, which work bit-by-bit on integers). Chapter 4 introduced `0b...` binary literals and `bin()`; here they let us _see_ Boolean algebra running across all bit positions at once.
]

#gopython("Comprehensions, generator expressions, and enumerate")[
  Chapter 4 showed the `for` loop, which walks through a collection one item at a time. Python has a compact shorthand for "loop over a collection and collect a value from each item": a *comprehension*. Instead of
  ...
  (`range(4)` is the numbers `0, 1, 2, 3`, from Chapter 4.) Drop the square brackets and you get a *generator expression*, which produces the same values one at a time without building a whole list in memory. That is handy for feeding a function like `bytes(...)`, `sum(...)`, or `all(...)`. The function `all(...)` returns `True` only if _every_ value it receives is true (a software echo of the AND we just studied, AND of a whole stream of conditions):
...
#tryit[
  ...
  The second `assert` exhaustively checks De Morgan's first law across all $256 times 256 = 65{,}536$ byte pairs. It is the same brute-force proof we did by hand, now done by the machine in a blink. This pattern, _encode a law as a test and verify it exhaustively_, is how real codec authors guard against subtle bit-twiddling bugs.
]

== One gate to rule them all: NAND and NOR

We have been treating NOT, AND, OR as three separate primitives, but we can also _combine_ them. Two especially important combinations are AND-then-NOT and OR-then-NOT:
...
#proof[
  ...
  *OR* from NAND: by De Morgan, $A or B = overline(overline(A) and overline(B)) = overline(A) "NAND" overline(B)$. So invert each input with a NAND, then NAND the two results:
  ...
  Having reconstructed NOT, AND, and OR purely from NAND gates, and knowing those three suffice for every Boolean function, we conclude NAND alone is functionally complete. The same argument applied to the dual law $A and B = overline(overline(A) or overline(B))$ proves NOR is complete too. #h(1fr)
]
...
In terms of our basic operations, $A xor B = (A and overline(B)) or (overline(A) and B)$: "($A$ but not $B$) or (not $A$ but $B$)", which is precisely "exactly one of them". The reason engineers reach for XOR so often is a cluster of properties that fall straight out of its truth table:
...
That last property is the key one. Because XOR-ing twice by the same value returns the original, XOR is the simplest possible _reversible_ transformation. Reversibility is the non-negotiable requirement of all lossless compression: if a compressor scrambles your data with some operation, it had better be able to unscramble it _exactly_. XOR offers a whole family of such perfectly reversible scrambles for free.
...
#proof[
  ...
  The steps are: regroup by associativity, apply the self-inverse law to collapse $B xor B$ to $0$, then remove the $0$ by the identity law. The same chain shows $C xor A = B$. #h(1fr)
]

Where does this show up in real compression? In more places than you might expect:

- *PNG image filters.* Before PNG hands a row of pixels to its DEFLATE compressor (Chapter 30), it often replaces each pixel with the _difference_ from a neighbouring pixel. On the encode side that difference is computed; on the decode side it is added back. The "transform now, exactly undo later" structure that XOR embodies is the soul of every reversible filter, and PNG's "Paeth" and "Sub" filters are built on exactly this idea. Smooth images become long runs of small numbers, which compress beautifully.
- *Delta and XOR encoding* of time series and columnar data (Chapters 67 and 68). When successive values barely change, storing $x_i xor x_(i-1)$ turns a stream of large similar numbers into a stream of mostly-zero bits, and zeros are cheap to compress.
...
- *The one-time pad*, the only provably unbreakable cipher, is nothing but message XOR key. Encryption and compression are cousins; both reshape redundancy, and both rely on reversible bit operations.
...
#align(center, table(columns: (auto, auto, 1fr), inset: 7pt, align: (center, center, center),
  fill: (_, row) => if row == 0 { c-accent.lighten(85%) },
...
Let us end the body with a worked example that uses several laws at once. Simplification is the skill that turns Boolean algebra from a curiosity into a tool. Real circuit designers and compiler writers do exactly this: take a sprawling expression and grind it down to the smallest equivalent one, because fewer operations mean fewer gates, less power, and faster code.
...
A three-term, five-operation expression collapses into a single OR gate. The original would have needed several ANDs, a NOT, and two ORs; the simplified version needs _one gate_. Across a chip with billions of such expressions, or across a decompression inner loop run trillions of times, that difference matters enormously. And we reached it with nothing but the laws in our table, each step provable by a four-row truth check if we ever doubted it.

#note[
  Doing this by hand is fine for three or four variables. Beyond that, engineers reach for systematic tools: *Karnaugh maps* for a handful of variables, and the *Quine-McCluskey* algorithm or modern *binary decision diagrams* (BDDs) for many. We will not need those tools in this book, but it is worth knowing they exist and that they are pure, mechanical Boolean algebra, the same laws you just used applied by a machine. Every one of them rests on the handful of identities in our little table.
]

#takeaways((
  [Boolean algebra is ordinary algebra restricted to two values, $0$ (false/off) and $1$ (true/on), the very bits of Chapter 4, now able to _compute_ rather than merely store.],
  ...
  [The laws of Boolean algebra (identity, null, complement, commutative, associative, distributive, absorption, De Morgan) come in dual pairs: swap AND with OR and 0 with 1, and you get the partner law for free. Any law is provable by exhaustive truth table.],
  [De Morgan's laws ($overline(A and B) = overline(A) or overline(B)$) are the everyday workhorse: they push a NOT through a bracket by flipping AND with OR, and they let you correctly negate compound conditions in code.],
  ...
  [XOR (different → 1) is reversible: XOR-ing twice by the same value restores the original. That makes it central to reversible filters (PNG), delta/XOR encoding, parity error-detection, and the one-time pad. The "transform then perfectly undo" property is the requirement all lossless compression must satisfy.],
))
...
#solution("5.1")[
  ...
  $F$ is true in three of the four rows. It is false in exactly one case: when $A = 1$ and $B = 0$. In other words, "if $A$ then $B$" is broken only when $A$ holds but $B$ fails to follow.
]
...
#solution("5.8")[
  ...
  The sum column $0,1,1,0$ is exactly XOR: $S = A xor B$. The carry column $0,0,0,1$ is exactly AND: $C = A and B$. So a half-adder is one XOR gate (sum) plus one AND gate (carry). Chain half-adders into full-adders and you can add any two binary numbers, exactly as Shannon foresaw in 1937.
]

#exercise("5.9", 3)[
  Simplify $F = (A or B) and (A or overline(B)) and (overline(A) or B)$ to as few operations as possible, naming each law. (This is the dual of the worked AND/OR example in the chapter.)
]
#solution("5.9")[
...
#solution("5.10")[
  *(i)* $g(x) = x xor 42$ is its own inverse: $g(g(x)) = (x xor 42) xor 42 = x xor (42 xor 42) = x xor 0 = x$ (self-inverse and identity laws). Applying $g$ twice recovers $x$; the function is invertible, and its inverse is $g$ itself.
...
- George Boole (1854), _An Investigation of the Laws of Thought_. The founding text, surprisingly readable and freely available. #link("https://www.gutenberg.org/ebooks/15114")[Project Gutenberg].
