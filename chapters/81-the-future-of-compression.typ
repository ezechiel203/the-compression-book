...
The industry analyst firm IDC has tracked the *global datasphere* — all the data
created, captured, and replicated in a year — for over a decade. In 2010 it was a
few zettabytes. A *zettabyte* is a 1 followed by 21 zeros of bytes; written out,
...
#worked[
  *How big is a zettabyte, really?* Suppose you stored 180#sym.space.thin ZB on the best 2026
  ...
  10^12) = 6 times 10^9$ — *six billion* drives. Stacked flat, each drive about
  25#sym.space.thin mm thick, the pile would be 150,000#sym.space.thin km tall — a third of the way to the
  Moon. Now remember the pile must grow tall enough to *double* in three years.
...
#theorem("No free lunch in lossless compression")[
  ...
  *shorter*, then other inputs must get *longer*. No single lossless scheme can map
...
#checkpoint[
  ...
  to zero bits, from which nothing could be recovered — impossible by the counting
  proof above; some inputs *must* grow.
]
...
cetz.canvas({
  ...
  content((8.0,4.95))[#text(size:8pt, fill: rgb("#9a2617"))[data created]]
  // hardware curve: flattening
  line((0,0.4),(2,1.1),(4,1.7),(6,2.1),(8,2.3), stroke: 1.4pt + rgb("#0b5394"))
  content((7.4,2.05))[#text(size:8pt, fill: rgb("#0b5394"))[cheap-byte capacity]]
  // entropy floor
...
the same subject. To see why, we only have to take the thesis of Chapter 23
...
stand, and the whole of classical compression lives inside it. The next decade is
the field's first serious, well-funded attempt to step *across* it.
...
flip this: a dedicated hardware engine *inside the SSD* compresses and decompresses
...
#keyidea[
  *Compression is migrating down the stack and out of sight.* It is moving off the
  CPU and into dedicated silicon — in network cards, in SSD controllers, in
  database storage engines — so it becomes free in the sense that matters: it no
...
#table(
  ...
  inset: 7pt, align: (left + top, left + top),
  fill: (_, row) => if row == 0 { c-accent.lighten(85%) } else { none },
  table.header([*Prediction*], [*Why it follows / why it might be wrong*]),
  [*1. Learned codecs win the high-value edges first, not the middle.*
   By the early 2030s, neural compression is standard for *precious, scarce,
   long-lived* data — scientific archives, medical and satellite imagery, genomic
   reference sets — while everyday bytes stay on fast classical coders.],
  [*Follows:* the ratio–energy curve of this chapter; neural wins ratio, loses
   speed by orders of magnitude (Chapter 62), so it colonizes data where ratio is
   worth the compute. *Might fail if:* dedicated neural-codec silicon makes
   inference so cheap that learned coders undercut zstd even on hot data — the same
   way GPUs made neural nets ubiquitous.],

  [*2. Compression and the AI stack become inseparable.*
   "Compressing the model" (quantization, low-rank, KV-cache) and "the model as
   compressor" stay two faces of one coin; sub-2-bit weight coding and architectural
   cache compression like MLA become routine.],
  [*Follows:* the merger argument of §2 and the deployed wins of Chapters 63–65.
   *Might fail if:* a non-neural AI paradigm displaces today's giant transformers,
   loosening the link — though the *compression = prediction* identity itself is a
   theorem (Chapter 23), not a fashion, so the deep tie survives any single
   architecture.],

  [*3. Semantic/goal-oriented coding ships in machines talking to machines — and
   stays out of safety-critical human loops longest.*
   By ~2030 (6G era) it is real for telemetry, sensor fusion, and constrained
   links; general "meaning-level" human communication remains research.],
  [*Follows:* the 6G-GOALS-style programmes and the information-bottleneck framing
   of §3. *Might fail if:* the trust/robustness problem (silent catastrophic
   failure) proves harder than expected and the field retreats to classical lossy
   coding with a semantic *garnish* rather than a semantic *core*.],

  [*4. Compression disappears into hardware and becomes invisible.*
   Transparent in-storage and in-network compression makes "is this compressed?" a
   question application programmers stop asking, the way they stopped asking "is
   this HTTP response gzipped?"],
  [*Follows:* the computational-storage trend of §4 and DEFLATE's own history
   (Chapter 30). *Might fail if:* fragmentation of formats and the need for
   application-specific models keeps compression visible at the application layer
   for longer than the storage layer would like.],

  [*5. The classical champions do not die.*
   zstd, DEFLATE, JPEG, AV1/AV2, Opus, FLAC remain the deployed backbone through
   2040, gaining incremental tuning, not replacement.],
  [*Follows:* the no-free-lunch theorem plus the brutal economics of installed base
   and interoperability (Chapters 77–78). *Might fail if:* a royalty-free learned
   codec with hardware decode reaches the speed *and* ratio bar simultaneously —
   possible, but it has to beat fifty years of relentlessly optimized engineering.],

  [*6. The fundamental limits never move.*
   Shannon's entropy floor and Kolmogorov's uncomputability bound everything, in
   2040 exactly as in 1948.],
  [*Follows:* they are *proved* (Chapters 18–19, 22). *Might fail if:* mathematics
   itself is overturned — which is to say, it will not. This is the one prediction
...
#history[
  ...
  *under*-predicting the staying power of the boring classics. In the 1990s wavelets
  ...
  safe long-term bet in compression has almost always been: *the simple, fast,
