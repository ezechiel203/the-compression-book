...
#proof[
  ...
  For uniqueness, suppose $n$ had two different digit strings. Look at the rightmost place where they differ. Everything to the right of that place contributes the same amount in both (they agree there), and everything from that place leftward is a multiple of $b^i$ for that place's power $b^i$. But the digit in that place is exactly the remainder of $(n - "lower places") div b^i$ modulo $b$, and a remainder is unique — a number divided by $b$ has only one remainder. So the two digits must be equal after all, contradicting the assumption that they differed. Hence the expansion is unique. #h(1fr)
]

That "repeatedly divide by the base, collect the remainders" recipe is not just a proof device — it is the actual algorithm we will use all chapter to convert numbers into any base. Tuck it away; we are about to run it many times.
...
Divide and record the remainder each time. The remainder is always $0$ or $1$ — exactly a bit:
...
This single theorem is why a byte holds $256$ values, why an "8-bit" colour channel has $256$ shades, and — looking far ahead — why no compressor can shrink every file (there simply aren't enough short bit-strings to give them all; we will prove that *counting bound* rigorously in Chapter 8). For now, just internalise the table: each extra bit *doubles* your reach.

#table(columns: (auto, auto, auto), inset: 6pt, align: (right, right, left),
  fill: (_, row) => if row == 0 { c-accent.lighten(85%) } else { none },
...
Now the names. Four bits are a *nibble* (yes, really — half a byte, and the pun is intentional). One nibble holds $2^4 = 16$ values, which is exactly why hexadecimal, coming up next, fits a nibble like a glove. Eight bits are a *byte*, the workhorse unit of computing: $2^8 = 256$ values, enough for every letter, digit, and punctuation mark of early computing with room to spare. The byte is the smallest chunk most computers will hand you individually — you address memory and measure files in bytes, not bits. A group the processor likes to chew in one bite is a *word*; its width depends on the machine (modern laptops and phones use 64-bit words), so "word" is a relative term while "byte" is nailed down at eight bits.
...
Group binary digits into *threes* and you have invented *octal*, base 8. Why threes? Because three bits make exactly $2^3 = 8$ patterns, which is precisely the eight digits of base 8: $0, 1, 2, 3, 4, 5, 6, 7$. Each group of three bits becomes one octal digit, and back again, with no arithmetic at all — just a lookup.
...
#keyidea[
  *Hex is just binary wearing a compact suit.* Because $16 = 2^4$, each hex digit stands for exactly four bits — no arithmetic needed to convert, only a 16-row lookup. Two hex digits = one byte = the unit files are measured in. This is why hex, not octal and not decimal, is the lingua franca of everyone who works at the level of raw bytes, which is to say everyone who builds a compressor.
]
...
Four bases, one number, and every answer agrees — because, by our uniqueness theorem, they *must*. That is the quiet power of place value: change the base and the *spelling* changes, but the number underneath is rock-solid. Notice especially how binary→hex and binary→octal needed *no division at all*, only regrouping. When you see a programmer rattle off "that's `0x3E8`" while staring at a binary dump, this nibble-regrouping is the magic trick — there isn't any arithmetic, just slicing the bits into fours.

#table(columns: (auto, auto, auto, auto, auto), inset: 6pt, align: (center, center, center, center, center),
  fill: (_, row) => if row == 0 { c-accent.lighten(85%) } else { none },
...
#table(columns: (auto, auto, auto), inset: 6pt, align: (right, right, center),
  ...
  [$0.8 times 2$], [$1.6$], [`1`],
  [$0.6 times 2$], [$1.2$], [`1`],
  [$0.2 times 2$], [$0.4$], [`0` (and we are back to $0.2$…)],
)
...
#tryit[
  ...
  Both numbers round-trip and match every conversion we did by hand. With these two helpers you can show any value in any base — exactly the eyes we will want when we start emitting and inspecting real compressed bytes from Chapter 15 onward. The type hints (`n: int`, `-> str`) and the `for` loop get their full from-scratch treatment in Chapters 15–16; read them here as plain English — "n is a whole number; this returns text."
]
...
The winning scheme is *two's complement*, and the idea behind it is a clock. Picture a 12-hour clock face. If it is 3 o'clock and you wind the hands *back* 4 hours, you land on 11. But you'd reach the very same 11 by winding *forward* 8 hours, because $3 + 8 = 11$ and going forward 8 is the same as going back 4 on a 12-hour wheel ($8 = 12 - 4$). On a clock, "minus 4" and "plus 8" are the same motion. Two's complement plays exactly this trick with bits: on an $n$-bit wheel of $2^n$ positions, it represents $-x$ by the forward step $2^n - x$. Subtraction becomes addition that's allowed to wrap around — and wrapping around is *free* in hardware, because an 8-bit adder naturally throws away any 9th bit that overflows.
...
Why is this scheme so beloved? Three reasons, and they are exactly the flaws of sign-and-magnitude turned into virtues. First, *there is only one zero*: `00000000` is zero, and nothing else is, so no wasted pattern and no "is it plus-zero or minus-zero?" confusion. Second, *the whole range is used*: all $256$ bytes mean a distinct number, from $-128$ to $127$, with nothing wasted. Third — and this is the killer feature — *addition just works*. To compute $-5 + 7$, add the bytes `11111011` ($-5$) and `00000111` ($+7$) column by column, just like grade-school addition but in binary, carrying a $1$ whenever a column sums to two:
...
][Start from $+1 = $ `00000001`. Flip every bit: `11111110`. Add one: `11111111`. So $-1$ is the all-ones byte. Decode to check: top bit gives $-128$, the rest give $64+32+16+8+4+2+1 = 127$, and $-128 + 127 = -1$. ✓ (It is satisfying that $-1$ is "all switches on" — it sits one step counter-clockwise from $0$ on the wheel.)]
...
#takeaways((
  ...
  [*Convert with two recipes:* "weigh and sum" to reach decimal, "divide and collect remainders" to leave it — and just regroup bits (no arithmetic) between binary, octal, and hex.],
  ...
  [*Two's complement* stores negatives with no sign symbol: the top bit's place is worth $-2^(n-1)$. It gives one zero, the full range, and free addition — and "flip the bits, add one" negates (proved). It is why a byte can read as $251$ or $-5$ depending only on the agreement.],
))
...
#solution("4.2")[
  Split into byte pairs: `1E`, `90`, `FF`. Red $= 1 times 16 + 14 = 30$. Green $= 9 times 16 + 0 = 144$. Blue $= 15 times 16 + 15 = 255$. Blue is strongest (and maxed out at $255$), with green moderate and red faint — which is why the colour reads as a vivid blue.
]
...
#exercise("4.7", 3)[
  Prove that in $n$-bit two's complement the single value $-2^(n-1)$ (the most negative number) has no positive counterpart inside the same $n$ bits — that is, "flip and add one" applied to it gives back *itself*. What does this tell you about the symmetry of the representable range?
]
#solution("4.7")[
