...
#history[
  Jorma Rissanen also invented MDL -- the Minimum Description Length principle -- in 1978 ("Modeling by Shortest Data Description," _Automatica_). MDL says: the best model is the one that gives the shortest description of the data _plus_ the description of the model itself. It is a formalization of Occam's razor in information-theoretic terms, and it sits beneath how we think about regularization, model selection, and the deep connection between compression and machine learning. Rissanen worked at IBM Research for most of his career and died in 2020.
]
...
#solution("76.3")[
  (1) _Institutional prestige and visibility_: Duda was a PhD student at a non-elite institution, and a mathematics preprint server reaches a different audience than the systems-programming and compression communities. A paper from Google or Bell Labs would have been noticed immediately; a preprint from Krakow took longer to propagate. (2) _Translation to practice_: The theoretical insight (ANS is fast and near-optimal) needed an engineer -- Fabian Giesen -- to implement it clearly, test it, and publish accessible explanations before the broader community could evaluate it. The lag between theory and practice is structural. (3) _Incumbent inertia_: Huffman coding and arithmetic coding were both well-understood, well-implemented, and embedded in existing standards. There is a real cost to switching, even if the new technique is superior. (4) _Patent risk_: After the arithmetic-coding patent experience, compression engineers were cautious about adopting new entropy-coding techniques without confidence about the patent situation.
]
