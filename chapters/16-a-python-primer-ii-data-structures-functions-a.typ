...
Reach for a tuple whenever a clutch of values belongs *together as one thing* and should not drift apart or mutate underfoot: a coordinate `(x, y)`, an RGB colour `(255, 128, 0)`, a date `(2025, 10, 7)`, or — the case dearest to us — an LZ77 match written as `(distance, length)`. The fixedness documents your intent ("these three numbers are one record, not a growable list") and lets Python store the tuple more compactly and, crucially, lets it serve as a *dictionary key*, which a list can never do — a privilege we will cash in shortly.
...
#fig([The four core containers at a glance: ordered vs unordered, changeable vs fixed, and whether they carry values per key.], cetz.canvas({
  ...
  let cell(x, y, title, sub, col) = {
    ...
    content((x+1.8, y+1.05), text(size: 9.5pt, weight: "bold")[#title])
    content((x+1.8, y+0.45), text(size: 7.6pt)[#sub])
  }
...
#scoreboard(caption: "tinyzip toolkit after Chapter 16 (sample = \"banana bandana\", 14 chars)",
  ...
  [Frequency model (`utils.histogram`, Step 2)], [—], [—], [One typed call; tested vs `Counter`],
  [Streaming RLE demo + round-trip test], [—], [—], [First lossless encode/decode pair (illustrative)],
)
