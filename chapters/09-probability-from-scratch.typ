...
These three little puzzles share one secret. Each is really a question about _counting the ways the world could be_ and asking what fraction of those ways match what you care about. That is the whole of probability. By the end of this chapter you will solve all three from first principles, with nothing more than fractions and careful bookkeeping. In the final section you will also see why a coin that is _predictable_ is a coin that can be _compressed_, and a coin that is perfectly unpredictable cannot be compressed at all.
...
#objectives((
  ...
  [Connect a probability distribution to _surprise_ and _predictability_. This is the bridge that carries us into information theory and the whole rest of the book.],
))
...
The symbol $in$ means *"is an element of."* So $"heads" in S$ reads "heads is in the set $S$." The notation $abs(S)$ (vertical bars around a set) means *the size of the set*, i.e. how many elements it has. For our coin, $abs(S) = 2$. The empty set, written $emptyset$, is the set with nothing in it, so $abs(emptyset) = 0$.
...
These three lines are deceptively powerful. Watch how much falls out of them. Throughout, we write $A^c$ (read "$A$ complement") for the event "$A$ does _not_ happen," meaning all the outcomes _not_ in $A$.
...
Two events are *mutually exclusive* (or *disjoint*) when $A inter B = emptyset$, i.e. no shared outcomes. In probability-speak, additivity applies exactly when events are disjoint.
]
...
A coin flipped once is dull. The interesting questions, the ones compression cares about, involve _sequences_: many flips, many die rolls, a whole string of characters. To handle sequences we need to know how to combine the probabilities of separate events. The key word is *independence*.

Two events are *independent* when knowing that one happened tells you nothing about whether the other happened. A coin has no memory: the second flip doesn't care how the first landed. Drawing two cards _without_ putting the first back, though, is _not_ independent, because removing a card changes what's left. The distinction matters enormously.
...
cetz.canvas({
  ...
  content((3.5,-0.6), box(width: 6.6cm, inset: 1pt, align(center, text(size: 8pt)[Given $B$: only the blue world remains; how much of it is also $A$?])))
}))
...
Knowing it was even _raised_ the probability of a 2 from $1\/6$ to $1\/3$. Information changed the odds. That is the whole point: *conditioning on what you have already seen sharpens your prediction of what comes next*. A sharper prediction, we will prove in Chapters 18 and 26, means fewer bits.
...
Conditional probability has a direction to it. $P("test positive" | "sick")$ (how often a sick person tests positive) is a property of the test, easy to measure in a lab. But what you actually want to know when you get a result is the _reverse_: $P("sick" | "test positive")$, i.e. given that I tested positive, how likely am I to be sick? These two are _not_ the same number, and confusing them is one of the most consequential mistakes a human being can make. The tool that converts one into the other is *Bayes' rule*, just two lines of algebra with life-or-death consequences.
...
cetz.canvas({
  ...
  content((4.5,1.55), box(width: 8.2cm, inset: 1pt, align(center, text(size: 7.5pt, fill: rgb("#9a2617"))[~999 of them test positive (false alarms)])))
  // sick group bar (tiny)
  rect((0,0),(0.5,1), fill: rgb("#0b5394").lighten(40%), stroke: 0.6pt + gray)
  content((2.4,0.5), box(width: 4.2cm, inset: 1pt, align(left, text(size: 8pt)[100 sick → ~99 test positive (true)])))
  content((4.5,3.4), box(width: 8.2cm, inset: 1pt, align(center, text(size: 8.5pt, weight: "bold")[Out of 100,000 people])))
}))
...
One more habit worth seeing now: a sanity check that a dictionary really _is_ a distribution. It encodes Kolmogorov's first two axioms directly, requiring every probability $>= 0$ and their sum to equal 1.
...
#keyidea[
The *surprise* (or *self-information*) of an outcome with probability $p$ is $log_2(1\/p) = -log_2 p$ bits. A certain event ($p = 1$) carries $log_2 1 = 0$ bits of surprise (no news). A one-in-a-million event ($p = 10^(-6)$) carries about $20$ bits. Rare things are surprising; surprising things cost bits. The _average_ surprise of a source, $sum_x p(x) log_2(1\/p(x))$, is its *entropy*: the exact number of bits per symbol that the best possible compressor must spend. This is Shannon's 1948 masterstroke, and we now have every ingredient to understand it.
]
...
The other side is white only *one third* of the time, so the hustler wins two times out of three at even money. _Decline the bet._ The flaw in the naive argument is that it treated "RR" and "RW" as equally likely _after_ seeing red, but they are not: the RR card has _two_ red sides that could have shown, while the RW card has only _one_, so seeing red is twice as much evidence for RR as for RW. Counting _sides_, not _cards_, is the honest sample space, and Bayes' rule did that bookkeeping automatically. This is the same structure as the medical test: the evidence weighs the hypotheses by how _readily_ each would have produced what you saw.
...
#solution("9.4")[
(a) Each die has 6 faces and they are independent, so $6 times 6 = 36$ equally likely ordered outcomes. (b) Sum 7 occurs for $(1,6),(2,5),(3,4),(4,3),(5,2),(6,1)$ (6 ways), so $P = 6\/36 = 1\/6$. Sum 2 occurs only for $(1,1)$, so $P = 1\/36$. (c) $P("sum 7") = 1\/6$ and $P("first die 1") = 1\/6$. The intersection is just $(1,6)$, so $P("both") = 1\/36 = (1\/6)(1\/6)$. Since the product equals the joint probability, the two events _are_ independent, a small surprise that holds only for the sum 7.
]
