  ...
  cetz.canvas({
    ...
    for (i, (name, size, desc)) in files.enumerate() {
      ...
      content((2.8, y - row-h * 0.5), anchor: "west",
        box(width: 8.6cm, text(size: 7pt)[#desc]))
    }
    content((col-w * 1.25, 0.35), text(weight: "bold", size: 8pt)[Silesia Corpus: 12 files, ~211.9 MB])
  })
)
...
#pitfall[
  ...
  eight threads on a 64-core machine, or it may just mean LZ4 on modern hardware, where
  3 GB/s is normal.
]
...
Intelligence," is that intelligence is precisely the ability to compress. A system that
cannot summarize or predict natural language has not understood it.
...
#algo(
  ...
  percentage point of improvement becoming harder to achieve. Compressing enwik9 from its
  1 GB original to ~110 MB represents a factor of about 9× compression, compared with
  gzip's factor of ~3.6× - a remarkable gap.
]
...
#checkpoint[
  ...
  minimum. DEFLATE does not model long-range semantic patterns; it only
...
#fig(
  ...
  cetz.canvas({
    ...
    content((8.3, 0), anchor: "west", text(size: 8pt)[Speed →])
    content((0, 6.3), anchor: "south", text(size: 8pt)[← Ratio])
    // axis labels
...
#solution("36.1")[
  ...
  double the theoretical floor, with still plenty of room to improve.
]
...
- #link("https://arxiv.org/abs/2309.10668")[*Language Modeling Is Compression* (Deletang et al., ICLR 2024)]: the DeepMind paper proving that LLMs are state-of-the-art lossless compressors when paired with arithmetic coding.
