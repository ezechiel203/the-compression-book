...
#objectives((
  [Write down a set, test membership, and combine sets with union, intersection, difference, and complement; read every symbol ($in$, $subset.eq$, $union$, $inter$, $emptyset$) fluently.],
  ...
  [Use the *pigeonhole principle* to prove that no compressor can shrink every possible input. This is your first impossibility result.],
...
We have two ways to pin down a set. The first, *roster notation*, simply lists the elements: $\{2, 3, 5, 7\}$. This is perfect when the set is small. But how would you list the set of all even numbers? You cannot, because it never ends. So we use the second way, *set-builder notation*, which describes the elements by a _rule_ they must satisfy:
...
#checkpoint[With $A = \{1,2,3,4\}$ and $B = \{3,4,5,6\}$ as above, what is $(A union B) minus (A inter B)$, and what everyday idea does that operation capture?][It is $\{1, 2, 5, 6\}$: the elements in exactly one of the two sets, never both. This is the *symmetric difference*, set theory's version of the XOR operation from Chapter 5: "in $A$ or in $B$, but not in both".]
...
The product of a set with itself, $A times A$, is written $A^2$. The set of all pixel coordinates on a $1920 times 1080$ screen is a product: $\{0, ..., 1919\} times \{0, ..., 1079\}$, which has $1920 times 1080 = 2{,}073{,}600$ pairs, the famous "two megapixels". The set of all $8$-bit bytes is the eightfold product $\{0,1\}^8$, with $2^8 = 256$ elements, which is precisely why a byte has $256$ possible values. The Cartesian product is the formal reason that "more slots multiply, not add".
...
#fig([Left: a genuine function, with every input on the left having exactly one arrow leaving it. Right: not a function, because the input $2$ fires two arrows (an input with two outputs), which a function forbids.],
cetz.canvas({
  import cetz.draw: *
  // left: valid function
...
#gopython("Functions as dicts, and def")[
  ...
  The second is a `dict` (dictionary), which stores a function as an explicit table of input-output pairs. This is perfect for a code, where the "rule" is just a lookup. The keys are the domain, the values are the outputs:
...
A function is *surjective* (or *onto*) when every element of the codomain is hit by _at least one_ input: the image fills the whole codomain, leaving no unused outputs.
...
To reverse a function $f: A -> B$ means to find a second function $f^(-1): B -> A$ (read "$f$ inverse") that perfectly undoes it: feed an output back in and get the original input out. Formally, $f^(-1)(f(a)) = a$ for every $a$, and $f(f^(-1)(b)) = b$ for every $b$. The big claim is that such an undoing function exists exactly when $f$ is a bijection, no more and no less.
...
#tryit[
  This chapter has no `tinyzip` build step of its own. The project's Python skeleton does not begin until Chapter 15, once we have learned the language. But the ideas here are so central that it is worth seeing them in code _right now_, as a preview. Below are the two functions that ask the make-or-break question of this chapter (_is this code safe to use?_), written with the sets and dicts exactly as the theory prescribes. (When `tinyzip` reaches its real Huffman coder in Chapter 24, this is precisely the check it will run.)
...
#gomaths("The geometric series $1 + 2 + 4 + dots.c + 2^(n-1) = 2^n - 1$")[
  ...
  Double both sides, so every term moves up to the next power:
...
#takeaways((
  ...
  [The *Cartesian product* $A times B$ is the set of ordered pairs; its size multiplies, $abs(A times B) = abs(A) times abs(B)$, which is why an $8$-bit byte has $2^8 = 256$ values.],
