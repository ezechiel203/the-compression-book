...
networks.], cetz.canvas({
  ...
  let stage(x, y, w, t1, t2, col) = {
    ...
    content((x + w/2, y + 0.6), box(width: (w - 0.2) * 1cm, align(center, text(size: 7pt, weight: "bold")[#t1])))
    content((x + w/2, y + 0.28), box(width: (w - 0.2) * 1cm, align(center, text(size: 6pt)[#t2])))
  }
  // classical row
  ...
  stage(0.4, 0.5, 1.8, "g_a net", "trained", rgb("#e6f0ea"))
  ...
  content((3.4, -0.2), text(size: 6.5pt, style: "italic")[same three jobs; every box learned from data])
}))
...
#gomaths("The Lagrange multiplier λ, as a price")[
  ...
  $lambda = 1$, the same quality is worth only $0.01$ bits, and the optimizer would
...
of the encoder as ordinary floating-point numbers, say $1.273$ or $-0.488$.
To compress them we must *quantize*: round each to a whole number, say to
...
The fix, introduced by Ballé and colleagues and now used by nearly every
...
#gopython("Checking the variance match by simulation")[
  ...
  underscore in `200_000` is just Python's digit separator (Chapter 15), which reads
...
#aside[
  ...
  flowing. Some codecs use a hybrid: they round in the distortion path but add noise
...
equalize_ the signal, spreading information out so no single number carries too
...
#gomaths("Generalized Divisive Normalization (GDN), gently")[
  ...
  energy of raw image features into something more uniform. Uniform,
...
where the image is calm, broad where it is busy. All automatic, learned, and
...
latent element gets its own Gaussian.], cetz.canvas({
  ...
  let bx(x,y,w,h,t,col) = {
    ...
    content((x+w/2, y+h/2), box(width: (w - 0.15) * 1cm, align(center, text(size: 6.5pt, weight:"bold")[#t])))
  }
  ...
  content((4.4,1.05), text(size:6pt, fill: rgb("#0b6e4f"))[$mu, sigma$ per elem.])
...
#algo(
  ...
  complexity: "One extra small autoencoder; still fully parallel encode/decode with no serial bottleneck. Practical and fast.",
...
These parallel context models are what the JPEG AI standard and the strongest
...
making $p$ optimistic to fake a low rate while the real bitstream is fat? It cannot,
...
is the same $p$. Coding against any model $p$ costs, on average, at least the
...
that gain for far faster decoding. JPEG AI was also designed to serve *two*
...
decisively ahead, a gap I will widen in Chapter 58, where generative decoders
...
#takeaways((
  ...
  [The *hyperprior* is a small second autoencoder that transmits side information describing the main latent's local Gaussian statistics ($mu$, $sigma$ per element): a few bits to describe the model in order to save many bits using it.],
  ...
  [Because coding under any model costs at least the true entropy ($H(q,p) >= H(q)$), the learned rate is honest: minimizing it _is_ matching the true latent distribution. This is Chapter 23's "compression = prediction," made mechanical.],
  [*JPEG AI* (International Standard, 2025) is the first learned image standard: hyperprior + parallel context, royalty-free, \~28.5% over VVC-intra, serving both human and machine vision. Classical codecs still rule deployment, gated by neural decode cost, hardware, and energy.],
))
...
#solution("57.5")[
  ...
  tells you how to read the symbols, short but making the whole map cheaper to
  interpret.
]
...
- Ballé, J., Laparra, V. & Simoncelli, E. (2017). #emph[End-to-End Optimized Image Compression]. ICLR 2017. #link("https://arxiv.org/abs/1611.01704")[arXiv:1611.01704]. Foundation paper: GDN and the noise-quantization trick.
- Ballé, J. et al. (2018). #emph[Variational Image Compression with a Scale Hyperprior]. ICLR 2018. #link("https://arxiv.org/abs/1802.01436")[arXiv:1802.01436]. The hyperprior, this chapter's centerpiece.
...
- Minnen, D. & Singh, S. (2020). #emph[Channel-wise Autoregressive Entropy Models for Learned Image Compression]. ICIP 2020. #link("https://arxiv.org/abs/2007.08739")[arXiv:2007.08739]. Parallel context that restored practical speed.
...
#bridge[
  ...
  *Chapter 58* confronts this head-on with the *rate-distortion-perception*
  ...
  smear to mush. The catch is invented detail that was never there, which forces
  a hard question: when is a "reconstruction" no longer the truth?
]
