...
On 19 May 2010 at Google I/O, Google released VP8 under a BSD-style source license and --- crucially --- an irrevocable, royalty-free patent license covering the bitstream specification. This was not just "free software": it was a pledge that Google would never sue anyone for implementing VP8, and would defend against third-party patent claims on the core technology.
...
#aside[
  Nokia separately sued HTC (which shipped Android phones with VP8) in 2012, claiming VP8 infringed Nokia's video patents. That suit was eventually settled quietly. The pattern --- large incumbent threatens open codec with patent litigation, then retreats --- would repeat with AV1.
]
...
- *Larger superblocks*: VP9 raised the coding-unit maximum from 16×16 to 64×64 pixels --- the same insight that HEVC's CTU embodied, arrived at independently. Large flat areas in 4K video could now be described in a single block with minimal overhead.
...
- *Segmentation*: VP9 introduced per-segment feature overrides (quantizer, loop-filter strength, reference frame eligibility), allowing fine-grained control of how different regions of a frame are coded.
...
- *Tiling*: Frames could be divided into independently decodable tiles, enabling multi-threaded decode --- critical for real-time 4K on multi-core devices.
...
By 2016, YouTube was using VP9 for the majority of its video delivery to Chrome users. The scale was staggering: tens of billions of minutes of VP9 video decoded per day. This gave Google real-world feedback at a scope no research lab could simulate --- finding edge cases in the spec, discovering encoding pathologies on unusual content, and motivating the `libvpx` encoder's continuous improvement.
...
Google could see, by 2014, that VP9 was a real commercial success --- but also that a single company controlling an open codec invited skepticism. If Google ever changed its mind about openness, or if patent litigation pinned VP9 from an unexpected direction, the royalty-free web video project would collapse.
...
Standard B-frame inter prediction blends two reference frames --- one past, one future. AV1 extends this idea in several directions.
...
```
  ...
  A difference of 4 dB in PSNR is perceptible on a good monitor. That 4 dB costs roughly 10--50x more encode time --- a stark illustration of the diminishing-returns curve at the heart of encoder optimization.
]
...
The dispute escalated enough that the European Commission opened a preliminary investigation in 2021 into whether AOMedia's royalty-free licensing policy constituted an anticompetitive practice --- essentially, investigating whether a group of companies _agreeing to give technology away for free_ could somehow harm competition. In May 2023, the EC closed the investigation without finding a violation. AOMedia called it a vindication; the streaming industry collectively exhaled.

As of mid-2026, AV1 is deployed at massive scale --- Netflix, YouTube, Meta, Amazon, Android, Chrome, Firefox, Edge --- without paying royalties to Sisvel or any other third-party pool. The legal dust has not entirely settled, but the commercial reality is clear.
...
AV2 supports *spatial* and *temporal scalability* --- the ability to extract a lower-resolution or lower-frame-rate version of the video from the bitstream without re-encoding. This is valuable for adaptive streaming: a single AV2 encode can serve 4K, 1080p, and 720p clients from the same file.
...
Netflix's Director of Codec Development Andrey Norkin framed the adoption reality plainly: production AV2 deployment begins "once device support makes it sensible" --- a single sentence that captures the entire economics of codec transitions.
...
#takeaways((
  ...
  "VP9 (June 2013) raised superblock size to 64x64, added 10 intra prediction angles, adaptive entropy contexts, and tiled parallel decode. YouTube deployed it at exabyte scale, making it one of the most-decoded codecs ever.",
  ...
  "AV1 (June 2018) introduced 10 partition types, CDEF for ringing suppression, the Wiener/SGR loop-restoration filter for banding, warped affine motion compensation, compound prediction modes, and film grain synthesis --- ~30% over HEVC, ~62% over H.264.",
...
#solution("54.4")[
  Translational motion compensation models each block as having moved by a fixed pixel offset $(Delta x, Delta y)$ --- the block shifts uniformly with no rotation, scaling, or shearing. Affine motion compensation allows the block to be rotated, scaled, and sheared in addition to translated, using six parameters (two per row of the $2 times 3$ affine matrix). A clear advantage scenario: a drone filming a city from above while flying forward. The camera zooms in slightly (scale change) and may rotate as the pilot adjusts heading. Every block in the frame moves with a different zoom amount depending on its distance from the image center --- objects near the center zoom less; objects at the edges zoom more. Translational motion vectors must use a different vector for every block and still leave large residuals at the zoom boundaries. A single global affine warp model captures the entire camera motion in six parameters, reducing residuals across the whole frame simultaneously.
]
