...
cetz.canvas({
  ...
  content((width / 2, 1.7))[#box(width: 9cm, inset: 1pt, align(center, text(size: 8pt, fill: rgb("#0b5394"))[← 24 Bark bands (logarithmic frequency axis) →]))]
}))
...
cetz.canvas({
  ...
  // labels
  content((5.0, 6.4))[#box(width: 7cm, inset: 1pt, align(center, text(size: 8pt, fill: rgb("#9a2617"))[Absolute Threshold of Hearing]))]
...
cetz.canvas({
  ...
  content((3.5, 1.75))[#box(width: 0.6cm, inset: 1pt, align(center, text(size: 7pt, fill: rgb("#9a2617"))[drum hit]))]
...
A band with SMR of 30 dB needs about 5 bits per sample to push the noise 30 dB below the signal (and thus under the mask). A band with SMR of 0 dB (signal right at the mask) needs 0 bits and can be dropped.
...
Johnston's key measurement was this: for a wide variety of CD-quality music, the perceptual entropy is approximately *2.1 bits per sample*. CD audio uses 16 bits per sample. So the theoretical compression ratio for transparent quality is about $16 / 2.1 approx 7.6:1$. At 44,100 samples per second, stereo, that corresponds to roughly 185 kbps, which is not coincidentally close to the perceptual transparency threshold that audiophiles quote for well-tuned MP3 encoders.

#keyidea[
  Perceptual entropy is the Shannon entropy of the signal *after masking*: the bits that actually matter to the listener. Johnston's 1988 measurement of about 2.1 bits/sample for typical music implies that a ratio of approximately 7:1 is the hard perceptual limit for lossless-sounding audio compression, regardless of how good the codec is. Better codecs approach this limit; they cannot beat it.
]

This is a profound result. It says that the 11:1 ratio achieved by 128 kbps MP3 in the late 1990s was actually slightly beyond the perceptual transparency threshold for typical listeners in typical conditions. A 320 kbps MP3 was providing about twice as many bits as transparency requires, just for safety margin.
...
Psychoacoustic models work beautifully when the audio signal is *stationary*, meaning its frequency content is roughly the same across the analysis window. But music is not stationary. Transients (drum hits, piano attacks, consonant plosives in speech) introduce sudden, sharp changes that violate this assumption.
...
cetz.canvas({
  ...
  content((0.9, 2.0))[#box(width: 1.4cm, inset: 1pt, align(center, text(size: 7pt)[PCM input]))]

  // filterbank / MDCT
  rect((2.2, 1.5), (4.0, 2.5), fill: box_fill, stroke: box_stroke, radius: 3pt)
  content((3.1, 2.0))[#box(width: 1.4cm, inset: 1pt, align(center, text(size: 7pt)[Filter bank (MDCT)]))]
  line((1.8, 2.0), (2.2, 2.0), mark: (end: ">"), stroke: 0.7pt)

  // quantizer
  rect((4.4, 1.5), (6.2, 2.5), fill: box_fill, stroke: box_stroke, radius: 3pt)
  content((5.3, 2.0))[#box(width: 1.4cm, inset: 1pt, align(center, text(size: 7pt)[Quantizer]))]
  line((4.0, 2.0), (4.4, 2.0), mark: (end: ">"), stroke: 0.7pt)

  // entropy coder
  rect((6.6, 1.5), (8.4, 2.5), fill: box_fill, stroke: box_stroke, radius: 3pt)
  content((7.5, 2.0))[#box(width: 1.4cm, inset: 1pt, align(center, text(size: 7pt)[Entropy coder]))]
  line((6.2, 2.0), (6.6, 2.0), mark: (end: ">"), stroke: 0.7pt)

  // bitstream output
  rect((8.8, 1.5), (10.6, 2.5), fill: box_fill, stroke: box_stroke, radius: 3pt)
  content((9.7, 2.0))[#box(width: 1.4cm, inset: 1pt, align(center, text(size: 7pt)[Bitstream output]))]
  ...
  content((4.2, 0.55))[#box(width: 3.6cm, inset: 1pt, align(center, text(size: 7pt, fill: model_stroke)[Psychoacoustic model (masking threshold T(f))]))]
...
#aside[
  This asymmetry between a complex encoder and a simple decoder is a deliberate design choice. There is one encoder (or a small number of professional mastering tools), but millions of decoders (every phone, browser, car). Moving complexity to the encoder is almost always the right trade-off for mass-market distribution formats.
]

== Why This Matters: The Perceptual Coding Revolution

The shift from signal coding to perceptual coding was genuinely radical, and I think it deserves a moment's attention.
...
Modern codecs address these failure modes with refinements: better transient detection, shape-adaptive quantization, joint-stereo psychoacoustic modelling, and more recently neural post-processors that learn the residual errors that classical psychoacoustic models leave behind. (Chapter 49 covers Opus 1.5's neural layer, which does exactly this.)
...
#solution("46.4")[
  ...
  Shorter blocks have better time resolution: a 5 ms block captures a smaller time slice, so a transient can only dominate its entire block if the block is short enough that the transient fills it. The trade-off: shorter blocks have fewer samples, so the FFT has fewer frequency bins and worse frequency resolution. Critical bands become wider, and it is harder to allocate bits to individual frequency ranges. Codecs therefore use long blocks for stationary signals (good frequency resolution, better masking) and switch to short blocks for transients (better time resolution, pre-echo prevention).
]
...
- Zwicker, E. & Fastl, H. (1999). _Psychoacoustics: Facts and Models_ (2nd ed.), Springer. The technical reference; Zwicker invented the Bark scale and was the leading figure in quantitative psychoacoustics.
