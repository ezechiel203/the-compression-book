...
#fig([A continuous signal (smooth curve) and the discrete signal obtained by *sampling* it at evenly spaced instants, keeping only the dots. The whole question of this chapter is: when do the dots remember the curve?], cetz.canvas({
  ...
  content((8.4, -0.25), text(size: 9pt)[time $t$])
  content((-0.35, 1.5), text(size: 9pt)[$s$])
  let pts = ()
...
A worked example fixes the units in place. Suppose we sample a $1$-second tone at $f_s = 8{,}000$ Hz. Then the sampling interval is $T = 1\/8000 = 0.000125$ s $= 125$ microseconds, and we collect exactly $f_s times 1 "s" = 8{,}000$ samples. If instead we record $10$ seconds at the CD rate $44{,}100$ Hz, we collect $44{,}100 times 10 = 441{,}000$ samples; in _stereo_ (two channels) that doubles to $882{,}000$ numbers, each typically $2$ bytes, giving about $1.76$ megabytes for ten seconds of raw audio. That number is exactly why compression exists: a three-minute song is over $30$ megabytes raw, and MP3 will shrink it tenfold by working, as we will see, in the frequency domain.
...
*Amplitude* $A$ is the radius of the wheel, measuring how far the height swings from the middle to the top. Bigger $A$ means a louder sound or a brighter stripe.

*Frequency* $f$ is how many full turns the wheel makes per second, which is how fast it wiggles, measured in hertz (Hz). One full turn is one *cycle*. A 440 Hz wave (musical A) completes 440 cycles every second.

*Phase* $phi.alt$ records where the painted point started: it is a head-start, a shift left or right in time. It decides whether the curve begins at zero, at the top, or somewhere between.
...
#fig([Building a jagged signal by adding sinusoids. The flat-topped wave (heavy line) is the sum of just three odd-frequency sine waves (thin lines): a square wave is mostly its lowest frequency plus shrinking high-frequency corrections.], cetz.canvas({
  import cetz.draw: *
  line((-0.2, 0), (8.2, 0), mark: (end: ">"))
  content((8.45, -0.25), text(size: 9pt)[$t$])
  let comp(mult, amp) = {
    let p = ()
    for i in range(0, 161) {
      let x = i/20
...
#keyidea[
To capture a signal whose fastest wiggle is $B$ hertz, sample _faster than twice_ $B$. You need at least two samples per cycle of the fastest sinusoid present: one to catch its rise, one to catch its fall. Below that rate the fast wiggle disguises itself as a slow one (aliasing). The threshold $f_s = 2B$ is the *Nyquist rate*; half your sampling rate, $f_s\/2$, is the highest frequency you can honestly record.
]
...
The single most consequential theorem for digital media. It tells the engineer the one number that decides whether digitising a signal keeps the music or wrecks it: the sampling rate. Sample above $2B$ and the analog world fits, losslessly, into a finite list of numbers; sample below and you suffer aliasing forever.
]

== Aliasing: when fast pretends to be slow

What actually goes wrong when you sample too slowly is worth seeing directly, because *aliasing* is the constant enemy that every camera, microphone, and codec must defend against. When the sampling rate falls below the Nyquist rate, a frequency that is too high to capture does not simply vanish. It _disguises itself_ as a lower frequency that the sampling rate _can_ represent, and the disguise is perfect: from the samples alone, the impostor is indistinguishable from a genuine low tone. The high frequency has stolen the "alias" of a low one.
...
#fig([Aliasing. A fast sine wave (light) sampled too slowly (dots) is indistinguishable from a slow sine wave (heavy) drawn through the very same dots. From the samples alone you cannot tell which signal you had.], cetz.canvas({
  ...
  content((8.6, -0.25), text(size: 9pt)[$t$])
  let fast = ()
...
Two pieces of vocabulary make this readable. The number $abs(X_k)$ is how _much_ of frequency $k$ there is; the collection of all the $abs(X_k)$ is the *magnitude spectrum*, the picture you see on a graphic equaliser's dancing bars. For a real-valued signal, $X_k$ and $X_(N-k)$ are mirror images (complex conjugates), so the genuinely new information lives in the first half, frequencies $0$ up to $N\/2$. That is exactly the Nyquist frequency again, reappearing on its own. That two-for-the-price-of-one symmetry is why a real-valued block of $N$ samples really only has about $N\/2$ independent frequencies, and it is exploited by every real-signal FFT to run twice as fast.
...
and it answers "how _much_ of frequency $k$ is present," meaning the loudness of that pitch. The *phase* is the angle,
...
#fig([The DFT as a change of view. The same data, twice: on the left a row of time samples; on the right the magnitude spectrum, showing how much of each frequency is present. Compression happens because the right-hand picture is often mostly empty.], cetz.canvas({
  import cetz.draw: *
  content((1.4, 2.0), text(size: 9pt)[*time domain*])
  ...
  content((6.6, 2.0), text(size: 9pt)[*frequency domain*])
...
#checkpoint[Without computing all of it, what is $X_0$ for the samples $x = (3, 1, 4, 1, 5, 9, 2, 6)$?][$X_0$ is always the plain sum of the samples (every rotation factor is $1$ when $k = 0$): $3+1+4+1+5+9+2+6 = 31$. It is the signal's total; dividing by $N = 8$ gives the average, $3.875$, which is the "DC" level the signal hovers around.]
...
So nothing is lost in going to the frequency domain and back. It is a lossless change of coordinates, a rotation of the data in the precise sense of Chapter 12's orthogonal transforms. Why this works is worth proving, because the same orthogonality is the engine inside every transform codec to come.
...
That someone, in the public record, was *James Cooley* of IBM and *John Tukey* of Princeton, who in 1965 published _An Algorithm for the Machine Calculation of Complex Fourier Series_. Their *Fast Fourier Transform* (FFT) computes the _exact same_ DFT (same outputs, to the last bit) but in $O(N log N)$ operations instead of $O(N^2)$. The trick is *divide and conquer* (Chapter 14): split the $N$-point DFT into two $N\/2$-point DFTs, one over the even-indexed samples and one over the odd-indexed, then stitch their results together with a single layer of cheap combinations. Each half splits again, and again, $log_2 N$ times down to trivial one-point transforms; at each level the stitching costs only $O(N)$, so the total is $O(N log N)$.
...
I will not code the full FFT here; Chapter 38 builds its close relative, the *Discrete Cosine Transform*, which is the form compression actually uses. Every numerical library ships a battle-tested FFT (`numpy.fft.fft`, for instance), and whenever this book says "take the spectrum," an FFT is what runs under the hood.
...
#fig([A spectrogram: time on the horizontal axis, frequency on the vertical, darkness showing energy. Two rising tones (the diagonal bands) and a steady hum (the flat band) become visible at once, showing _when_ and _at what pitch_, together.], cetz.canvas({
  import cetz.draw: *
  // frame and axes
  line((0, 0), (7.4, 0), mark: (end: ">"))
  line((0, 0), (0, 3.2), mark: (end: ">"))
  content((7.5, -0.3), text(size: 9pt)[time])
  content((-0.55, 3.1), text(size: 9pt)[freq])
...
Three more reasons tie the whole arc together. First, *decorrelation*: neighbouring samples in time are highly correlated (one pixel predicts the next), which wastes bits. The Fourier coefficients are nearly _uncorrelated_, so each can be coded independently, the very property Chapter 21 named and which the next chapter's transforms achieve. Second, *perceptual selectivity*: human hearing and vision are organised by frequency (Chapter 46's critical bands, Chapter 42's contrast sensitivity), so the frequency axis is where "what people notice" is easiest to express; you can be coarse in the bands the senses ignore. Third, the *sampling theorem* itself tells us the highest frequency worth keeping ($f_s\/2$); everything above is either inaudible or was filtered away, so the spectrum has a natural, finite top edge across which to budget bits.

#aside[
There is a small but important caveat the next chapter resolves. The plain DFT assumes the signal _repeats_ forever, which it does not; chopping a long signal into blocks and DFT-ing each block creates artificial jumps at the block edges that smear energy across all frequencies (called *spectral leakage*) and cause audible or visible *blocking artifacts*. The fixes are tapering each block with a *window* and using the overlapping *Modified DCT* (MDCT) that cancels the seams exactly. Those are the subjects of Chapter 38 and the audio chapters. For now, just know that "transform each block" hides a real wrinkle, and that clever windowing irons it out.
]
...
#scoreboard(caption: "where the frequency domain enters the pipeline (preview)",
  ...
  [After quantizing small coefficients], [≈10–20%], [5–10×], [the lossy step, subject of Ch 38–42; bytes finally melt],
)

The scoreboard above is a _preview_, not a measurement: this chapter adds no codec to `tinyzip` and melts no bytes yet. It marks where, in the chapters just ahead, the savings will come from. The transform itself is free of loss; it merely sets the table. In the next chapter I build the specific transform compression actually uses, the Discrete Cosine Transform, and add `transform.py` to `tinyzip`. Then quantization (Chapter 39) and JPEG (Chapter 42) finally turn this rearrangement into shrinking files.
...
#takeaways((
  ...
  [The *FFT* computes the DFT exactly in $O(N log N)$ instead of $O(N^2)$, a ~50,000x speed-up at a million points. That single algorithm made all digital media practical.],
...
#solution("37.3")[
$X_0 = 1+1+1+1 = 4$. For $X_1$: $1 dot 1 + 1 dot(-i) + 1 dot(-1) + 1 dot(i) = (1-1) + (-i + i) = 0$. By the same cancellation, $X_2 = 1 - 1 + 1 - 1 = 0$ and $X_3 = 0$. The spectrum is $(4, 0, 0, 0)$: _all_ the energy is at frequency $0$. That is correct, since a constant signal has no oscillation at all, only a DC (zero-frequency) level, here equal to $N$ times the average value $1$.
]
...
- E. O. Brigham, _The Fast Fourier Transform and Its Applications_ (Prentice Hall, 1988). A picture-rich book-length treatment of everything in this chapter.
