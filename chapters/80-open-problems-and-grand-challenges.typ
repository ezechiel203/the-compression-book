...
For *general English text*, the most famous estimate is Shannon's own. In 1951 he ran an experiment: he showed people partial sentences and asked them to guess the next letter, and from how often they guessed right he estimated the entropy of English at roughly *0.6 to 1.3 bits per character*. Ordinary text stored in ASCII uses 8 bits per character; even a good classical compressor lands around 2 bits. So Shannon's number says there is still a factor of two or more left on the table for a compressor that truly *understood* English the way a person does.
...
Now the problem that has come to dominate the entire field: a codec is not its compression ratio alone. It is a *budget*. Every byte you save costs computation to save and computation to undo, and that computation costs *time*, *silicon*, *battery*, and ultimately *carbon*.
...
The classical picture had two knobs: *rate* (bits) and *distortion* (signal error). Recent theory adds a third, the *rate–distortion–perception* tradeoff we met in Chapter 58, and semantic coding pushes further still to *rate–task* tradeoffs, where the only thing that matters is whether the receiver's job got done. Each new axis is a new frontier of open theory: we do not yet have a clean, general "semantic rate–distortion theorem" the way Shannon gave us one for signals.

#misconception[
  Semantic compression means the codec "understands" the content the way a human does.
][Not in any deep sense. It means the encoder is *trained jointly with the downstream task* so that the bits it keeps happen to be the ones the task needs. There is no comprehension — just a learned, task-shaped bottleneck. But the effect is striking: for a fixed task, you can often cut the bitrate by an order of magnitude versus sending a faithful image, because most of the image was never relevant to the task in the first place.]

The danger, of course, is that *goal-oriented* means *lossy in a way that bites the moment the goal changes*. Compress a medical scan to preserve only "is there a tumor?", and you have thrown away everything the next, unforeseen question would have needed. Classical compression is *general* — the reconstruction serves any future purpose. Semantic compression is *specialized* — it serves one purpose superbly and betrays all the others. Knowing when that trade is wise is an unsolved engineering and ethical question.
...
Here is the frontier that ties the others together, and it starts from a simple observation about *who is watching*.
...
To make "coding for machines" concrete, here is the smallest possible illustration of the core idea — that the *right* thing to keep depends entirely on the receiver's goal. We will not build a neural codec (that needs the machinery of Chapter 57); we will simulate the *principle* in a dozen lines. Our "image" is a list of pixel brightnesses. A *human* receiver wants the picture to look right (low average error). A *machine* receiver has one job: answer "is the brightest spot above a threshold?" We will see that a compression that is terrible for the human can be perfect for the machine — at a fraction of the bits.
...
- *A semantic rate–distortion theory.* Shannon gave us $R(D)$ for signals (Chapter 21). There is no comparably clean, general theory for *task* fidelity — for goal-oriented compression.
...
#takeaways((
  ...
  "Semantic / goal-oriented compression abandons Shannon's signal-fidelity contract: it keeps only what the receiver's task needs, measured by task success, not distortion — superb for one purpose, brittle for all others.",
  ...
  "Every open problem is a face of one sentence — the best compressor is the best model of the data — which is why the frontier of compression has merged with the frontier of machine learning itself.",
))
...
#solution("80.4")[
  (a) The theorem assumes (i) *infinite block length / unlimited delay* — but a live call must send tiny packets with hard latency limits, far from the infinite-length limit where separation is proved optimal; and (ii) a *fixed, known channel* — but a wireless link fades and varies moment to moment as you move, so the channel the code was designed for is not the channel you actually get. (b) A separated system protects the compressed bits with an error-correcting code rated for some noise level. While the channel stays within that level, decoding is perfect; the instant noise exceeds it, the error correction is overwhelmed and the decoded bits become garbage *all at once* — perfect, then suddenly destroyed: the cliff. A joint system maps the source directly to transmitted signals with no sharp bit boundary, so as noise rises the reconstruction just gets gradually fuzzier instead of collapsing. (c) On a call, a momentarily fuzzy face you can still recognize and a voice you can still understand is vastly more useful than a stream that is crisp until it abruptly freezes and blocks out entirely. Humans tolerate gradual degradation gracefully; the cliff is jarring and breaks the interaction. So for short, urgent, noisy links, *graceful* beats *peak*.
]
...
#bridge[
  We have stared straight at the unsolved. Every open problem turned out to be a face of one idea: the best compressor is the best model, and our models have grown into the engines of modern AI. The final chapter, *The Future of Compression* (Chapter 81), steps back from the open problems to ask where the whole arc is heading — why, in a world where data grows faster than the chips that store it, compression stays not just useful but *invaluable*, the quiet substrate beneath intelligence, communication, and the sustainability of computing itself.
]
