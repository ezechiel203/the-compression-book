...
cetz.canvas({
  ...
  let bx(x, y, w, h, label, col) = {
    ...
    content((x+w/2, y+h/2), box(width: (w - 0.15) * 1cm, inset: 1pt, align(center, text(size: 7pt)[#label])))
  }
  bx(0, 0, 1.4, 0.7, "message", rgb("#f4f6f8"))
  bx(1.7, 0, 1.5, 0.7, [source\ coder], rgb("#cfe3d8"))
  bx(3.5, 0, 1.7, 0.7, [channel\ coder], rgb("#cfe0f0"))
  bx(5.5, 0, 1.2, 0.7, "channel", rgb("#f0d9d6"))
  bx(7.0, 0, 1.7, 0.7, [channel\ decoder], rgb("#cfe0f0"))
  bx(9.0, 0, 1.5, 0.7, [source\ decoder], rgb("#cfe3d8"))
  bx(10.8, 0, 1.3, 0.7, "message", rgb("#f4f6f8"))
  line((1.4,0.35),(1.7,0.35), mark: (end: ">"))
  line((3.2,0.35),(3.5,0.35), mark: (end: ">"))
  line((5.2,0.35),(5.5,0.35), mark: (end: ">"))
  line((6.7,0.35),(7.0,0.35), mark: (end: ">"))
  line((8.7,0.35),(9.0,0.35), mark: (end: ">"))
  line((10.5,0.35),(10.8,0.35), mark: (end: ">"))
  content((2.45,0.9), text(size: 6.5pt, fill: rgb("#0b6e4f"))[remove redundancy])
  content((4.35,0.9), text(size: 6.5pt, fill: rgb("#0b5394"))[add redundancy])
  content((6.1,-0.35), text(size: 6.5pt, fill: rgb("#9a2617"))[noise enters here])
}))
...
#keyidea[
  *Compress-then-protect.* Run the data through the best compressor you can (Volumes
  ...
  hands the decompressor a clean stream, so the brittleness never gets tested.
]
...
corrupted word for *any* small code, with no algebra required, just counting
...
#history[
  ...
  entire discipline of coding theory. Hamming won the 1968 Turing Award.
]
...
word $1101101$ is now *exactly divisible* by $G$, so its remainder is zero. The
...
#proof[
  ...
  non-zero constant term, so $x^j$ shares no factor with $G$, meaning $G$ cannot divide
...
#theorem("Reed–Solomon correction budget")[
  ...
  It can correct any $n - k$ _erasures_, or any $t$ _errors_ with $2t <= n - k$:
...
tells us Reed--Solomon is *provably optimal* for its rate: you
...
#proof[
  ...
  _maximum-distance separable_, extracting the maximum possible protection from
...
#mathrecall[The _Gaussian_ (or _AWGN_, additive white Gaussian noise) channel is a
...
#keyidea[
  ...
  The question was "can we build codes that are *both* near-capacity *and* efficiently
...
that reason. Two breakthroughs finally crashed through the wall: a 1990s resurrection
and a 2008 invention. They now carry your Wi-Fi, your 5G, your
satellite TV.
...
belief propagation*, where each bit and each parity check passes "messages" of soft
...
#keyidea[
  *Channel polarization.* Combine $N$ copies of a mediocre channel through a simple
  ...
  grows, something remarkable happens: the synthetic channels _polarize_, each becoming
...
#gomaths("Polarization from two channels")[
  ...
  XORs, cheap to build and $O(N log N)$ to decode by _successive cancellation_.
]
...
#algo(
  ...
  weaknesses: [Plain successive-cancellation decoding is mediocre at finite lengths and needs CRC-aided _list_ decoding to compete; inherently somewhat serial; tricky rate-matching.],
...
co-design, the places where the separation theorem's fine print bites. The first is the
...
*compressible* text, and a compression engineer's instinct is to *shrink* it. The QR
...
WebRTC, Zoom, and Discord. On a real network, packets *drop* for all the usual
reasons: Wi-Fi hiccups, cellular handoffs, congestion. Each lost packet is a hole in someone's
...
#keyidea[
  ...
  speech into a few hundred bits, with compression and protection fused into one learned
...
its reads have become, then *adapts* the code rate over the drive's life, adding more
...
#exercise("72.8", 3)[
  ...
  cheaper. (b) Argue why DRED is a productive violation of strict
