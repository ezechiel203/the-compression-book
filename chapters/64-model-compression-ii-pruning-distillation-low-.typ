...
These three techniques are not competitors. A real production pipeline at Google, Meta, or Huawei often applies all three in sequence: prune, then distill, then quantize. The goal in every case is the same one that has run through this entire book: exploit the redundancy in your data — and a trained neural network turns out to be extraordinarily redundant.
...
#fig([Three pruning regimes, illustrated on a 4×4 weight matrix. Stars mark removed weights.],
  cetz.canvas({
    ...
    // Title labels
    content((1.4, 3.5), text(size: 8pt)[Unstructured])
    content((5.4, 3.5), text(size: 8pt)[Structured (row)])
    content((9.4, 3.5), text(size: 8pt)[Semi-structured 2:4])
...
#algo(
  ...
  complexity: "O(T × k) where T = full training time and k = number of pruning rounds; very expensive — you train the full network k+1 times total.",
...
The idea draws directly from Optimal Brain Surgeon (1993) and from the same Hessian-compensation trick that GPTQ used for quantization. In fact, the math is identical in structure — the connection is intentional and explicit in the paper.
...
To reach 50% sparsity in this row, prune the two lowest-scoring weights: positions 2 and 3 (the naive magnitude pruner would have kept position 2 because $0.50$ looks large, while removing position 1 because $0.01$ looks small — the exact opposite of Wanda's correct answer).
...
- *Entire layers*: *SliceGPT* (2024, from Microsoft Research) removes entire weight columns and rows from matrices, effectively reducing the model's hidden dimension. The result is a model with the same number of layers but uniformly narrower — easy to run on hardware optimized for smaller dense matrices.
...
#algo(
  ...
  strengths: "Can close most of the accuracy gap between student and teacher; works without the teacher's training data (black-box distillation); student is architecturally free — any smaller model works; widely deployed (every major tech company uses some form of this).",
...
The challenge: choosing $k$ per layer (different layers may have different effective ranks) and the accuracy loss from discarding singular values. A well-known result (Eckart–Young theorem) says the rank-$k$ approximation by SVD is the *best possible* rank-$k$ approximation in the _Frobenius norm_ sense — the Frobenius norm being just the $L_2$ norm of a matrix flattened into one long vector, $norm(W)_F = sqrt(sum_(i,j) w_(i j)^2)$, so "small Frobenius distance" means "the two matrices differ by little, entry by entry." But "best approximation of the weight matrix" is not the same as "best approximation of the model's behavior" — you are minimizing the wrong objective.
...
#fig([LoRA architecture. The frozen pretrained weight $W_0$ (grey) handles the main computation. Two small trainable matrices $A$ and $B$ add a low-rank correction $B A x$ to the output. After fine-tuning, $B A$ is merged into $W_0$ for zero inference overhead.],
  cetz.canvas({
    ...
    content((1, 1), text(size: 8pt, fill: rgb("#555555"))[$W_0$ (frozen)])

    // Lower branch: A then B
    rect((0, -1.8), (1.2, -1.0), fill: rgb("#b7e4c7"), stroke: 1pt)
    content((0.6, -1.4), text(size: 7pt)[$A$ ($r times n$)])
    rect((1.4, -1.8), (2.0, -1.0), fill: rgb("#aed6f1"), stroke: 1pt)
    content((1.7, -1.4), text(size: 7pt)[$B$ ($m times r$)])
    ...
    line((2.0, -1.4), (3.2, -1.4), stroke: 0.5pt)
...
A 2024 development worth knowing: *GaLore* (Zhao et al., ICML 2024) extends the low-rank insight from *weight updates* to *gradients*. Recall from Chapter 56 that training adjusts each weight by following its _gradient_ (the slope of the loss). Modern training does not use the raw gradient: the standard *Adam* optimizer keeps two extra running averages per weight — a smoothed gradient (its "momentum") and a smoothed squared gradient (its "second moment") — to take steadier steps. Storing the gradient plus this optimizer state costs 8–12 bytes per parameter — for a 7B model, that is 56–84 GB just for optimizer state. GaLore projects the gradient matrix into a low-rank subspace (using a periodically updated SVD basis) before accumulating into the optimizer state. This reduces optimizer memory by 4–8× with almost no accuracy loss.
...
#solution("64.1")[
  ...
  As temperature increases: D's probability rises from 1.5% to 12.3% — a large relative increase. This amplification of near-zero probabilities is exactly what makes soft targets useful for distillation.
]
...
#link("https://arxiv.org/abs/2301.00774")[Frantar and Alistarh, "SparseGPT: Massive Language Models Can Be Accurately Pruned in One Shot," ICML 2023] — the extension of Optimal Brain Surgeon to trillion-parameter scale; the mathematical connection to GPTQ is made explicit.
