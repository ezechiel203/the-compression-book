...
#objectives((
  ...
  [Use "the derivative is zero at a peak or valley" to _optimise_: find the input that minimises or maximises a quantity,],
...
That is the expectation from Chapter 10, now in its natural notation. When the range is obvious (say, "over every symbol $s$ in the alphabet"), we get lazy and write $sum_s$ with nothing above or below, meaning "add over all of them."
...
A handy closed form we will reuse: the first $n$ whole numbers add to $sum_(i=1)^(n) i = (n(n+1))/2$. Check it at $n=5$: $(5 dot 6)/2 = 15$. The young Carl Friedrich Gauss reputedly found this in seconds as a schoolboy by pairing $1+100$, $2+99$, and so on - fifty pairs each summing to $101$.
]
...
`range(1, 6)` produces the integers from 1 up to *but not including* 6. That is Python's universal "start, stop" convention, which trips up every beginner once. The variable `total` starts at the sum's empty value, `0`, exactly as a $sum$ with no terms equals $0$.
...
This product drives every statistical compressor: a model that assigns a probability to each next symbol assigns, by multiplication, a probability to the entire file. As Chapter 18 will show, the ideal number of bits to store the file is the logarithm of one over that product.
...
Subtract the second line from the first. Every middle term cancels its twin (a *telescoping* cancellation), leaving only the two ends:
...
A quick sanity check on the torn paper, with $a = 1\/2$, $r = 1\/2$, $n = 4$: the formula gives $(1\/2) dot (1 - (1\/2)^4)/(1 - 1\/2) = (1\/2) dot (1 - 1\/16)/(1\/2) = 1 - 1\/16 = 15\/16$. And indeed $1\/2 + 1\/4 + 1\/8 + 1\/16 = 15\/16$. Four tears leave you one-sixteenth short of a whole sheet, exactly as the picture suggests.
...
For completeness, the arithmetic cousin sums just as cleanly. The *method* used to crack the geometric series (making terms cancel in pairs) deserves a name, because it recurs whenever a compressor analyses its own running cost.
...
The arrow notation $n -> infinity$ reads "as $n$ tends to infinity." A limit is not "the last term" - there is no last term. It is the *destination* of an endless process, which the process may approach without ever arriving. That is the conceptual leap, and it is the one piece of genuinely new thinking in this chapter; everything else is arithmetic.
...
We need $sum_(k=1)^infinity k thin q^(k-1)$. This is an infinite series with an extra factor of $k$; the slick way to get it is to *differentiate* the plain geometric series, a calculus move we develop in the next section. The result is $sum_(k=1)^infinity k q^(k-1) = 1/(1-q)^2 = 1/p^2$. Therefore
...
The `assert` statement raises an error if its condition is false, so a silent formula bug becomes a loud crash. We will reuse this defensive habit throughout the project: `assert` your invariants. Run it; it prints `geometric sums verified`.
]
...
#fig([Zooming in on a curve at a point. From far away the curve bends; up close, near the point, it is indistinguishable from a straight line, the *tangent*. The slope of that line is the derivative at the point. Here the curve $y = x^2$ is flat at the bottom (slope 0) and steeper to the right (slope grows).], cetz.canvas({
  ...
  content((2.5, 3.25), text(size: 9pt, fill: rgb("#0b5394"))[$y = x^2$])
...
$ f'(p) = 1 - 2p. $
Set $f'(p) = 0$: $1 - 2p = 0$, giving $p = 1\/2$. The slope is zero there, at the peak, paying a second Chapter 10 IOU.
...
The three rules differentiate functions built by adding and multiplying. One common shape they miss is a function *inside* another, a *composition*, like $(y - hat(y))^2$, which is "square" wrapped around "subtract." The *chain rule* handles these: to differentiate an outer function of an inner one, multiply the outer derivative (with the inner left alone) by the inner derivative. In symbols, if $h(x) = f(g(x))$ then $h'(x) = f'(g(x)) dot g'(x)$, read "outer-prime times inner-prime."
...
#keyidea[
At the very bottom of a valley, or the very top of a hill, a smooth curve is momentarily *flat*: its slope is zero. To find where a quantity is smallest or largest, find where its derivative is zero. These zero-slope spots are called *critical points* or *stationary points*. Solve $f'(x) = 0$, and the minimiser (or maximiser) is among the solutions. This one sentence is the workhorse of applied optimisation, and we already used it twice without ceremony.
]
...
This is not an idle exercise: it is exactly the quantity a quantizer minimises when it picks a representative value for a cluster of inputs (Chapter 39), and it is the "distortion" half of every rate–distortion tradeoff (Chapter 21). Differentiate with respect to $hat(y)$. Using the rules, including the chain rule we just met, which gives the derivative of $(y_i - hat(y))^2$ as $-2(y_i - hat(y))$:
...
#fig([Gradient descent on a bowl-shaped error surface. The rings are contours of equal error (like a topographic map); the centre is the minimum. From a starting point, each step moves a little in the downhill direction $-nabla E$, scaled by the learning rate, spiralling into the valley.], cetz.canvas({
  import cetz.draw: *
...
#algo(
  ...
  complexity: "One gradient evaluation per step; number of steps depends on the error surface and the step size.",
...
Repeat until the steps stop helping (the gradient is near zero, meaning you have reached a valley). The learning rate $eta$ is the crucial dial: too small and you crawl; too large and you overshoot the valley and bounce out. Choosing it well is half the art of training a model.
]
...
One debt remains: Chapter 10 promised that the logarithm's gentle *bending* matters, and that it underlies a fact called Jensen's inequality that pins down where entropy is largest. Bending is a second-order idea, about how the *slope itself* changes, and the derivative gives it to us cleanly.
...
Let us use *optimisation* to compute something a real compressor wants: the ideal codeword lengths. Chapter 19 will prove it; here we *discover* it numerically. (Still a stand-alone illustration; the `tinyzip` Huffman coder that realises these lengths arrives in Chapter 24.)
...
#takeaways((
  [The signs $sum$ (add) and $product$ (multiply) are loops frozen into symbols; a $sum$ is a `for` loop, and $log$ turns a $product$ of probabilities into a $sum$ of logs, which is why information theory lives in log-space.],
...
#solution("11.10")[
By symmetry the budget should split evenly, $r = 50$. To confirm, differentiate $D(r) = 200(r+1)^(-1) + 200(101 - r)^(-1)$: $D'(r) = -200(r+1)^(-2) + 200(101 - r)^(-2)$ (the second term's inner derivative $d/(d r)(101 - r) = -1$ flips the sign back to positive). Set $D'(r) = 0$: $(r+1)^(-2) = (101 - r)^(-2)$, so $r + 1 = 101 - r$, giving $2r = 100$, $r = 50$. Each frame gets 50 bits, the symmetric guess, now proven. This is *bit allocation*, the core of rate control, solved by exactly the zero-slope reasoning of this chapter (Chapter 41).
]

== Further reading

- Grant Sanderson (3Blue1Brown), #link("https://www.3blue1brown.com/topics/calculus")[_Essence of Calculus_]: a free video series that builds derivatives, the chain rule, and $e$ from visual first principles; a natural companion to this chapter's pictures.
- Silvanus P. Thompson, #link("https://www.gutenberg.org/ebooks/33283")[_Calculus Made Easy_] (1910, public domain). The original "fear-not" calculus book, whose opening line ("What one fool can do, another can") captures the spirit here; free on Project Gutenberg.
- Gilbert Strang, #link("https://ocw.mit.edu/courses/res-18-001-calculus-online-textbook-spring-2005/")[_Calculus_] (MIT OpenCourseWare, free PDF). Chapters 1–4 give a rigorous but readable treatment of limits, derivatives, and optimisation.
...
#bridge[
Our purely-mathematical toolkit is now nearly complete: we can count, weigh chances, average, measure spread, sum endless lists, and walk downhill to a minimum. One shape of object kept appearing without a proper home. The *gradient* was a list of numbers we treated as a single arrow, and the "probability of a file" was begging to be a coordinate in some vast space. *Chapter 12 - Vectors, Matrices, and Linear Transformations* gives these lists a geometry: arrows you can add and stretch, dot products that measure alignment, and matrices that rotate and reshape whole spaces at once. That geometry is the language of the Discrete Cosine Transform that powers JPEG, the wavelet transforms behind JPEG 2000, and every layer of every neural network in Volume IV. After it, we will have earned every symbol Shannon is about to throw at us.
]
