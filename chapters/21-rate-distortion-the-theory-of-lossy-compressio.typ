...
Only if "better" is measured by the same $d$ the encoder optimized. A codec tuned to minimize squared error can spend its extra bits sharpening edges your eye does not care about while leaving a perceptually obvious color shift untouched. Two files of identical size, identical MSE, can look dramatically different to a person, because MSE is a _model_ of perception and a crude one at that. "More bits" buys lower distortion _under the chosen measure_, not automatically lower _perceived_ error.
]
...
- *At $D = 0$ it returns the lossless floor.* For a discrete source, demanding zero distortion forces $hat(X) = X$, and then $I(X; hat(X)) = H(X)$, exactly the entropy of Chapter 18. Rate–distortion contains lossless coding as its $D = 0$ corner. (For _continuous_ sources, $R(0) = infinity$: perfection costs infinitely many bits, because a real number carries infinite precision. This is why lossy compression is not a nicety for images and audio but a necessity.)
- *It is convex.* The curve bows toward the origin. Convexity has a concrete payoff: it means _time-sharing_ between two operating points never beats the curve. If you can hit $(D_1, R_1)$ and $(D_2, R_2)$, then by using the first scheme on a fraction of your data and the second on the rest, you can hit any point on the straight line between them. Convexity guarantees the true $R(D)$ lies on or below that line, so the curve is the genuine frontier.
...
Here is the one-line reason. Write $I(X^n; hat(X)^n) = H(X^n) - H(X^n | hat(X)^n)$. Since the $X_i$ are independent, $H(X^n) = sum_i H(X_i)$ exactly (the surprise of independent things adds; see Chapter 18). And "conditioning only reduces entropy" (Chapter 20) means that knowing the _whole_ reconstruction tells you at least as much about $X_i$ as knowing nothing, so $H(X^n | hat(X)^n) = sum_i H(X_i | hat(X)^n, X_(<i)) <= sum_i H(X_i | hat(X)_i)$. Subtracting the smaller second sum from the equal first gives the inequality. The block's leak is bounded below by its symbols' leaks because correlation in the _reconstruction_ can only help a per-symbol decoder, never hurt it.
...
#aside[
The random-coding argument is gloriously non-constructive: it proves a near-optimal codebook _exists_ without telling you a single entry of it. Shannon used the identical trick for channel capacity in 1948. For sixty years, "we know it exists but cannot build it" defined the gap between information theory and engineering, a gap that vector quantization (Chapter 39) and then trained neural codecs (Chapter 57) spent decades narrowing. The theory says where the finish line is; the rest of the book is the race to reach it.
]

#keyidea[
The rate–distortion theorem is the lossy twin of Chapter 19's source coding theorem. There, $H(X)$ was the exact floor for _zero_ distortion. Here, $R(D)$ is the exact floor for _at most $D$_ distortion. The logic is the same (converse by an information inequality, achievability by random typical codes), with one extra knob: the distortion budget $D$. Lossless coding is just the corner $D = 0$ of a much wider theory.
]
...
The Gaussian is the "most random," hardest-to-compress source of its variance: the worst case. Beat the Gaussian and you beat everything tamer.
]
...
- *A quiet coefficient* ($sigma_i^2 <= theta$, a bar fully underwater) gets distortion equal to its _entire_ variance and pays _zero bits_. It is abandoned: you spend none of your budget describing it and just output its mean (zero). High-frequency JPEG coefficients quantize to zero and vanish for precisely this reason; they are the drowned bars.
...
#tryit[
Take three coefficients with variances $sigma^2 = (4, 1, 0.25)$ and a water level $theta = 1$. Then $D = (min(1,4), min(1,1), min(1,0.25)) = (1, 1, 0.25)$, a total distortion budget of $2.25$. The rates are $1/2 log_2(4\/1) = 1$ bit, $1/2 log_2(1\/1) = 0$ bits, and $1/2 log_2(0.25\/0.25) = 0$ bits, for a total of $1$ bit per sample, all spent on the single loud coefficient. The two quiet ones are free riders, reconstructed as zero. That, in three numbers, is why energy compaction (Chapter 38) is worth so much: concentrate the variance into a few coefficients and water-filling spends bits on almost nothing.
]

== Computing R(D) for any source: the Blahut–Arimoto algorithm

The Gaussian formula is a gift, but most real sources are not Gaussian and have no closed form. For a general discrete source with a general distortion measure, $R(D)$ is the answer to a constrained minimization over all test channels. In 1972 two researchers, working independently, found a beautifully simple iterative algorithm that solves it. Suguru Arimoto attacked the dual problem of channel capacity; Richard Blahut handled the rate–distortion side. Their methods turned out to be two faces of one idea, and the field has called it the _Blahut–Arimoto algorithm_ ever since.
...
```
Run it and the printed `(R, D)` pairs land exactly on the curve we are about to compute by hand, confirming that the code and the theory agree.
]
...
#scoreboard(caption: "Fair binary source, Hamming distortion: theory vs. Blahut--Arimoto",
  [$D = 0.0$], [$R = 1.000$], [$s = infinity$], [insist on perfection: full 1 bit (lossless floor)],
...
#takeaways((
  ...
  [For many independent Gaussians, _reverse water-filling_ pours distortion to a common level $theta$: loud coefficients get the bits, drowned ones get _zero_. This is the theory behind energy compaction and zeroed JPEG coefficients.],
