...
There is a special, especially well-behaved family of uniquely decodable codes, and it turns out we never need anything else. They are the *prefix codes* (also, confusingly, called "prefix-free" codes or "instantaneous" codes — three names for one idea).
...
Why does that property help? Because it lets you decode *on the fly*, reading left to right, committing to a symbol the instant you have seen a complete codeword, never needing to look ahead or take anything back. Consider the prefix code
...
Here is a question that sounds like it should have a complicated answer but has a stunningly clean one. Suppose I tell you I want a prefix code with four symbols, and I demand the codeword lengths $1, 2, 3, 3$ bits. Can such a code exist? What about lengths $1, 1, 2, 2$? Or $2, 2, 2, 2$? Is there a simple test — just looking at the list of lengths, never mind the actual bits — that tells me whether a prefix code with those lengths is even possible?
...
#history[
Leon G. Kraft proved the prefix-code version in his 1949 MIT master's thesis, _A Device for Quantizing, Grouping, and Coding Amplitude-Modulated Pulses_ — and explicitly credited the key analysis to his classmate Raymond Redheffer. Brockway McMillan, a Bell Labs colleague of Shannon's, published the uniquely-decodable generalization in 1956, attributing the prefix special case to a 1955 remark by the probabilist Joseph L. Doob. The names stuck together as "Kraft--McMillan," a tidy example of how a clean idea accretes its discoverers.
]
...
#misconception[A good enough algorithm can compress any data below its entropy.][No uniquely decodable code can average below $H(X)$ — that is the lower bound we just proved, airtight, with no escape hatch. What clever algorithms _actually_ do is one of two things: (1) shave the leftover gap between $H$ and $H+1$ down toward zero (Huffman, arithmetic coding), or (2) discover that the _true_ entropy is far lower than a naive per-symbol count suggested, by modelling correlations between symbols. "Beating gzip" always means a better _model_ — a lower $H$ — never beating the floor of the model you share.]
...
The fix is one of the cleverest ideas in the subject, and it is pure Shannon: *stop coding one symbol at a time. Code blocks of $n$ symbols as if each block were a single super-symbol.*
...
#keyidea[
Out of the $m^n = 2^(n log_2 m)$ conceivable length-$n$ sequences, only about $2^(n H)$ are *typical* — and the rest, taken all together, almost never happen. So here is a compressor: number the typical sequences $0, 1, 2, dots$ and transmit a sequence by sending its number. A number up to $2^(n H)$ takes about $n H$ bits to write down — that is $H$ bits *per symbol*. The atypical sequences we handle with a longer fallback code, but since they occur with vanishing probability, their cost averages to nothing. Result: $approx H$ bits per symbol, the floor, reached. _Entropy is the floor because it is the exponent in the count of sequences that actually occur._
]

#fig([The space of all $2^(n log_2 m)$ length-$n$ sequences. The typical set is a vanishingly small sliver — only $approx 2^(n H)$ sequences — yet it captures essentially all the probability. Compression = give the sliver short numbers.],
cetz.canvas({
  import cetz.draw: *
  rect((0,0),(8,3.2), fill: rgb("#f4f6f8"), stroke: rgb("#d0d7de"))
  content((4,2.85), text(size:8pt, fill: rgb("#777"))[all $2^(n log_2 m)$ sequences (almost all are near-impossible)])
  // typical sliver
  rect((0.5,0.4),(3.2,1.9), fill: rgb("#cdeccd"), stroke: rgb("#0b6e4f"))
  content((1.85,1.45), text(size:8pt, fill: rgb("#0b6e4f"))[typical set])
  content((1.85,1.0), text(size:7.5pt, fill: rgb("#0b6e4f"))[$approx 2^(n H)$])
  content((1.85,0.62), text(size:7pt, fill: rgb("#0b6e4f"))[probability $approx 1$])
}))
...
#takeaways((
  ...
  [The *AEP* explains _why_: out of $m^n$ sequences only $approx 2^(n H)$ are *typical*, yet they carry almost all the probability. Numbering the typical sequences costs $approx n H$ bits — $H$ per symbol.],
...
#solution("19.2")[
(a) $H = 1\/2(1) + 1\/4(2) + 1\/8(3) + 1\/8(3) = 1.75$ bits. (b) Code `0, 10, 110, 111` (lengths $1,2,3,3$) gives $L = 1.75$. Equality is possible because every probability is a power of $1\/2$, so the ideal lengths $-log_2 p_i = 1,2,3,3$ are already whole numbers — no rounding loss.
]
