...
#recap[
  ...
  thresholds, temporal pre- and post-masking — and showed how a psychoacoustic model turns those
...
an audible click or ringing before the actual drum hit.
...
#keyidea[
  ...
  in steady state _and_ better time resolution on transients — at the same time, by switching modes.
]

== The Three New Tools: TNS, PNS, and Joint Stereo

AAC did not merely replace MP3's filterbank. It added three new compression tools that MP3 lacked
entirely.
...
spectrum — no tonal structure that the ear can track. If you replace them with a synthetic
...
#fig([The AAC profile ladder, from AAC-LC (1997) to xHE-AAC (2012). Each rung adds new tools while
  ...
  let rung(y, label, sub) = {
    ...
    content((5, y + 0.45), text(size: 9pt)[*#label* — #sub])
  }
  ...
  content((5, 5.85), text(size: 8.5pt, fill: rgb("#0b5394"))[Higher efficiency / more tools])
}))
...
transparent (indistinguishable from the original) for most listeners — something MP3 at 128
kbit/s is not.
...
USAC can switch — frame by frame, transparently — between:
...
Vorbis uses two MDCT block sizes — long (2048 samples by default) and short (256 samples) — with
...
#gomaths("Geometric mean vs arithmetic mean")[
  ...
  are, the "flatter" the set of numbers is — that is, the less variation they have. For a spectrum,
...
An AAC encoder uses exactly this kind of classification — band by band, frame by frame — to decide
...
Vorbis — and later Opus — were designed from the ground up to be unencumbered. Xiph.Org's
...
#solution("48.6")[
  ...
  the residue captures the fine structure — so the actual bits for the residue (VQ-coded) plus the
