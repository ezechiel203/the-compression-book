...
That is not a fluke, and it is not magic. It is the single most important idea in this book finally made visible. This chapter is the hinge. Behind us lie twenty-two chapters of mathematics, history, and information theory. Ahead lie the neural codecs and the LLM compressors of Volumes IV and V. The thing that connects those two halves is an equation you already have all the pieces for, the thesis the whole modern field is built on:
...
- $log_2 (a b) = log_2 a + log_2 b$ (logs turn the multiplication of independent probabilities into the _addition_ of bits, which is why total code length adds up),
...
#example[
*Betting your way to the entropy floor.* The 90/10 coin again, with a gambler who has learned the true bias, betting $Q("H") = 0.9$, $Q("T") = 0.1$. On a head (90% of rounds) wealth multiplies by $1\/0.9 approx 1.111$; on a tail by $1\/0.1 = 10$. The average log-growth per flip is $0.9 log_2 (1.111) + 0.1 log_2 (10) approx 0.9(0.152) + 0.1(3.322) = 0.137 + 0.332 = 0.469$ doublings/flip. That number is _exactly_ the entropy $H(P) = 0.469$. The gambler's optimal growth rate equals the compressor's optimal code length, to the last decimal. A gambler who mis-modelled the coin as fair would grow at only $1 - H(P,Q) = 1 - 1.0 = 0$... but that is a story for the exercises. The point stands: model error costs the gambler exactly the bits it costs the coder.
]
...
#aside[
This gambling–coding duality is due to John Kelly Jr. (1956), a colleague of Shannon's at Bell Labs, who realised that Shannon's channel capacity was also the maximum rate at which a gambler with inside information could grow money. Information theory, compression, and getting rich at the racetrack are three views of one staircase of logarithms. We will not pursue the betting thread further, but keep it in mind: whenever you see $-log_2 Q$, you may read it as "bits paid" or "log-dollars won." The choice is yours.
]
...
The trick is *adaptation*, and it is beautifully simple. Process the stream left to right. At each position, predict the next symbol using only the statistics of what you have already seen. Code that symbol with your current prediction. Update your counts. Repeat. Early on you predict badly and overpay; but every symbol teaches you a little, your model creeps toward the truth, and the per-symbol overpayment shrinks. The redundancy is the tuition you pay to _learn the source while compressing it_, and the theory says that tuition, spread over a long message, goes to zero.
...
Total: $1.00 + 0.42 + 2.58 + 0.68 = 4.68$ bits for 4 bits. We are over the raw 4 bits because the source is short and the lone `0` surprised us. On a long run of a strongly biased source, the per-bit cost converges toward the source's entropy. That convergence _is_ universality.
]
...
#keyidea[
*Learning is not free, and its price is exactly $1/2 log_2 n$ bits per parameter.* The number is tiny and grows only _logarithmically_ in the message length. Over a megabyte ($n approx 10^6$), a parameter costs about $1/2 log_2 10^6 approx 10$ bits to learn, utterly negligible next to the millions of bits in the file. This is why adaptation works so well in practice: the tuition is real but cheap, amortised over the whole stream. Remember the number $1/2 log_2 n$. It will reappear, unchanged, when Rissanen builds MDL, and again in statistics as the BIC penalty. The same number appears three times because it is the same idea three times.
]
...
This is canonical *Step 7* of the `tinyzip` build. We give `model.py` its first _learner_: an `AdaptiveBitModel` that models a stream of bits with the Krichevsky–Trofimov estimator. It starts with imaginary half-counts, predicts, then after each real bit, updates. This is the predictor that the adaptive arithmetic coder of Chapter 26 will plug in to actually emit bits; here we let it score its own cost in bits, which by the identity _is_ the cross-entropy of the model against the data. No probabilities are given to it; it learns them. Append to `tinyzip/model.py` (alongside Step 6's `entropy`):
...
The true entropy of a 90/10 source is $H = 0.469$ bits/symbol (we computed it earlier). The model, told _nothing_, spends about $0.456$ bits/symbol on this particular 10,000-bit draw, essentially _at_ the Shannon floor. This sample happened to skew slightly above 90% ones, so its empirical entropy dips a touch below $0.469$. The redundancy (the gap between what it spends and the ideal for the bias it _eventually_ learns) is only the $1/2 log_2 n approx 11$ bits the theory predicts for one parameter, a vanishing $approx 0.001$ bits/symbol. It _learned the bias for free_, near the floor. Feed it the bit-expansion of any file and you have a real, if humble, universal compressor. In Chapter 26 the very same `AdaptiveBitModel` becomes the brain of a working arithmetic coder.
]

#checkpoint[
In `AdaptiveBitModel.__init__`, why do we initialise `n0` and `n1` to `0.5` rather than `0`?
][
To avoid ever predicting probability $0$. With `n0 = n1 = 0` the first bit would have `p1 = 0/0` (undefined), and after seeing only ones, a zero would get probability $0$ and cost $-log_2 0 = infinity$ bits. That is an infinite file. The half-count is the KT smoothing that keeps every prediction strictly between $0$ and $1$.
]
...
print(ctx[1])                  # -> [1.5, 0.5]
```
The type hint `dict[int, list[float]]` reads "a dictionary whose keys are integers and whose values are lists of decimals". Dictionaries are how every context model in this book remembers "what usually follows _this_ history": order-$k$ adaptive coders, PPM and context mixing (Chapter 33), and the context-mixing champions (Chapter 34) all rely on this same structure.
]
...
cetz.canvas({
  ...
  content((8.2,-0.1))[#text(size: 9pt)[model complexity]]
  content((0.1,4.9))[#text(size: 9pt)[bits]]
...
Push MDL to its logical extreme and you reach the most beautiful object in the whole theory, and the most uncomputable. MDL asked us to pick the shortest description from some _chosen family_ of models. But why restrict to a family at all? What is the shortest description, full stop, over _every_ conceivable model, every program in every language?
...
Two threads from earlier chapters now braid together. The negative logarithm of algorithmic probability is essentially the *Kolmogorov complexity* $K(x)$ of Chapter 22. The shortest program dominates the sum, so $-log P_M(x) approx K(x)$: the length of the _shortest_ program that prints $x$. That makes $K(x)$ the true, model-free, ultimate compressed size of an object, the floor beneath all floors. Solomonoff induction is, in a precise theorem, the optimal universal predictor.
...
*The Hutter Prize.* Launched in 2006 by Marcus Hutter (since 2020 targeting `enwik9`, a 1 GB excerpt of English Wikipedia), the prize pays cash for compressing human knowledge, on the argument that compression and intelligence are the same thing. As of June 2026 the record stands at *110,793,128 bytes* (about 110.8 MB, an 8.9x ratio), set by Kaido Orav and Byron Knoll's *fx2-cmix*, accepted in October 2024. The winning techniques read like a tour of this book: context mixing (Chapter 33) carried to the extreme by the cmix lineage (Chapter 34), NLP-driven preprocessing, and embeddings to reorder articles so similar text sits together.
...
A model trained only on text, with no notion of sound or pixels, out-compressed the specialist codecs. As this chapter proves, predicting the next byte well _is_ compressing well, whatever the bytes mean.
]
...
#scoreboard(caption: "the running 100 KB English-text sample, methods so far and the ceiling this chapter reveals",
  ...
  [Kolmogorov $K(x)$], [unknowable], [n/a], [the uncomputable floor beneath all of the above],
)
...
- Solomonoff, R. J. (1964). _A Formal Theory of Inductive Inference, Parts I & II._ Information and Control 7. #link("http://world.std.com/~rjs/1964pt1.pdf")[canonical PDF]. Origin of algorithmic probability.
- Krichevsky, R. & Trofimov, V. (1981). _The Performance of Universal Encoding._ IEEE Transactions on Information Theory 27(2), 199–207. Introduces the "add one-half" estimator and its redundancy bound.
