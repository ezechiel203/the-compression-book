...
This chapter sorts out what those three words actually mean, why they are different things, and why that difference is the single most important idea before we can talk about compression at all. No formulas yet. Just the concepts, built from everyday examples. By the time we're done you will see redundancy not as wastefulness but as a handle that compression can grip.
...
- If you type that number into a computer as the text string `"37.2"`, you have *encoded* it as four ASCII characters: 51, 55, 46, 50 in decimal, totalling 32 bits.
...
- Finer quantization (more bits per sample) gives smoother values, but human ears and eyes can only tell the difference down to some limit. Beyond that, the extra bits are pure waste.
...
#definition("Information content (informal)")[
  The *information content* of a message is proportional to how surprised you were when you received it. Common events are unsurprising; they carry little information. Rare events are surprising; they carry a lot of information. A certain event (probability 1) carries *zero* information. An impossible event (probability 0) would carry infinitely much, but of course it never arrives.
]
...
Now notice: the number of bits you *need* to encode a message is related to the number of equally-likely possibilities you might receive. If your friend can only ever say one of 4 things, you need at most 2 bits to encode each message. If they can say any of $10^6$ possible things, you need about 20 bits ($2^(20) approx 10^6$). The more possibilities, the more bits. This is *exactly* why compression is possible: real messages don't use all possibilities equally, and some are far more likely than others.
...
- *Cryptography.* A one-time pad with a truly random key produces ciphertext that has zero redundancy (maximum entropy), so it looks like pure noise. That's what makes it unbreakable, and also what makes it unattractive for compression: you can't compress random data.
...
The "variable (good)" column in the figure above encodes `A` (the most common symbol) with just 1 bit, while `E` (the rarest) needs 4 bits. The average bits per symbol works out to about 2.24, a significant saving over the fixed 3 bits. The code is also *decodable*: you can read a stream of bits and always know where one symbol ends and the next begins.
...
Modern video codecs (H.264, AV1, AV2) exploit both kinds simultaneously, achieving compression ratios of 100× or more over raw video. That's the engineering that makes streaming services possible.
...
Louis Braille, blinded at age three in an accident in his father's workshop, developed the Braille writing system by 1824. Braille uses a $2 times 3$ grid of raised dots (six positions, each either raised or flat) to encode 64 possible symbols (2^6). The system is a fixed-length binary code: 6 bits per character. Its efficiency isn't its point; its point is that fingers can read it at high speed, even though the encoding is inefficient by compression standards.
...
#gopython("A Python 'histogram': counting symbol frequencies")[
  Before compressing anything, we need to know how often each symbol appears. The Python `dict` type (a dictionary, that is, a map from keys to values) is perfect for counting:
...
The model stage is where almost all the interesting competition happens. Every improvement in compression over the past 75 years has been primarily an improvement in modeling: building a better probability estimate for what comes next. The entropy coder then converts that estimate into bits, and the best possible entropy coder (arithmetic or ANS) does it nearly perfectly. So the compressor that wins is the one with the best model.
...
#bridge[
  We now know what data is, what redundancy is, and why it can be removed. Before we can talk about *how* to remove it, we need the mathematical foundation on which everything else rests: *number systems*. Computers represent all symbols (text, images, audio, video) as sequences of 0s and 1s. To understand any compression algorithm, you must be comfortable converting between binary, decimal, and hexadecimal, and understanding what a "byte" actually contains. That's Chapter 4.
]
