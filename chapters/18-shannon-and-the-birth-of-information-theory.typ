...
for some base $b$. The base only rescales the answer, like choosing inches versus centimetres. Choose base $b = 2$ and the unit is the *bit*; choose base $e$ and the unit is the *nat*; choose base 10 and it is the *hartley* (after Ralph Hartley). This book always uses base 2 — we are counting bits.
]
...
In the four-symbol example, the optimal questions are: "Is it `A`?" (yes half the time, and you are done in one question); if no, "Is it `B`?"; if no, "Is it `C`?". The number of questions is 1 for `A`, 2 for `B`, 3 for `C` or `D` — exactly the codeword lengths, and exactly $i(x)$. Average: 1.75 questions. The same number, three different costumes: average surprise, average code length, average yes/no questions. When you next see $H(X)$, hear "the price of this source in bits."
...
#algo(
  ...
  aim: "Measure, in bits per symbol, the average information of a source — and thereby the exact lower bound on its lossless compressed size.",
  ...
  superseded: "Extended, never replaced — by conditional/joint entropy, differential entropy (continuous sources), and rate–distortion R(D) for lossy coding (Chapter 21).",
)[
...
#proof[
Each probability satisfies $0 <= p(x) <= 1$. For such a $p$, the surprisal $-log_2 p(x) >= 0$ (the log of a number in $(0, 1]$ is $<= 0$, so its negative is $>= 0$). Therefore every term $p(x) dot (-log_2 p(x))$ in the sum is a non-negative number times a non-negative number — non-negative. A sum of non-negative terms is non-negative, so $H(X) >= 0$.
...
We will prove this with a small, reusable tool — *Gibbs' inequality* — which is itself one of the most useful facts in the book and reappears (as the non-negativity of the Kullback–Leibler divergence) in Chapter 20.
...
$ ln x <= x - 1, $
with equality only at $x = 1$. Why is it true? The curve $y = ln x$ and the line $y = x - 1$ touch at $x = 1$ (both equal 0 there, and both have slope 1 there, since the derivative of $ln x$ is $1\/x$, equal to 1 at $x=1$). Because $ln x$ is _concave_ (it bends downward — its slope $1\/x$ always decreases), it lies entirely _below_ its tangent line. That tangent line at $x = 1$ is exactly $y = x - 1$. Hence $ln x <= x - 1$ everywhere, touching only at $x = 1$. We will use this to compare two distributions.
]
...
#fig([The binary entropy function $H(p)$. A coin is hardest to compress when it is fair ($p = 0.5$, entropy $1$ bit) and trivially compressible when it is heavily biased ($p$ near $0$ or $1$, entropy near $0$). Predictability — bias in either direction — is what compression feeds on.],
cetz.canvas({
  import cetz.draw: *
  // axes
...
The curve tells the whole compression story in one picture. It is symmetric (a coin biased 90% heads is exactly as compressible as one biased 90% tails — bias is bias), it peaks at a flat 1 bit when $p = 0.5$, and it plunges toward 0 as the coin becomes lopsided. A coin that is 99% heads has entropy $H(0.99) approx 0.08$ bits — you could store a thousand such flips in about 80 bits, because they are almost all heads and the rare tails are the only surprises worth recording. _Bias is compressibility._
...
- *Joint entropy:* $H(X, Y) = -sum_(x, y) p(x, y) log_2 p(x, y)$ — total uncertainty of the pair.
- *Conditional entropy:* $H(Y | X) = -sum_(x, y) p(x, y) log_2 p(y | x)$ — average uncertainty remaining in $Y$ after $X$ is revealed.
]
...
#takeaways((
  ...
  [*Joint* and *conditional* entropy extend the idea to correlated symbols, and the *chain rule* $H(X, Y) = H(X) + H(Y | X)$ shows uncertainty splits stage by stage — exactly how a streaming compressor pays its bits.],
  ...
  [English carries only ~1.3 bits per character (Shannon, 1951) against 8 bits of naive storage — the redundancy that every text compressor in this book attacks.],
))
...
#solution("18.4")[
(a) $H(X,Y) = -[1/2 log_2 1/2 + 1/4 log_2 1/4 + 1/8 log_2 1/8 + 1/8 log_2 1/8] = 1/2 + 1/2 + 3/8 + 3/8 = 1.75$ bits. (b) Marginals of $X$: $p(X=0) = 1/2 + 1/8 = 5/8$, $p(X=1) = 1/4 + 1/8 = 3/8$. $H(X) = -(5/8)log_2(5/8) - (3/8)log_2(3/8) = (5/8)(0.678) + (3/8)(1.415) = 0.424 + 0.531 = 0.954$ bits. (c) Chain rule: $H(Y|X) = H(X,Y) - H(X) = 1.75 - 0.954 = 0.796$ bits. For the check, marginals of $Y$: $p(Y=0) = 1/2 + 1/8 = 5/8$, $p(Y=1) = 3/8$, so $H(Y) = 0.954$ bits as well. Indeed $H(Y|X) = 0.796 <= 0.954 = H(Y)$: knowing $X$ removed about $0.158$ bits of uncertainty about $Y$ — that is the mutual information, the correlation in bits.]
...
#solution("18.5")[
(a) One symbol, certain: $H = 0$ bits/symbol; ideal size $= 0$ bytes (a constant run). (b) 256 symbols, each appearing equally often: uniform over 256, $H = log_2 256 = 8$ bits/symbol; ideal size $= 8 times 1024 \/ 8 = 1024$ bytes — _no_ compression, it is already at the byte ceiling. (c) 16 equally likely symbols: $H = log_2 16 = 4$ bits/symbol; ideal size $= 4 times 1000 \/ 8 = 500$ bytes, exactly half, because each hex character needs only 4 of its 8 stored bits.]
...
#solution("18.8")[
Introduce a hidden selector $S$: a coin that comes up "use $p$" with probability $lambda$ and "use $r$" with probability $1 - lambda$. Let $Y$ be the symbol actually drawn — from $p$ if $S$ chose $p$, from $r$ otherwise. Then the _marginal_ distribution of $Y$ is exactly the mixture $m = lambda p + (1-lambda) r$, so $H(Y) = H(m)$. The _conditional_ entropy $H(Y | S)$ averages the entropy within each chosen source: $H(Y | S) = lambda H(p) + (1 - lambda) H(r)$. By the theorem that conditioning reduces entropy, $H(Y | S) <= H(Y)$, i.e. $lambda H(p) + (1 - lambda) H(r) <= H(m)$. This is concavity. Intuitively: revealing _which_ source produced each symbol can only reduce your average uncertainty, so hiding that selector (i.e. just mixing) leaves uncertainty at least as high as the within-source average. Mixing distinct sources blurs them together and raises entropy.]
