...
Imagine you are watching a film on your phone right now (any streaming service, any country, any device made in the last fifteen years). Odds are better than 8-in-10 that the video reaching your eyes is compressed with a standard whose first version was finished in a university conference room in Fairfax, Virginia, in May 2003. That standard is H.264/AVC, the most universally deployed video codec in history. Its 84% production-adoption figure in 2026 dwarfs every competitor. To understand why, you need to trace the forty-year lineage that made it possible, and to understand what every video codec built since has either borrowed or rebelled against.
...
#gomaths("Integer vs real-valued transforms")[
  ...
  The solution H.264 adopted (and every standard since) is an *integer transform*: a carefully chosen integer approximation of the DCT that produces bit-identical results on every conforming decoder in the world, regardless of operating system or hardware. There is a tiny quality loss versus the true DCT, but the gain in predictability is invaluable. Think of it as choosing a less elegant knife that everyone sharpens to exactly the same angle, so the dish always tastes the same.
]
...
#history[
  H.261's name refers to a coding rate of "p×64 kbit/s", where "p" in the ITU naming system stands for any positive integer. When the standard was drafted, the committee had to demonstrate that the proposed algorithm could run on hardware that would actually be affordable. The reference design was implemented partly in custom silicon because the general-purpose processors of 1988 were far too slow to decode video in real time. The idea that your phone would casually run H.264 at 4K resolution would have seemed like science fiction.
]
...
MPEG-2 used a single block size: 16×16 macroblocks for the luma motion search. This is fine for large, smooth objects but wasteful at complex boundaries. A diagonal edge through a macroblock gives a bad motion match and a large residual everywhere.
...
In MPEG-2, I-frame macroblocks were coded entirely independently, with no prediction from neighbors. H.264 added *intra prediction*: before taking the DCT of an I-frame block, the encoder predicts the block's content from the already-coded pixels in neighboring blocks (above, left, upper-left diagonal, etc.).
...
The result is a further 10–15% bitrate improvement over CAVLC, since arithmetic coding uses the full entropy while Huffman-based CAVLC leaves a small gap. CABAC is computationally more expensive than CAVLC (roughly 2–3× the entropy-coding cost), which is why it is optional: low-power devices can use CAVLC.
...
#fig(
  ...
  cetz.canvas({
    ...
    for (name, i) in boxes {
      ...
      content((x + box_w / 2, box_h / 2), anchor: "center", padding: 1pt)[#box(width: 1.5cm, align(center, text(size: 7pt)[#name]))]
      if i < boxes.len() - 1 {
        let arr_x = x + box_w
        line((arr_x, box_h / 2), (arr_x + gap, box_h / 2), mark: (end: ">", fill: rgb("#0b5394"), size: 0.15))
      }
    }
    ...
    content((fb_x_start / 2 + fb_x_end / 2, -0.82))[#box(width: 5.5cm, align(center, text(size: 7pt, fill: rgb("#9a2617"))[Reconstructed ref. (with in-loop deblocking)]))]

    // Bitstream arrow out
    let last_x = 4 * (box_w + gap) + box_w
    line((last_x, box_h / 2), (last_x + gap, box_h / 2), mark: (end: ">", fill: rgb("#0b5394"), size: 0.15))
    content((last_x + gap + 0.1, box_h / 2), anchor: "west")[#text(size: 7pt)[Bitstream]]
  })
)
...
*Extended Profile (XP)*: A hybrid for streaming that adds switching slices (for adaptive bitrate streams to switch quality mid-stream) and data-partitioning for partial error concealment. Rarely used in practice; the streaming ecosystem ended up using Baseline or High instead.
...
#checkpoint[
  ...
  The standard guarantees that any compliant bitstream can be decoded correctly by any compliant decoder: interoperability. But it says nothing about *how* the encoder searches for good motion vectors, chooses block partitions, or decides when to use CABAC vs CAVLC. Encoders are free to be as clever (or as fast, or as lazy) as they like. The result is that high-quality encoders like x264 (with weeks of engineering and careful rate-distortion optimization) produce dramatically better-compressed bitstreams than a naïve reference encoder. Both are "H.264," but one is 30%+ more efficient. This asymmetry also means codec quality improves over time without changing the standard: new encoding algorithms can produce better results that older decoders still correctly play back.
]
...
#misconception[
  ...
  H.264 is not sent to you when something better is available. But "available" requires hardware decoding (for battery life), license clarity (for legal safety), and server-side encoding capability. In 2026, 84% of all video streaming production still uses H.264, because it remains the only codec guaranteed to work on every device from a 2009 Android phone to a 2026 gaming PC. For live streaming, where encoding must happen in real time at low latency, H.264 encodes 3–30× faster than H.265 or AV1 at equivalent quality. The architecture of the internet is long; legacy devices are slow to retire.
]

== The Patent Story: How Licensing Shaped Adoption

H.264's licensing history is, counterintuitively, one of its greatest features. It is also a cautionary tale about how bad things could have been.

=== The Patent Pool Model

Video codecs rely on dozens of patents covering specific technical innovations: the way a particular interpolation filter is specified, the way context models are initialized, the way motion vectors are coded. These patents are scattered across universities, companies, and countries. Without a licensing mechanism, any company implementing H.264 would need to separately negotiate with every patent holder, potentially hundreds of individual licenses.
...
In early 2026, Via LA quietly restructured the H.264 streaming license for new licensees, meaning companies that had not already signed an agreement by the end of 2025. The new tiered structure:
...
#gopython("Measuring codec quality: PSNR")[
  ...
  Higher is better. Each +6 dB corresponds roughly to halving the noise amplitude, so +6 dB ≈ the same improvement as doubling the bitrate (roughly). This is only a rough guide, since PSNR correlates poorly with human perception for strong artifacts. (Chapter 75 covers SSIM and VMAF, which do better.)
]
...
#solution("52.2")[
  MV (−3, 5) means the prediction is taken from 3 pixels to the left and 5 pixels downward in the reference frame, so the block "moved" 3 left and 5 down between frames. Time estimate: 30 ms × 3,600 = 108,000 ms = 108 seconds per frame. At 30 fps, that is 3,240 seconds (over 54 minutes) to encode one second of video: clearly impractical. Real encoders use fast search algorithms (three-step search, diamond search, hierarchical motion estimation) to reduce this to milliseconds.
]
...
- #link("https://www.vcodex.com/h264avc-context-adaptive-variable-length-coding")[Vcodex: H.264 CAVLC Deep Dive] Iain Richardson's technical breakdowns of H.264 entropy coding.
