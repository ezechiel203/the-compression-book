...
Layer II never died. It remains the mandatory audio format in DAB digital radio (still dominant in the United Kingdom as of 2026) and was the audio track in DVD-Video alongside Dolby AC-3. Its encoder is simpler than MP3's, making it a practical choice in broadcast transmission chains where real-time encoding on modest hardware matters.

=== Layer III (MP3): The Revolution

Layer III is Layer II plus two major upgrades: a *modified discrete cosine transform* applied on top of the polyphase subbands (making the effective frequency resolution 18× finer), and a *psychoacoustic model* that can compute perceptual masking at that full MDCT resolution. The result is substantially better quality at low bitrates — Layer III can match Layer II's quality at roughly half the bitrate. That was the margin that made internet audio distribution practical.
...
Those 576 coefficients have far finer frequency resolution than the raw 32 subbands, and — crucially — they are more closely aligned with the critical-band structure of hearing. The psychoacoustic model can now assign a masking threshold to each of the 576 bins individually.
...
These two constraints can conflict — a very "difficult" passage might not be fully maskable within the available bits — and MP3 resolves the conflict with a famous *two-loop* iteration.
...
#pitfall[
  The two-loop system can fail to converge when the audio content is genuinely beyond what the bit budget allows at this bitrate. In that case, the encoder settles for some noise above the masking threshold — which is when audible "MP3 artifacts" (pre-echo, metallic ringing, loss of high-frequency "air") become perceptible, especially at bitrates below 96 kbit/s.
]
...
#gopython("Fixed vs. Adaptive Huffman Tables")[
  JPEG (Chapter 42) derives optimal Huffman tables from the image statistics and transmits them as part of the compressed file. MP3 does not — it uses predefined tables. Why? Because MP3 frames are very short (about 26 ms at 128 kbit/s), and if you had to transmit table headers per frame the overhead would be crippling. The fixed tables are a compromise: they are near-optimal for the *typical* distribution of quantized MDCT coefficients across many real audio signals, but not optimal for any one frame.
...
5. *Main data*: the Huffman-coded coefficient data — but not necessarily for *this* frame (see the bit reservoir below).
...
#keyidea[
  The bit reservoir smooths out the variable-difficulty nature of music: it lets the encoder be generous when the ear most needs it, without exceeding the average bitrate. It is equivalent to VBR behavior within the constraints of a fixed declared bitrate.
]
...
#scoreboard(
  caption: "Compression of a 3-minute stereo pop song (≈ 31.7 MB uncompressed PCM at 44.1 kHz / 16-bit stereo), illustrating the MP3 bitrate ladder.",
  [Mode], [Bitrate], [File Size], [Ratio], [Notes],
  [Uncompressed PCM], [1411 kbit/s], [31.7 MB], [1.0×], [CD-quality baseline],
  [MP3 Layer III 320 kbit/s CBR], [320 kbit/s], [7.2 MB], [4.4×], [Transparent to all listeners on most content],
  [MP3 Layer III 128 kbit/s CBR], [128 kbit/s], [2.9 MB], [11.0×], [Near-CD; occasional artifacts on difficult material],
  [MP3 Layer III 64 kbit/s CBR], [64 kbit/s], [1.4 MB], [22.1×], [Acceptable for speech; music noticeably degraded],
  [MP3 LAME VBR -V2 (~190 kbit/s avg)], [~190 kbit/s], [4.3 MB], [7.4×], [Recommended archival quality],
)
...
*2001:* Apple releases the first iPod on October 23, 2001. The marketing slogan — "1,000 songs in your pocket" — could have been "1,000 MP3s in your pocket," but Apple was wise enough to hide the technology. The iPod becomes the fastest-growing consumer electronics product in history.
...
#fig(
  ...
  cetz.canvas({
    ...
    content((3.25, 0.0))[Encoder Complexity →]
    content((0.0, 2.5), angle: 90deg)[← Quality at low bitrate]

    // Layer I
    circle((1.5, 1.2), radius: 0.55, fill: rgb("#d0e8d0"), stroke: 0.7pt)
    content((1.5, 1.2))[*L I*]
    content((1.5, 0.55))[384 kb/s]

    // Layer II
    circle((3.0, 2.5), radius: 0.55, fill: rgb("#f0e8d0"), stroke: 0.7pt)
    content((3.0, 2.5))[*L II*]
    content((3.0, 1.85))[192 kb/s]

    // Layer III
    circle((5.0, 4.0), radius: 0.55, fill: rgb("#f8d0d0"), stroke: 0.7pt)
    content((5.0, 4.0))[*L III*]
    content((5.0, 3.35))[128 kb/s]
