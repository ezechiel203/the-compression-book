...
#objectives((
  ...
  [Construct a *canonical Huffman* code so the decoder needs only the codeword *lengths*, not the whole tree; then build *length-limited* and *adaptive* (FGK, Vitter) variants.],
...
*Recurse on $\{A\}$.* One symbol. Codeword for `A` is `0`.
...
So the Huffman code is `A`=`0`, `N`=`10`, `B`=`11`, with codeword lengths $(1, 2, 2)$. Encoding `BANANA`: `11 0 10 0 10 0` = *10 bits*. Average $L = 0.5(1) + 0.33(2) + 0.17(2) = 1.5$ bits/symbol. The result is identical to Shannon–Fano here, because `BANANA` is too small and too friendly to separate them. On the five-symbol example earlier, and on real files, Huffman is the one with the optimality guarantee.
]
...
cetz.canvas({
  ...
  let leaf(p, s, w) = { rect((p.at(0)-0.42, p.at(1)-0.32), (p.at(0)+0.42, p.at(1)+0.32), fill: rgb("#e8f5ee"), stroke: rgb("#0b6e4f")); content(p)[#box(width: 0.7cm, inset: 1pt, align(center, text(size: 9pt, weight: "bold")[#s:#w]))] }
  // root weight 6
...
In some optimal prefix code, the two least-probable symbols have codewords of the *same length* and differ only in their last bit, that is, they are *sibling leaves* at the deepest level of the tree.
]
...
Both factors have opposite-or-equal sign: $p_a - p_x >= 0$ and $ell_x - ell_a <= 0$, so their product $Delta L <= 0$. The swap cannot make the code longer (and since $T$ was optimal, it stays optimal). Swap $y$ with $b$ the same way. Now $x$ and $y$ are sibling leaves at the deepest level, giving an optimal tree of the required form.
]

The sibling lemma says: there is _no loss_ in forcing the two rarest symbols to be deepest siblings. That is precisely Huffman's first move. The second lemma says that once you have committed to that move, the problem shrinks to a strictly smaller one of exactly the same kind.
...
#keyidea[
*When Huffman is fine, and when it is not.* Huffman is near-optimal when symbols carry _roughly a bit or more_ each and no symbol's probability is wildly close to $1$. It is _wasteful_ when (a) one symbol dominates ($p$ near $1$, so its ideal cost is far below 1 bit), or (b) the alphabet is tiny and skewed. The classic cures are to *block the symbols* (code pairs or triples together so fractional bits average out, Chapter 19's trick) or switch to a coder that spends fractional bits natively. The latter is *arithmetic coding* (Chapter 26) and *ANS* (Chapter 27), and the skewed-binary disaster above is exactly the case where they crush Huffman.
]
...
There is a practical problem we have been ignoring. To _decode_ a Huffman stream, the receiver needs the code, but the code was built from frequencies that only the _encoder_ saw. So the encoder must ship the codebook alongside the compressed data. A naive codebook (every symbol's full bit-pattern and length) can be large and is annoying to serialize. *Canonical Huffman coding* is the elegant fix that nearly every real format uses (DEFLATE, JPEG, PNG): it makes the codewords follow a fixed rule so the decoder can rebuild them from *the lengths alone*.
...
#keyidea[
*Building canonical codes from lengths.* (1) Sort symbols by codeword length, shortest first; break ties by the symbol's own value (e.g. byte order). (2) Give the very first symbol the all-zeros codeword of its length. (3) For each next symbol, take the previous codeword, *add 1* (as a binary number), and if this symbol's length is longer than the previous, *shift left* (append zeros) until the codeword has the right number of bits. A running counter that increments and occasionally shifts. The decoder, given the same lengths, runs the identical procedure and recovers the identical codewords.
]
...
Let us see it work on the running sample. Throughout the book we compress the same little corpus; here we use a 10,000-byte slice of English-like text whose byte entropy our Step 6 meter reports as about $4.18$ bits/byte (so the theoretical floor is $approx 5{,}225$ bytes). Canonical Huffman, paying its small integer-bit tax plus the 256-byte length header, lands close to that floor, making the first real dent in the file.
...
The naive fixes (clamp long codes and patch up the tree) produce a valid prefix code but no longer an optimal one. The exact, optimal solution is the gorgeous *package-merge algorithm* of *Lawrence Larmore* and *Daniel Hirschberg* (Journal of the ACM, July 1990). It reframes the problem as a *coin collector's problem*: you must "pay" a total of $n - 1$ using coins of denominations $2^{-1}, 2^{-2}, dots, 2^{-L_max}$ (one bit of Kraft budget at each allowed depth), each coin carrying a cost equal to a symbol's probability, and you want the cheapest way to pay. The cheapest solution assigns each symbol its optimal length-limited depth. The algorithm runs in $O(n L_max)$ time. We will not implement it here, but its existence matters: it is _why_ DEFLATE can promise a 15-bit cap and still be essentially optimal.
...
Vitter's $Lambda$ algorithm keeps the tree's leaves numbered so that, among nodes of equal weight, the leaves come before the internal nodes. This extra invariant bounds the per-symbol overhead to under one bit above the running static optimum.
]
...
#exercise("24.6", 2)[
Prove the upper half of Huffman's gap bound, $L < H + 1$, for _any_ source. Do it for the Shannon code that assigns symbol $s$ the length $ell_s = ceil(-log_2 p_s)$; Huffman, being optimal, can only do better.
]
#solution("24.6")[
...
- #link("https://people.math.harvard.edu/~ctm/home/text/others/shannon/entropy/entropy.pdf")[Shannon, C. E. (1948). _A Mathematical Theory of Communication._ Bell System Technical Journal 27.] Section 9 sketches the Shannon–Fano construction and the entropy bound.
