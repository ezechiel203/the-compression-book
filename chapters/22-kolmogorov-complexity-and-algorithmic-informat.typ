...
There is just one catch, and it is a doozy: this perfect compressor _cannot be built_. Not "is hard to build," not "we haven't figured it out yet" — it is _provably impossible_, forever, for fundamental logical reasons twinned with Gödel's incompleteness theorem and the unsolvability of the halting problem. This chapter is the story of that magnificent, unreachable ideal: what it is, why it is exactly the right definition, why it is uncomputable, and why — even though we can never touch it — it quietly governs everything every real compressor does.
...
cetz.canvas({
  ...
  content((1.3,0.35))[#text(size:8pt)[$p^*$ on $V$, length $K_V(x)$]]
  // arrow
  line((2.9,0.35),(3.9,0.35), mark: (end: ">"))
  // U-program = interpreter + p*
  rect((4.2,0),(5.5,0.7), fill: rgb("#fff3e0"), stroke: rgb("#783f04"))
  content((4.85,0.35))[#text(size:8pt)[$I_V$ ($c$)]]
  rect((5.5,0),(8.1,0.7), fill: rgb("#eef4fb"), stroke: rgb("#0b5394"))
  content((6.8,0.35))[#text(size:8pt)[$p^*$, length $K_V(x)$]]
  content((6.15,-0.5))[#text(size:8pt)[program on $U$, length $K_V(x)+c$]]
}))
...
#aside[
Chaitin pushed further to his famous constant *$Omega$* (read "omega"), the *halting probability*: the probability that a program whose bits are chosen by coin flips eventually halts. $Omega$ is a single, perfectly definite real number between $0$ and $1$ — and it is *uncomputable* and *algorithmically random*: its binary digits have no pattern, no formula, no shortcut. Knowing the first $n$ digits of $Omega$ would let you solve the halting problem for all programs up to $n$ bits and settle famous open conjectures. $Omega$ is, in a sense, all of mathematics' hardest yes/no questions compressed into the bits of one number that no one can ever fully write down. It is the most concentrated incompressible object we know how to point at.
]
...
- #link("https://arxiv.org/abs/0712.3329")[Legg, S. & Hutter, M. (2007). _Universal Intelligence: A Definition of Machine Intelligence_.] — the formal bet, behind the Hutter Prize, that compression and intelligence are one.
