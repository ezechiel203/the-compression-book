...
#proof[
  By the definition, $b^m$ is $m$ copies of $b$ multiplied together, and $b^n$ is $n$ copies. Writing the first group and then the second group, all multiplied, gives a single run of $m + n$ copies of $b$. A run of $m + n$ copies of $b$ is exactly what $b^(m+n)$ means. Multiplication is associative, so the grouping does not matter; only the *total count* of factors does. Hence $b^m times b^n = b^(m+n)$. #h(1fr)
]
...
#definition("Logarithm")[
  ...
  $ b^y = x. $
  Equivalently: $log_b x$ and $b^y$ are inverse questions, with $log_b (b^y) = y$ and $b^(log_b x) = x$. They perfectly undo each other.
]
...
#gomaths("Reading log notation, and the three bases that matter")[
  ...
  In this book, *whenever the base is unwritten, assume base 2*, because we are almost always counting bits. A worked read: $log_2 64$ means "two to the what is $64$?"; since $2^6 = 64$, the answer is $6$. And $log_10 10000$ means "ten to the what is $10000$?"; that is $4$ (one less than the number of digits in $10000$). The common log of a number is, give or take, *how many digits it has*. That is not a coincidence; it is the whole point of logarithms, and we prove the digit-counting connection below.
  ]
...
So far, "equally likely." But real data is *lopsided*: the letter "e" is common, "q" is rare. The leap Shannon made (the leap this chapter has secretly been preparing you for the whole time) is to handle unequal probabilities. Suppose an event has probability $p$. If it were one of $N$ equally likely outcomes, its probability would be $p = 1/N$, so $N = 1/p$, and its information content would be $log_2 N = log_2 (1/p)$. By the quotient law, $log_2(1/p) = log_2 1 - log_2 p = 0 - log_2 p = -log_2 p$. There it is.
...
#pitfall[
  `math.log2(0)` does not return a number: the logarithm of zero is undefined (there is no power of $2$ that gives $0$; the curve plunges toward minus-infinity but never arrives). In compression this matters: a symbol that *never occurs* has probability $0$ and infinite surprise, but it also never appears in the message, so it contributes nothing. Code must *guard the zero*; our `info_bits` rejects $p <= 0$ outright. Entropy code that sums $p log_2 p$ must treat the $p = 0$ term as $0$ (a convention justified because $p log_2 p -> 0$ as $p -> 0$, which the gentle calculus of Chapter 11 will confirm).
]
...
#takeaways((
  [An *exponent* counts how many copies of a base are multiplied: $b^n$ is $n$ copies of $b$. The exponent is tiny while $b^n$ is huge, and that lopsidedness is the whole point.],
