...
Because the signal barely changes between samples, $e_n$ is almost always close to zero. A histogram of $e_n$ peaks sharply at zero and falls off quickly — much less spread than the original signal. Less spread means lower entropy, which means fewer bits for the entropy coder.

#keyidea[
  Prediction does not remove information; it redistributes it. We replace a *flat* distribution of large values with a *peaked* distribution of small residuals. The entropy coder downstream (Huffman, arithmetic, ANS — all from earlier chapters) then has an easy job.
]
...
#gomaths("Toeplitz matrices — when every diagonal is constant")[
  ...
  Read it off: the main diagonal is all $d_0$; the diagonal just above it is all $d_1$; the one just below is also $d_1$ (so the matrix is *symmetric* here too). The matrix $bold(R)$ of autocorrelations is exactly this shape because its entry in row $j$, column $k$ is $R(|j - k|)$ — it depends only on the gap $|j - k|$ between the two lags.

  Why care? An ordinary $p times p$ matrix holds $p^2$ independent numbers, and solving a system with it costs $O(p^3)$ work. A Toeplitz matrix is secretly described by just the $p$ numbers $d_0, d_1, dots.c, d_(p-1)$ — far less information — and that hidden simplicity is precisely what the Levinson–Durbin algorithm exploits to solve the system in $O(p^2)$ time instead of $O(p^3)$. Whenever you see "Toeplitz", read it as "the system has repeating structure we can shortcut".
]
...
#gopython("Matrix and list indexing in Python")[
  ...
  The slice notation `list[start:stop:step]` was introduced in Chapter 16. Here `[::-1]` reverses the list — useful in the Levinson–Durbin reflection-coefficient update.
]
...
The JPEG-LS standard (ISO 14495-1, 1999) takes this further with the *LOCO-I* (LOw COmplexity LOssless Compression for Images) predictor. LOCO-I selects the predictor adaptively based on the context, and crucially, it handles edges well.
...
#algo(
  ...
  The first two cases handle edges (vertical or horizontal gradient), where the simple average $A + B - C$ would overshoot. In smooth regions ($A approx B approx C$), $A + B - C approx A approx B$ — the average extrapolation works fine. This cleverness comes for free: no coefficients to estimate, no matrix to solve.
]
...
Reducing $sigma_e$ by a factor of 2 reduces the theoretical bit-rate by 1 bit per sample. Every 6 dB improvement in prediction gain (halving $sigma_e$) saves 1 bit per sample — the "6 dB per bit" rule we saw in Chapter 21's rate-distortion theory for Gaussian sources.

#mathrecall[
  A *decibel* (dB) is just a logarithmic unit for ratios, introduced in Chapter 21 and Chapter 39: for a power ratio, $"value in dB" = 10 log_10("ratio")$. A factor of $4$ in power is $10 log_10 4 approx 6$ dB. Halving the residual amplitude $sigma_e$ quarters the residual *power* $sigma_e^2$, hence the $6$ dB — and from $R(D) = 1/2 log_2(sigma^2 \/ D)$ that quartering is exactly one extra bit.
]
...
#takeaways((
  ...
  "Prediction is not a replacement for entropy coding — it is preparation for it. The entropy coder (Huffman, arithmetic, ANS) then compresses the now-small residuals. Prediction and entropy coding always work together.",
))
...
#solution("40.1")[
  *Encoder trace* (reconstructed history starts at 0):
  ...
  - $n=4$: $x_4 = 105$, pred = 104. Residual = 1. Nearest multiple of 2: $hat(e) = 0$ or 2. $|0-1|=1$, $|2-1|=1$ — tie; say round half-up: $hat(e) = 2$. Send 2. $hat(x)_4 = 104+2 = 106$.
...
#solution("40.4")[
  *Slope overload* occurs when the signal changes faster than $Delta dot f_s$ per second — the staircase cannot keep up, and the error grows monotonically in one direction rather than oscillating. This distortion is most audible as a "smearing" of fast transients.
