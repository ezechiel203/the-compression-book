...
#recap[
  We have spent thirteen chapters assembling the raw materials. In Chapter 4 we learned that $n$ bits name exactly $2^n$ things, and met the *byte* (eight bits, $256$ values). Chapter 6 gave us *sets* (collections of distinct things), *functions* (rules turning each input into exactly one output), and *bijections* - perfect one-to-one pairings, which a lossless compressor must be. Chapter 7 made *exponents* and *logarithms* second nature, and showed why halving a search space $n$ times takes about $log_2 n$ steps (the phone-book trick, named). Chapter 8 taught us to *count possibilities* and proved the no-magic-box theorem by counting. Chapter 11 introduced $sum$ (sigma) summation notation and the idea of a function's _growth rate_. And Chapter 13 showed how real data becomes *bytes*: bits, integers, ASCII, the actual material our structures will hold. This chapter spends all of it: complexity _is_ growth rate from Chapter 11 wearing work-clothes; the structures _are_ sets and functions made concrete; and the searching and sorting we do _are_ the logarithms of Chapter 7 in action.
]
...
But "about $n$ steps" and "about $log_2 n$ steps" are still a little vague. Do we count the `+1`, the comparison, the assignment? It turns out we should _not_ sweat those details, and there is a tidy notation that tells us exactly which details to ignore and which to keep. It is called Big-O, and it is the single most useful piece of vocabulary a programmer owns.
...
#fig([The growth rates that matter, with steps for $n = 1{,}000{,}000$. The jump from $O(n log n)$ to $O(n^2)$ is the cliff that separates "runs instantly" from "go get coffee."],
  table(columns: (auto, auto, auto, 1fr), inset: 5pt, align: (left, left, right, left),
    fill: (_, row) => if row == 0 { c-accent.lighten(85%) } else { none },
...
#fig([An array of bytes. Each cell is one slot; the index below it is its position. Reaching any cell is one arithmetic step ($O(1)$) because the address is computed, not searched.],
  cetz.canvas({
    ...
    content((3.2, 1.4))[#box(width: 5.5cm, align(center, text(size: 8pt)[address = base + i × slotsize]))]
  }))
...
Where the array is weak (expensive insertion), the *linked list* is strong. Instead of one contiguous block, a linked list scatters its elements anywhere in memory and threads them together with pointers: each *node* holds a value _and_ the address of the next node, like a paper chase where each clue tells you where the next clue is hidden. To insert a new node, you do not shuffle anything; you re-thread two pointers. The previous node now points at the newcomer, and the newcomer points at what used to come next. That is $O(1)$ insertion, anywhere, once you are standing at the spot.
...
So far we look things up either by _position_ (arrays, $O(1)$) or by _walking_ (linked lists, $O(n)$). But often we want to look something up by a _key_ that is not a position: "has the word `the` appeared before?", "what code did I assign to this three-byte sequence?" The structure that does this in $O(1)$ _on average_ is the *hash table*, and it is one of the most important inventions in computing.

The idea is built on a humble function. A *hash function* takes a key of any kind (a string, a number, a chunk of bytes) and scrambles it into a number in a fixed range, say $0$ to $m - 1$. We keep an array of $m$ buckets, and we store each key in the bucket its hash function points to. To look a key up later, we hash it again. The _same_ key always hashes to the _same_ bucket, so we look only in that one bucket. We never search the whole table; the hash function _computes_ where to look, just as the array index did. That is the $O(1)$.
...
Hash tables are woven through compression. LZ77 and DEFLATE (Chapters 28, 30) find repeated strings by hashing short byte-sequences and keeping, in a hash table, a list of every place each sequence was last seen. That turns "have I seen these three bytes before, and where?" into an $O(1)$ question. LZW (Chapter 29) stores its growing dictionary of byte-strings in a hash table. Without hash tables, none of these would run at practical speed.
...
#misconception[Hash tables are always $O(1)$, so they are always the fastest choice.][
  The $O(1)$ is an _average_ over good hash behaviour, not a guarantee. A bad hash function, or an adversary who deliberately feeds colliding keys, can drive a hash table to $O(n)$ per operation, which is a real denial-of-service technique. The constant factor hidden in that $O(1)$ - computing the hash, chasing a pointer to the bucket - can also make a hash table _slower_ than a plain array scan for small collections. Hash tables shine when the key set is large and the keys are not simple positions; for a 256-entry byte table, the humble array wins every time.
]
...
#fig([A small *binary tree* (each node has at most two children). The root is on top; leaves are shaded. The path from the root to any leaf spells out a sequence of left/right turns, which is the idea behind every prefix code.],
  cetz.canvas({
    import cetz.draw: *
...
A *binary heap* is a binary tree with one simple rule, the *heap property*: every parent is smaller than or equal to its children (for a "min-heap"). The smallest item is therefore always sitting at the root, free to grab in $O(1)$. When you remove it, you patch the hole by moving the last leaf up to the root and letting it "sink" - repeatedly swapping with its smaller child until the heap property holds again, which takes at most the tree's height ($O(log n)$) steps. Inserting works in reverse: drop the new item at the bottom and let it "bubble up" past any larger parents, again $O(log n)$. A heap needs no pointers at all; it packs the tree into a plain array, with the children of slot $i$ living at slots $2i+1$ and $2i+2$. It is a tree wearing an array's body.

#fig([A min-heap: every parent ≤ its children, so the minimum (1) is always at the root, removable in $O(1)$. Insert and remove cost $O(log n)$ (the height of the tree). Huffman repeatedly removes the two smallest frequencies and inserts their sum.],
  cetz.canvas({
    import cetz.draw: *
...
The breakthrough idea is *divide and conquer*: split the list in half, sort each half recursively (the same method applied to a smaller problem), then _merge_ the two sorted halves into one.

#gopython("Recursion: a procedure that calls itself")[
  A procedure is *recursive* when, to solve a problem, it calls _itself_ on a smaller version of the same problem. It sounds circular, but it is not: each call works on _less_ data and there is always a _base case_ small enough to answer outright, so the chain of calls is finite and must end. Factorials are the classic illustration: $n! = n times (n-1)!$ (Chapter 8), with the base case $0! = 1$.
...
#fig([Merge sort on `[3,1,2]` versus selection sort, on a list of $n$. Merge sort's $log n$ levels of $O(n)$ merging give $O(n log n)$; selection sort's $n$ scans of $O(n)$ give $O(n^2)$. For $n = 10^6$ that is ~20 million steps versus a trillion.],
  table(columns: (1fr, auto, 1fr), inset: 5pt, align: (left, right, left),
    fill: (_, row) => if row == 0 { c-accent.lighten(85%) } else { none },
...
You will see this bargain made explicit again and again. LZ77's window size (Chapter 28) trades memory for ratio. Arithmetic coding (Chapter 26) trades a little speed for hitting the entropy floor more tightly than Huffman. ANS (Chapter 27) was celebrated precisely because it broke an _apparent_ trade-off, delivering arithmetic-coding ratios at Huffman-like speed. The giant neural compressors of Volume IV trade staggering amounts of computation for the best ratios known, turning the time-space dial to one extreme. Every one of these is the same bargain, written large.

#aside[
  Python 3.14, released on 7 October 2025, even ships a brand-new `compression.zstd` module in its standard library: the Zstandard codec (Chapter 32) built right in. We will not use it for `tinyzip` (the whole point is to build our own), but it is a sign of how central these algorithms have become. A compression algorithm that was research in 2016 is now a batteries-included part of the language. The same release also makes `bytes`-and-`int` heavy code faster, which our hand-built codecs will quietly benefit from.
]
...
#fig([The data-structure toolbox, by cost of the core operations and by where each one resurfaces in this book. "avg" marks an average-case bound that can degrade in the worst case.],
  table(columns: (1fr, auto, auto, auto, 1fr), inset: 5pt, align: (left, center, center, center, left),
    fill: (_, row) => if row == 0 { c-accent.lighten(85%) } else { none },
    ...
    [Stack], [n/a], [$O(1)$], [n/a], [Decoder bookkeeping; BWT inverse (35).],
    [Queue / `deque`], [n/a], [$O(1)$], [n/a], [LZ77 sliding window (28).],
    [Hash table / `dict`], [$O(1)$ avg], [$O(1)$ avg], [n/a], [LZ/LZW match finding (28–30).],
    [Binary tree], [$O(log n)$], [$O(log n)$], [n/a], [Huffman/prefix codes (24); quadtrees (53).],
    [Heap / `heapq`], [n/a], [$O(log n)$], [$O(log n)$], [Building Huffman codes (24).],
  ))
...
#solution("14.1")[
  Slowest to fastest growing: $O(1) < O(log n) < O(n) < O(n log n) < O(n^2) < O(2^n)$. You most _want_ $O(1)$ (constant: the cost never grows with input size). You most _fear_ $O(2^n)$ (exponential: adding one item _doubles_ the work, so even modest inputs become uncomputable). $O(1)$ is the _slowest-growing_ and therefore the _best_; "slow growth" is good when we are talking about cost.
]
...
#solution("14.3")[
  Here $n = 10^7$. Algorithm A: about $10^7$ operations $div 10^9$ per second $= 0.01$ s, a hundredth of a second. Algorithm B: about $(10^7)^2 = 10^14$ operations $div 10^9 = 10^5$ seconds $approx 27.8$ hours. A naïve "compare all pairs" match finder would take over a day on a 10 MB file. This is why LZ77 implementations use _hash tables_ (and hash chains) to jump straight to plausible matches, keeping the search close to $O(n)$ instead of $O(n^2)$. The data structure is what makes the codec usable.
]
...
#solution("14.6")[
  After inserting all five, the minimum, $1$, sits at the root; a valid min-heap is: root $1$, its children $3$ and $9$, and under $3$ the leaves $7$ and $5$ (one valid shape; heaps are not unique, only the parent-≤-children property must hold, and $1$ must be at the root). First remove-min returns $1$; the heap re-settles with $3$ at the root (children $5$ and $9$, leaf $7$). Second remove-min returns $3$. Each insertion and each remove-min is $O(log n)$: here $log_2 5 approx 2.3$, so two or three swaps at most per operation. Finding the minimum (reading the root) is $O(1)$.
]
