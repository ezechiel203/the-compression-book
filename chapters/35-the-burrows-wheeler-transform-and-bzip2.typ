...
Here is a magic trick. Take the word `banana`. Scramble its letters — but
scramble them in a very particular, reversible way — and you get `nnbaaa`.
Look at that output: the three `a`s now sit together, the two `n`s sit
together. The original `banana` alternated letters; the scrambled version
_clusters_ them. And here is the part that should feel impossible: from
`nnbaaa` alone — three `a`s, two `n`s, one `b`, in exactly that order — plus
a single small number, you can recover `banana` _exactly_, letter for letter,
position for position. No information was lost. The scramble is a perfect
round-trip.
...
it all — *bzip2*. Then we will watch the same idea escape compression
...
#gomaths("Why a run costs about $log_2 k$ bits")[
  Recall from Chapter 7 that $log_2 k$ answers "2 to what power equals $k$?" —
  it is the number of bits needed to write $k$ as a binary number. A run of
...
to bottom, is the BWT output. The original `banana` lands in row 3 — that index
...
that rotation's starting context — because a rotation wraps around, the last
...
#keyidea[
  ...
  cannot exploit into a form it can. The sort is the whole magic: it is a
...
index `3`. The sorted matrix is gone — we never stored it. From a single
...
spells `banana`.],
  align(center, table(columns: (auto, auto, auto, auto), inset: 6pt,
...
`b a n a n a` — `banana`, recovered exactly from six clustered letters and the
...
5. *Huffman coding*, with a twist: bzip2 builds _up to six_ different Huffman
   ...
   codes each segment best — a cheap form of adaptivity that squeezes out a few
   extra percent.
...
right-to-left, inverting each.],
  cetz.canvas({
    ...
    for (i, lab) in labels.enumerate() {
      ...
      content((x + 0.75, 0.35), text(size: 8pt)[#lab])
      if i < labels.len() - 1 {
...
#gopython("List comprehensions and `sorted` with a key")[
  ...
  (slice from $i$ to the end, then the start up to $i$ — Chapter 16's slicing).
...
and — using _backward search_, a repeated application of the LF-mapping — it can
...
#aside[
  ...
  carefully rewritten for memory safety is a quiet testament: the
  block-sorting idea is too useful to retire.
]
