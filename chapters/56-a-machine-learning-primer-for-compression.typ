...
cetz.canvas({
  ...
  let bx(x, y, w, t, c) = {
    ...
    content((x+w/2, y+0.45))[#box(width: calc.max(w - 0.4, 0.6) * 1cm, inset: 1pt, align(center, text(size:7.5pt)[#t]))]
  }
...
That is the entire neuron. The weights say how much each input matters (a big positive weight means "this input pushes the output up," a negative weight "pushes it down," a near-zero weight "ignore this input"). The bias shifts the whole thing up or down. We meet three squashing functions $f$ in a moment; each is the one nonlinear flourish that gives a neuron its power.
...
Why the squash? If we left $f$ out (if a neuron were just $bold(w) dot bold(x) + b$) then stacking neurons would buy us nothing. A linear function of a linear function is still linear (Chapter 12): a thousand layers without a squash collapse into a single matrix, able to draw only straight lines and flat planes. Real data is gloriously *curved*: the boundary between "cat" and "dog," the texture of grass, the statistics of a photo. The nonlinear squash is what lets stacked neurons bend. Three squashes you will meet everywhere:
...
#keyidea[
A neuron is *weighted sum → add bias → squash*. A layer is a *matrix multiply → add bias vector → squash, applied elementwise*. A network is *layers stacked one after another*. There is no other machinery. The intelligence is entirely in the *values* of the weights. Those values are not designed; they are *learned*.
]
...
Picture the network's total error as a *landscape* showing how badly it does on the training data. Every possible setting of the weights is a point on the ground; the *height* at that point is the error. A perfect network sits at the bottom of a valley. We start somewhere random (high up, error large) and want to reach a valley floor. The trick comes straight from Chapter 11: at any point, the *slope* tells you which way is downhill. Take a small step in the steepest downhill direction. Repeat. This is *gradient descent*, the engine of all learning.
...
cetz.canvas({
  ...
  let lay(y, t) = {
    ...
    content((1.1,y+0.35))[#box(width: 1.8cm, inset: 1pt, align(center, text(size:7.5pt)[#t]))]
  }
  lay(0, [input $bold(x)$])
  lay(1.0, [layer 1])
  lay(2.0, [layer 2])
...
#algo(
  ...
  superseded: "Refined, not replaced, by SGD, momentum, RMSProp, and Adam; all follow the same downhill idea.",
)[
...
#fig([An autoencoder. The encoder funnels the input down to a small latent bottleneck $bold(y)$; the decoder funnels it back up to a reconstruction $hat(bold(x))$. The narrow middle forces the network to keep only what matters: that constraint is compression.],
cetz.canvas({
  import cetz.draw: *
...
Read it slowly. The first term $R$ is the codelength of the quantized latent under the learned model, the *bitrate*. The second term $D$ is the reconstruction distortion, usually MSE. The number $lambda$ (Greek "lambda") is a *Lagrange multiplier*: it sets the exchange rate between bits and quality. Crank $lambda$ up and the optimizer cares mostly about quality, spending bits freely (a big, sharp file); crank it down and it cares mostly about size, tolerating blur (a tiny file). Sweep $lambda$ and you trace out the whole rate–distortion curve of Chapter 21. The difference is that the points on that curve are now achieved by a *trained network*, not a hand-built codec.
...
#algo(
  ...
  weaknesses: "Serial decode is slow; it is the field's central wound for image/video codecs.",
...
The newest and now-dominant family generates by *reversing a destruction process*. Take a clean image and add a tiny bit of random noise; repeat hundreds of times until nothing is left but pure static. That *forward* process is trivial. The magic is training a network to run it *backward*: take noisy static and, step by step, *denoise* it back into a clean image, each step removing a little noise and adding a little structure. Once trained, you start from fresh random static and let the network sculpt an image out of it. Because each denoising step is a small, learnable nudge, diffusion models are remarkably stable to train (unlike GANs) and produce the sharpest, most realistic samples yet.
...
#misconception[A generative codec "remembers" the original image perfectly and just stores it cleverly.][A low-bitrate generative codec *reconstructs a plausible image*, not the true one. It transmits a compact description and lets a generative prior *invent* detail that is statistically consistent but possibly never present in the original. This is a real hazard for medical, legal, or forensic images, where an invented-but-plausible detail is worse than honest blur. We weigh this trade-off carefully in Chapter 58.]
...
cetz.canvas({
  ...
  let card(x, y, t, s) = {
    ...
    content((x+1.5,y+0.7))[#box(width: 2.6cm, inset: 1pt, align(center, text(size:7.5pt, weight:"bold")[#t]))]
    content((x+1.5,y+0.28))[#box(width: 2.6cm, inset: 1pt, align(center, text(size:6.5pt, fill:c-ink.lighten(15%))[#s]))]
  }
...
```
The `@` operator is matrix multiplication: row $i$ of `W` is dotted with `x`, giving neuron $i$'s weighted sum. Add the bias vector `b` and you have the pre-squash output of a whole layer in one line, which is the `f(W x + b)` from the maths box, minus the squash itself.
]

#gopython("Defining functions with def, and type hints")[
We have used Python functions since Chapter 16. A quick refresh: `def name(arg: type) -> rettype:` declares a function; the `: type` and `-> rettype` are *type hints* (Chapter 15), documentation the reader (and tools) can check, which is ignored at run time. NumPy arrays are typed `np.ndarray`.
```python
...
TINYZIP's Phase D opens with a *concept-level* learned transform. We add a tiny module that *trains* a 2-pixel-wide autoencoder bottleneck on a stream of correlated pairs and shows it discovering, by gradient descent alone, the same "keep the average, drop the difference" structure a hand-built transform would impose. Watch the bottleneck *throw bits away*. This is a teaching sketch, not a production codec; the real Ballé architecture arrives in Chapter 57. We keep the names ready for `tinyzip/llmzip.py` (the neural module Chapter 62 completes).
...
#takeaways((
  ...
  [Learned compression minimizes the *rate–distortion loss* $R + lambda D$, provably the same as the *variational-autoencoder* objective, so compression is variational inference and every generative advance is a potential compression advance.],
  ...
  [By 2025 a learned codec (JPEG AI, ISO/IEC 6048-1:2025) became the first learning-based *international standard*, beating VVC. Learned codecs still lag on speed, hardware, and reproducibility.],
))
...
- Yibo Yang, Stephan Mandt, Lucas Theis (2023), _An Introduction to Neural Data Compression_: #link("https://arxiv.org/abs/2202.06533")[arXiv:2202.06533]. A good survey to read next.
...
#bridge[
We now own the entire vocabulary: neurons, gradient descent, the autoencoder, the rate–distortion loss, and the four generative families. Chapter 57 spends all of it at once. We build the *real* learned image codec: the Ballé line (2016–2018), with its end-to-end rate–distortion training, the GDN nonlinearity, quantization-as-noise made rigorous, and the *hyperprior*. The hyperprior is a second little autoencoder that transmits a description of the entropy model itself, letting the codec adapt its bit-allocation pixel by pixel. That is the architecture that overtook JPEG, BPG, and AVIF, and became JPEG AI. Bring the bottleneck; we are about to make it pay for its bits.
]
