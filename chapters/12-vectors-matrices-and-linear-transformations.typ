...
Here is a small magic trick. Take eight numbers (the brightnesses of eight pixels in a row across a photograph, which because the photo is smooth will be eight numbers that barely differ):
...
#objectives((
  [Read a *vector* two ways at once: as an ordered list of numbers and as an arrow in space. Add, scale, and measure them.],
...
Keeping both views in mind simultaneously is the whole reason vectors are powerful. The *list* view lets a computer store and crunch a vector as plain numbers in memory. The *arrow* view lets us reason with geometry: lengths, directions, angles, "this points the same way as that." Compression lives in the gap between the two. A block of pixels is just a list to the machine, but when we ask "is this block smooth or busy?" we are really asking a geometric question about the arrow, and the transform we apply is a geometric move.
...
Why does this matter? Because the dot product measures *alignment*. Here is the beautiful fact, which we will use without exhausting its depths: for any two vectors,
$ bold(u) dot bold(v) = norm(bold(u)) thin norm(bold(v)) thin cos theta, $
where $norm(bold(u))$ is the length of $bold(u)$ (defined in a moment), and $theta$ is the angle between the two arrows. We will not prove this formula from scratch (it follows from the law of cosines, geometry from before this book) but its *consequences* matter:
...
That word *energy* is not loose metaphor; it is the literal physical energy of a sound wave or the total squared brightness of an image patch, and it is the quantity transforms are built to concentrate. Remember our eight pixels: before the transform the energy was spread across all eight numbers; after, almost all of it sat in the first. We will soon *prove* that a good transform keeps the total energy unchanged while moving it around. That is the entire game.

*Distance.* The distance between two vectors is the length of the arrow between them: $"dist"(bold(u), bold(v)) = norm(bold(u) - bold(v))$. Subtract slot by slot, square, add, root. This is how a codec measures *distortion*: how far the reconstructed signal $hat(bold(x))$ strayed from the original $bold(x)$. The familiar "mean squared error" (MSE) you will meet in Volume III is exactly $norm(bold(x) - hat(bold(x)))^2$ divided by the number of components. Distortion is a distance between vectors.

*Orthogonality.* Two vectors are *orthogonal* (perpendicular) exactly when their dot product is zero. We just saw $(1,0)$ and $(0,1)$ are orthogonal, and so are $(2,1)$ and $(-1,2)$. Orthogonality is the property that makes coordinates *clean*: if your reference directions are mutually orthogonal, then the amount of "stuff" along one direction tells you nothing about the amount along another. They do not interfere, which is precisely what lets a codec store the eight transformed numbers independently and discard some without disturbing the rest.
...
The rule: to multiply $A$ (size $m times n$) by $B$ (size $n times p$), the entry in row $i$, column $j$ of the product $A B$ is *the dot product of row $i$ of $A$ with column $j$ of $B$*. Again, dot products all the way down. The inner sizes must match ($A$ has $n$ columns, $B$ has $n$ rows) and the result is $m times p$.
...
#gomaths("Span, linear independence, and why a basis needs exactly n vectors")[
  Two notions make "basis" precise. The *span* of a set of vectors is everything you can build from them by scaling and adding: all their linear combinations. The arrows $bold(e)_1, bold(e)_2$ span the whole plane: any point is some amount right plus some amount up. A set is *linearly independent* if none of its members is a redundant combination of the others; each adds a genuinely new direction. A *basis* is a set that is both: it *spans* (reaches everything) and is *independent* (wastes nothing). In $RR^n$ that forces the count to be exactly $n$: fewer than $n$ independent vectors cannot span (some directions stay unreachable); more than $n$ cannot stay independent (one will be redundant). This $n$ is the *dimension*, which is why $8$ pixels need exactly $8$ basis patterns to describe them with no loss, the precise number the DCT provides.
  ]

How do you actually *find* the new coordinates? For the friendliest kind of basis (one whose vectors are mutually perpendicular and of unit length, which we meet by name in the next section) it is just a pile of dot products, and it is worth doing once by hand. Take the two perpendicular unit arrows $bold(b)_1 = (1\/sqrt(2),thin 1\/sqrt(2))$ (pointing up-and-right at $45 degree$) and $bold(b)_2 = (-1\/sqrt(2),thin 1\/sqrt(2))$ (up-and-left), and the point $bold(x) = (4, 2)$. The coordinate of $bold(x)$ along $bold(b)_1$ is "how much of $bold(x)$ lies along $bold(b)_1$," which is exactly the dot product:
...
So the same point that the standard axes call $(4, 2)$, the rotated axes call $(4.24, -1.41)$: one point, two vocabularies. The punchline that ties the whole chapter together: *stacking the basis vectors as the rows of a matrix and multiplying by $bold(x)$ produces all the new coordinates at once.* Change of basis *is* a matrix--vector product; a transform is nothing but the matrix whose rows are the basis you chose. That thought carries us into the next section, where that matrix earns the name *orthogonal* and a remarkable property comes with it.

== Orthogonal matrices: the rotations that conserve energy

Not all bases are equally pleasant. The friendliest are the *orthonormal* ones: bases whose vectors are mutually orthogonal (every pair has dot product zero - perpendicular) and each of length one (*unit* vectors). "Ortho" for perpendicular, "normal" for unit length. The standard basis is orthonormal; so, crucially, is the DCT basis. A matrix whose columns form an orthonormal basis is called an *orthogonal matrix*, and orthogonal matrices are the central players in transform coding for one reason: *they conserve energy*.

Recall energy is squared length, $norm(bold(x))^2 = bold(x) dot bold(x)$. An orthogonal transform may *rotate or reflect* a vector - spin the arrow to point a new way - but it never stretches or shrinks it. The arrow keeps its length; only its direction (its coordinates) changes. So the total energy of the eight transformed coefficients exactly equals the total energy of the eight original pixels. Nothing is created or destroyed in the transform; energy is *redistributed* among the new coordinates, and a good transform redistributes it into a lopsided pile we can then cheaply quantise. Let us prove it.
...
#proof[
  First, a compact fact about the dot product: for any vectors $bold(u), bold(v)$, the dot product equals the matrix product $bold(u) dot bold(v) = bold(u)^T bold(v)$ (a row times a column is exactly "multiply matching slots and add"). Now compute, using this and one more rule (transposing a product flips its order: $(A B)^T = B^T A^T$, intuitively "undo, then redo, in reverse," the same reversal you see when taking off shoes-then-socks undoes socks-then-shoes):
...
where the $alpha_k$ are normalising constants chosen to make each row a *unit* vector. The exact formula matters less than the *shape* of the idea (we derive it properly in Chapter 38): the rows are a chosen orthonormal basis of cosine patterns, and multiplying a pixel block by this matrix *re-describes the block in terms of "how much of each cosine pattern it contains."*
...
#aside[
  Why cosines and not, say, the *Karhunen-Loeve transform* (KLT), which is provably the *optimal* energy-compacting orthogonal basis for a given source? Two practical reasons. The KLT's basis depends on the data's exact statistics, so you would have to compute it per image and ship it in the file: expensive and self-defeating. The DCT is a *fixed* matrix that, for the smoothly-correlated signals real photographs produce, gets startlingly close to the KLT's optimum while needing no transmission and admitting a much faster shortcut algorithm (the work it costs grows roughly like $n log n$ instead of the $n^2$ a plain matrix multiply needs, a distinction Chapter 14 explains). "Good enough, and free, and fast" beats "optimal, and costly." We meet the KLT and this trade-off properly in Chapter 38.
]
...
][You retain exactly the energy of the $2$ kept coefficients, because total energy is the sum of squared coefficients (the transform conserves it and orthonormal coordinates do not interfere). If those $2$ hold, say, $97%$ of the energy, you have discarded only $3%$. Since the transform preserves distances, that $3%$ is the distortion you will see in the reconstructed pixels. Energy compaction is *exactly* the promise that keeping few coefficients costs little distortion.]

== These same matrices are the whole of neural networks

One more dividend before we close, the one that pays off all of Volume IV. Open up any neural network (the kind powering the learned codecs of Chapter 57 and the language-models-as-compressors of Chapter 62) and you find this chapter, stacked. A single neural-network *layer* takes an input vector $bold(x)$, multiplies it by a matrix of learned numbers $W$ (the *weights*), adds an offset vector $bold(b)$ (the *bias*), and passes each result through a simple bend:

$ bold(y) = f(W bold(x) + bold(b)). $

That $W bold(x)$ is precisely the matrix--vector product we coded earlier in this chapter: a stack of dot products asking "how much does the input align with each learned pattern $W$ stores in its rows?" The bias $bold(b)$ is a vector addition. The only genuinely new ingredient is $f$, a tiny *non-linear* bend applied to each component (so the network can do more than rotate), and even that is a one-number function we will meet in Chapter 56. A "deep" network is many such layers composed, and composing linear maps is matrix multiplication, exactly as in Section 12.5. The trillion-parameter models of 2025--26 are, at the level of arithmetic, this chapter run at colossal scale: vectors flowing through matrices.

#note[
  This is also why compressing a neural network (Volume IV, Chapters 63--64) *is* compressing matrices: quantising the weight matrix $W$ to $4$ bits, factoring it into two thin matrices (low-rank / LoRA), or pruning its near-zero entries are all operations on the objects of this chapter. The modern *attention* mechanism is, at its core, a giant batch of dot products asking "how much does this token align with that one?," the very question Section 12.2 introduced. The vector and the matrix are the atoms of the entire AI era. You now know them.
]
...
#takeaways((
  ...
  [Matrix $times$ matrix = *composition* of transformations ("do $B$, then $A$"); it is associative but *not* commutative: order is meaning.],
...
#solution("12.4")[
  (a) Row 1: $1 dot 3 + 2 dot 2 = 7$; row 2: $0 dot 3 + 1 dot 2 = 2$. So $A bold(x) = (7, 2)$. (b) It left the second coordinate alone but added twice the second coordinate to the first: a horizontal *shear*, sliding points sideways by an amount proportional to their height. (c) $A^2 = mat(1, 2; 0, 1)mat(1, 2; 0, 1) = mat(1, 4; 0, 1)$. Shearing twice doubles the slide, giving a shear of strength $4$.
]
...
#solution("12.7")[
  (a) Energy $= 10^2 + 4^2 + 2^2 + 1^2 = 100 + 16 + 4 + 1 = 121$. (b) Kept energy $= 100 + 16 + 4 = 120$, fraction $120/121 approx 99.2%$. (c) Because *every* orthogonal transform conserves total energy ($Q^T Q = I$), and energy is the sum of squared coefficients regardless of the basis, the retained-energy fraction is a property of the coefficient list itself, not of which energy-preserving rotation produced it.
]
...
#solution("12.8")[
  Expand the squared norm as a dot product and use that the dot product distributes over addition (it is *bilinear*): $norm(bold(u)+bold(v))^2 = (bold(u)+bold(v))dot(bold(u)+bold(v)) = bold(u)dot bold(u) + bold(u)dot bold(v) + bold(v)dot bold(u) + bold(v)dot bold(v)$. The two cross terms are each $bold(u) dot bold(v) = 0$ by orthogonality, leaving $bold(u)dot bold(u) + bold(v)dot bold(v) = norm(bold(u))^2 + norm(bold(v))^2$. Because the cross terms vanish, energy along orthogonal directions simply adds with no interference, which is why a codec can store and discard orthonormal coefficients independently.
]
...
#solution("12.10")[
  A smooth signal - say $(50, 51, 52, 53)$ - is almost entirely "flatness plus a gentle trend," so it aligns strongly with the low-frequency DCT rows (the flat and slow-ripple patterns) and barely at all with the high-frequency rows. Its coefficients therefore come out large at the low end and near zero at the high end: the energy, which orthogonality conserves in total, piles into a few coordinates. Pure noise has no preferred direction. It aligns equally (and weakly) with *every* basis pattern, smooth or wiggly, so the conserved energy spreads evenly across all coefficients, none small enough to drop. (a) In both cases the *total* energy is unchanged by the transform; only its *distribution* differs: concentrated for smooth data, uniform for noise. (b) Compression by transform works only when energy concentrates; noise refuses to concentrate in any fixed basis, which is the geometric face of the deeper truth (Chapters 8 and 18) that random data is incompressible.
]

== Further reading

- Gilbert Strang, _Introduction to Linear Algebra_ (6th ed., 2023) and his MIT OpenCourseWare lectures: a geometry-first treatment of everything in this chapter. Start with the lectures on the four fundamental subspaces and orthogonality.
- N. Ahmed, T. Natarajan, and K. R. Rao, "Discrete Cosine Transform," #link("https://doi.org/10.1109/T-C.1974.223784")[_IEEE Transactions on Computers_, vol. C-23, no. 1, pp. 90--93, Jan. 1974]. The original three-page paper; remarkably readable.
- Gilbert Strang, "The Discrete Cosine Transform," #link("https://doi.org/10.1137/S0036144598336745")[_SIAM Review_, vol. 41, no. 1, pp. 135--147, 1999]. A linear-algebra-first account of why the DCT's cosines are the eigenvectors of a simple tridiagonal matrix, bridging this chapter directly to Chapter 38.
- Sheldon Axler, _Linear Algebra Done Right_ (4th ed., 2024): for the reader who wants the abstract, basis-and-transformation view developed rigorously. Harder than Strang, but the cleanest conceptual treatment of orthogonality and inner products.

#bridge[
  We can now turn lists of numbers into geometry, rotate them with energy-preserving matrices, and re-describe a signal in any vocabulary we please. That is the exact machinery the DCT (Chapter 38), wavelets (Chapter 43), and every neural layer (Volume IV) are built from. But we have leaned on one word without ever measuring it: *information*. We have said correlated data is compressible and noise is not, that a rare event "carries more information" than a common one. But how *much* more, in what units, and where is the hard floor below which no transform, no matrix, no cleverness can take us? For that we need the final foundation. Chapter 13 shows how a computer actually stores all these numbers as bits and bytes, and then Volume I culminates in Chapter 18, where Claude Shannon, in 1948, turns the vague word "information" into a precise number measured in bits, and tells us exactly how far compression can ever go.
]
