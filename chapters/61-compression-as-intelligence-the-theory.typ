...
This is the most ambitious idea in the whole book, and also the most contested. In the last chapter we watched neural audio codecs turn into the tokenizers that feed audio language models; before that, Chapter 23 promised an experiment in which a frozen language model out-compresses FLAC on audio and PNG on images, knowing nothing of sound or pictures. That was the _engineering_ punchline, and Chapter 62 will make it run. This chapter is the _theory_ underneath it — and, just as importantly, the honest list of reasons the slogan might be wrong, or at least incomplete. We will build the case carefully, brick by brick, and then we will try to knock it down.
...
The proof also leans on one more fact — that a measured average is, with high probability, _close_ to the true average it estimates. This is the single most useful idea in all of statistics, so it earns its own box.
...
*Expected value* (Chapter 10) is a weighted average of outcomes, each weighted by its probability — the "average payoff" of a gamble. If action $a$ leads to reward $10$ with probability $0.3$ and reward $0$ with probability $0.7$, its expected reward is $0.3 times 10 + 0.7 times 0 = 3$. A rational agent facing a choice computes the expected total future reward of each action and picks the largest — this is the "$max_a EE[...]$" at the heart of decision theory. AIXI does exactly this, except its expectation is taken over _all computable environments at once_, weighted by Solomonoff's universal prior. That is the only new ingredient: an ordinary reward-maximizer wearing a universal model of the world.
]
...
#fig([The thesis as a chain of equivalences, each link proved earlier in the book, and the place where the chain becomes a conjecture rather than a theorem.],
  cetz.canvas({
    ...
    let mkbox(x, y, w, t) = {
      ...
      content((x + w/2, y + 0.45), text(size: 8.5pt)[#t])
    }
...
#takeaways((
  ...
  [*AIXI is the ideal agent.* Solomonoff induction plus reward-maximizing decision theory yields the provably Pareto-optimal agent. Its beating heart is the universal compressor — the Hutter thesis: optimal compression and general intelligence are equivalent.],
...
#solution("61.7")[
The honest, MDL-fair size is $L(M) + L(D|M)$: the _model's own_ description length plus the coded residual. For a 70B model at, say, fp16, $L(M) approx 140$ GB, so $L(M) + L(D|M) approx 140 "GB" + 100 "MB" >> 1 "GB"$ — far _larger_ than the raw file. By the two-part accounting the model has not compressed this file; it has moved bits into its weights (an "expander," not a compressor, for a single file). It genuinely compresses only when amortized over a corpus far larger than the model: if the same frozen model codes, say, $10$ TB of text, the $140$ GB is a one-time cost spread thin and the _per-corpus_ total beats classical codecs. This is the MDL "cannot be fooled" theorem applied honestly: a model can only win in total bits if its structure plus residual beats raw storage — a memorizer (or an under-amortized giant) pays for itself in $L(M)$ and cannot profit. The thesis's own bookkeeping both powers and caps the LLM-compression claims.
]
